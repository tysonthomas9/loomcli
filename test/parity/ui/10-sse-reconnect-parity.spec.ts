...
test.describe("10 sse reconnect parity", () => {
  test("catch-up after disconnect/reconnect", async ({ tabs, browser }) => {
    ...
    try {
      ...
      await Promise.all([
        referenceObs.goto(`${PARITY_URLS.reference}/ws/${referenceWs}/${VIEW}`),
      ...
      await Promise.all([
        postIssueViaNode(PARITY_URLS.reference, referenceWs, gapTitleReference),
      ...
      // Latency parity: max(reference, fleet) <= 2 * min(reference, fleet),
    ...
    try {
      ...
      await Promise.all([
        ...referencePages.map((p) =>
          p.goto(`${PARITY_URLS.reference}/ws/${referenceWs}/${VIEW}`),
        ),
        ...fleetPages.map((p) =>
