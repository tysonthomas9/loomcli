...
const ISSUE_VIEWS = ["kanban", "table", "graph", "monitor", "settings"] as const;
...
test.describe("15 issue route coverage", () => {
    ...
    test("all issue-backed views and API routes are reachable", async ({
        ...
        const [beadsIssue, fleetIssue] = await Promise.all([
            findIssueByTitle(PARITY_URLS.beads, beadsWs, SEED_FIXTURE.children[0]),
            findIssueByTitle(PARITY_URLS.fleet, fleetWs, SEED_FIXTURE.children[0]),
        ]);

        for (const view of ISSUE_VIEWS) {
            await Promise.all([
                tabs.beads.goto(`${PARITY_URLS.beads}/ws/${beadsWs}/${view}`),
                tabs.fleet.goto(`${PARITY_URLS.fleet}/ws/${fleetWs}/${view}`),
            ]);
            await Promise.all([
                assertHealthyView(tabs.beads, view),
        ...
        expect(tabs.beads.url()).toContain(`/issues/${beadsIssue.id}`);
        expect(tabs.fleet.url()).toContain(`/issues/${fleetIssue.id}`);
        await Promise.all([
            apiJson(`${PARITY_URLS.beads}/api/workspaces/${beadsWs}/issues/${beadsIssue.id}`),
            apiJson(`${PARITY_URLS.fleet}/api/workspaces/${fleetWs}/issues/${fleetIssue.id}`),
        ]);

        await exerciseReadRoutes(PARITY_URLS.beads, beadsWs, beadsIssue.id, recordRouteHit, {
            requireIssueEvents: false,
        });
        await exerciseReadRoutes(PARITY_URLS.fleet, fleetWs, fleetIssue.id, recordRouteHit);
...
async function assertHealthyView(page: Page, view: string): Promise<void> {
    ...
    if (view === "kanban") {
        await page.waitForSelector('section[data-status], [data-testid="empty-workspace-board"]', {
            timeout: 15_000,
        });
        return;
    }
    ...
    if (view === "graph") {
        await page.waitForSelector(".react-flow, .react-flow__node", { timeout: 15_000 });
        return;
    }

    if (view === "monitor") {
        await expect(page.locator("body")).toContainText(/Agents|Queued|Blocked|Done/i);
        return;
    }
...
async function exerciseFleetWriteRoutes(
    ...
    expect([200, 201].includes(create.status), `create status=${create.status}`).toBeTruthy();
