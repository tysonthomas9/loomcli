...
test.describe("16 terminal fleet-db acceptance", () => {
    ...
    test("terminal route, metadata, state, and websocket attach work on fleet", async ({
        ...
        await Promise.all([
            tabs.beads.goto(`${PARITY_URLS.beads}/ws/${beadsWs}/terminal`),
            tabs.fleet.goto(`${PARITY_URLS.fleet}/ws/${fleetWs}/terminal`),
        ]);
        await Promise.all([
            assertTerminalShell(tabs.beads),
            assertTerminalShell(tabs.fleet),
        ]);

        const { id: issueId } = await findFleetIssueByTitle(SEED_FIXTURE.children[0]);
        ...
        expect(created.data?.session_name, "created terminal session").toBe(session);
