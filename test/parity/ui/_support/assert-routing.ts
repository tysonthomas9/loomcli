 ...
 * to the local reference store and every "parity pass" result would be
 * meaningless.
 *
...
function redisXLen(): number {
  try {
    const out = execSync(
      ...
      composeRun(
        `exec -T redis redis-cli XLEN fleet-db:${PARITY_URLS.workspace}:events`,
      ),
      {
        cwd: REPO_ROOT,
        encoding: "utf-8",
...
export async function assertRoutingForAction(
  ...
  if (browserDelta === 0) {
    ...
    verdict = "silent-fallback";
    notes.push(
      "Fleet-db saw the request but Redis events:PARITY did NOT grow — fleet-db rejected or stream not wired.",
    );
  }
