...
function urlFor(b: Backend): string {
  if (b === "reference" && isFleetOnlyMode()) return PARITY_URLS.fleet;
  return b === "reference" ? PARITY_URLS.reference : PARITY_URLS.fleet;
}
...
export async function resetBothBackends(
  ...
  resetInFlight = (async () => {
    ...
    if (isFleetOnlyMode()) {
      ...
      const referenceWs = await discoverWorkspaceId(PARITY_URLS.reference);
      await Promise.all([
        deleteAllIssues(PARITY_URLS.reference, referenceWs),
...
async function runSeedScript(): Promise<void> {
  ...
  const env: string[] = [
    ...
    "-e",
    `FLEET_WORKSPACE=${PARITY_URLS.workspace}`,
  ];
...
export async function findFleetIssueByTitle(
  ...
  const r = await fetch(`${PARITY_URLS.fleet}/api/workspaces/${wsId}/issues`);
  if (!r.ok) {
    throw new Error(
