...
test.describe("14 error-handling parity", () => {
  test("404 on missing issue id on both backends", async () => {
    ...
    const [b, f] = await Promise.all([
      fetch(
        `${PARITY_URLS.reference}/api/workspaces/${referenceWs}/issues/no-such-id-xyz`,
      ),
      fetch(
        `${PARITY_URLS.fleet}/api/workspaces/${fleetWs}/issues/no-such-id-xyz`,
      ),
    ]);
  ...
  test("422 on malformed create body on both backends", async () => {
    ...
    const [b, f] = await Promise.all([
      fetch(`${PARITY_URLS.reference}/api/workspaces/${referenceWs}/issues`, {
        method: "POST",
