...
test.describe("17 files and git diff fleet-db acceptance", () => {
    ...
    test("file tree/read/write and git diff routes work on fleet workspace mapping", async ({
        ...
        await Promise.all([
            tabs.beads.goto(`${PARITY_URLS.beads}/ws/${beadsWs}/files`),
            tabs.fleet.goto(`${PARITY_URLS.fleet}/ws/${fleetWs}/files`),
        ]);
        await Promise.all([
            assertFilesView(tabs.beads),
        ...
        expect(root.entries?.some((entry: any) => entry.name === ".init")).toBeTruthy();
...
async function assertFilesView(page: Page): Promise<void> {
    ...
    await expect(page.getByLabel("Select agent")).toBeVisible({ timeout: 15_000 });
    await expect(page.getByLabel("Filter files")).toBeVisible({ timeout: 15_000 });
}
...
function commitFleetWorktreeChange(
    ...
    execSync(`${composeRuntime()} exec loomcli-parity_loom-fleet_1 sh -lc ${shellQuote(command)}`, {
        encoding: "utf-8",
        timeout: 15_000,
