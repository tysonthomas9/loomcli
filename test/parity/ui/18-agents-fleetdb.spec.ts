 ...
 * and fleet-db-backed agent assignment APIs do not fall back to daemon/beads
 * control paths.
 */
...
test.describe("18 agents fleet-db acceptance", () => {
    ...
    test("agent monitor, sidebar, assignments, and daemon-free controls work in fleet mode", async ({
        ...
        await Promise.all([
            tabs.beads.goto(`${PARITY_URLS.beads}/ws/${beadsWs}/monitor`),
            tabs.fleet.goto(`${PARITY_URLS.fleet}/ws/${fleetWs}/monitor`),
        ]);
        await Promise.all([
            assertMonitorAndSidebar(tabs.beads),
        ...
        expect(status.agents?.some((agent: any) => agent.name === "workspace")).toBeTruthy();
        expect(status.tasks?.needs_planning, "monitor queue count").toBeGreaterThan(0);

        const agents = await apiJson(
            `${PARITY_URLS.fleet}/api/monitor/agents`,
            recordRouteHit,
        );
        expect(agents.by_workspace?.[fleetWs]?.some((agent: any) => agent.name === "workspace")).toBeTruthy();

        const tasks = await apiJson(
            `${PARITY_URLS.fleet}/api/monitor/tasks`,
            recordRouteHit,
        );
        expect(tasks.summary?.needs_planning, "task queue summary").toBeGreaterThan(0);
        ...
        expect(listed.data?.some((agent: any) => agent.name === agentName && agent.state === "active")).toBeTruthy();

        const queue = await fetchReachable(
            `${PARITY_URLS.fleet}/api/workspaces/${fleetWs}/agents/${agentName}/queue`,
        );
        recordRouteHit(`${PARITY_URLS.fleet}/api/workspaces/${fleetWs}/agents/${agentName}/queue`);
        expect(queue.status, "fleet agent queue unsupported status").toBe(501);
        const queueBody = await queue.json();
        expect(queueBody.kind, "fleet agent queue error kind").toBe("not_implemented");
        expect(queueBody.error, "fleet agent queue does not mention daemon").not.toContain("daemon");

        const control = await fetchReachable(
            `${PARITY_URLS.fleet}/api/workspaces/${fleetWs}/agents/${agentName}/stop`,
            { method: "POST" },
        );
        recordRouteHit(`${PARITY_URLS.fleet}/api/workspaces/${fleetWs}/agents/${agentName}/stop`);
        expect(control.status, "fleet agent control unsupported status").toBe(501);
        const controlBody = await control.json();
        expect(controlBody.kind, "fleet agent control error kind").toBe("not_implemented");
        expect(controlBody.error, "fleet agent control does not report daemon unavailable").not.toContain("daemon is not running");

        await tabs.fleet.goto(`${PARITY_URLS.fleet}/ws/${fleetWs}/monitor`);
        await assertMonitorAndSidebar(tabs.fleet);
        await expect(tabs.fleet.getByText(agentName).first()).toBeVisible({ timeout: 15_000 });
...
async function assertMonitorAndSidebar(page: Page): Promise<void> {
    ...
    await expect(page.getByTestId("monitor-dashboard")).toBeVisible({ timeout: 15_000 });
    await expect(page.getByTestId("agent-activity-panel")).toBeVisible({ timeout: 15_000 });
    await expect(page.getByText("Agents").first()).toBeVisible({ timeout: 15_000 });
    await expect(page.getByText("workspace").first()).toBeVisible({ timeout: 15_000 });
}

async function createFleetRole(workspace: string, roleName: string): Promise<void> {
    const response = await fetchReachable(`${PARITY_URLS.fleetDB}/api/v1/${workspace}/roles`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
            name: roleName,
            description: "Parity browser role for fleet-db agent assignment coverage",
            backend: "claude",
        }),
    });
    expect(response.ok, `create fleet role status=${response.status}`).toBeTruthy();
...
async function fetchReachable(url: string, init?: RequestInit): Promise<Response> {
