...
func TestLocalModeUsesSingleServeRuntime(t *testing.T) {
	raw, err := os.ReadFile("local-mode-entrypoint")
	if err != nil {
		t.Fatal(err)
	}
	source := string(raw)
	if strings.Contains(source, "loom daemon") || strings.Contains(source, "daemon profile") {
		t.Fatal("local-mode entrypoint still starts or configures the retired daemon")
	}
	if !strings.Contains(source, "loom serve --bind \"$LOOM_BIND\" --port \"$LOOM_PORT\" --fleet-mode") {
		t.Fatal("local-mode entrypoint must host agent execution in loom serve")
	}
}

func TestLocalModeKeepsAdmittedRepositoryRemote(t *testing.T) {
	raw, err := os.ReadFile("local-mode-entrypoint")
	if err != nil {
		t.Fatal(err)
	}
	source := string(raw)
	if strings.Contains(source, "remote remove origin") {
		t.Fatal("local-mode entrypoint removes the admitted repository remote shared by linked worktrees")
	}
	for _, want := range []string{
		...
		if !strings.Contains(source, want) {
			t.Fatalf("local-mode entrypoint missing %q", want)
		}
	}
}
