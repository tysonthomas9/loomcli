/**
 * 06 Issue-detail fleetdb-regression — every REQUIRED_FIELD from §5 rendered on both.
 */
import {
  fleetdbTest as test,
  expect,
...
test.describe("06 issue detail fleetdb-regression", () => {
  test("every required field renders on both tabs", async ({ tabs }) => {
    ...
    const [referenceList, fleetList] = await Promise.all([
      fetch(
        `${FLEETDB_URLS.reference}/api/workspaces/${referenceWs}/issues`,
      ).then((r) => r.json()),
      fetch(`${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/issues`).then((r) =>
        r.json(),
      ),
    ]);
    ...
    // own. Fetch each directly and assert field consistency with tolerated
