/**
 * 08 Dependencies fleetdb-regression — add / remove / blocks chain.
 */
import {
  fleetdbTest as test,
  expect,
...
test.describe("08 dependencies fleetdb-regression", () => {
  test("add and remove a dep on fleet — routing proven", async ({
    ...
    await tabs.fleet.goto(
      `${FLEETDB_URLS.fleet}/ws/${FLEETDB_URLS.workspace}/issues/${blockedId}`,
    );
