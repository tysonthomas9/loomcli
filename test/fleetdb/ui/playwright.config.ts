 ...
 *   PLAYWRIGHT_REFERENCE_UI_URL  default http://localhost:8084 in dual mode
 *   PLAYWRIGHT_FLEET_UI_URL      default http://localhost:8082
 *   PLAYWRIGHT_FLEET_DB_URL      default http://localhost:8080
 *   PLAYWRIGHT_REDIS_URL         default redis://localhost:6379 (used by redis-cli probe)
 *   PLAYWRIGHT_FLEETDB_WORKSPACE default FLEETDB
 *   PLAYWRIGHT_FLEETDB_MODE      dual (default) or fleet-only
...
// can override via PLAYWRIGHT_REFERENCE_UI_URL / PLAYWRIGHT_FLEET_UI_URL
// to "http://localhost:8081" or 8082. fleetDB stays at 8080 since several tests poke fleet-db
// directly to confirm the through-loom path actually wrote.
export const FLEETDB_URLS = {
  fleet: process.env.PLAYWRIGHT_FLEET_UI_URL ?? "http://localhost:8083",
  fleetDB: process.env.PLAYWRIGHT_FLEET_DB_URL ?? "http://localhost:8080",
  redis: process.env.PLAYWRIGHT_REDIS_URL ?? "redis://localhost:6379",
  workspace: process.env.PLAYWRIGHT_FLEETDB_WORKSPACE ?? "FLEETDB",
  mode: fleetdbMode(),
  ...
  reference: isFleetOnlyMode()
    ? (process.env.PLAYWRIGHT_FLEET_UI_URL ?? "http://localhost:8083")
    : (process.env.PLAYWRIGHT_REFERENCE_UI_URL ?? "http://localhost:8084"),
};
