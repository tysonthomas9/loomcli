/**
 * 09 SSE / realtime fleetdb-regression — create in tab1 reaches tab2 within 2s.
 *
 * Opens THREE pages: the standard dual tabs (for fleetdb-regression) plus a second
...
test.describe("09 SSE realtime fleetdb-regression", () => {
  test("event propagates to second tab within 2x of other side", async ({
    ...
    await Promise.all([
      referenceObs.goto(`${FLEETDB_URLS.reference}/ws/${referenceWs}/kanban`),
      fleetObs.goto(`${FLEETDB_URLS.fleet}/ws/${fleetWs}/kanban`),
      ...
      tabs.reference.goto(`${FLEETDB_URLS.reference}/ws/${referenceWs}/kanban`),
      tabs.fleet.goto(`${FLEETDB_URLS.fleet}/ws/${fleetWs}/kanban`),
    ]);
    ...
    try {
      const referenceTitle = `fleetdb-regression-sse-reference-${Date.now()}`;
      const fleetTitle = `fleetdb-regression-sse-fleet-${Date.now()}`;
