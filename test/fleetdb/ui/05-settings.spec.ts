/**
 * 05 Settings fleetdb-regression — backend selector shows correct backend per tab.
 *
...
test.describe("05 settings fleetdb-regression", () => {
  test("backend selector shows reference / fleet correctly", async ({
    ...
    const [referenceCfg, fleetCfg] = await Promise.all([
      fetch(`${FLEETDB_URLS.reference}/api/config`).then((r) => r.json()),
      fetch(`${FLEETDB_URLS.fleet}/api/config`).then((r) => r.json()),
    ]);
