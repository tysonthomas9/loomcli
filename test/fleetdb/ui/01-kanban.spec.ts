/**
 * 01 Kanban fleetdb-regression — swim lane count, drag-drop status change, ordering.
 *
...
test.describe("01 kanban fleetdb-regression", () => {
  test("both tabs render the same swim-lane counts", async ({
    tabs,
    stateBefore,
  }) => {
    ...
    await Promise.all([
      tabs.reference.waitForSelector("section[data-status]", {
        timeout: 15_000,
      }),
    ...
    const struct = await structuralDiff(tabs.reference, tabs.fleet);
    ...
    const [referenceCards, fleetCards] = await Promise.all([
      tabs.reference.locator("section[data-status] article").count(),
      tabs.fleet.locator("section[data-status] article").count(),
    ]);
    expect(referenceCards).toBe(fleetCards);
    ...
    if (!struct.match) {
      ...
      console.log(
        `[01-kanban] structural diff: reference=${struct.reference_nodes} fleet=${struct.fleet_nodes} extras=${struct.extra_on_reference.length}/${struct.extra_on_fleet.length}`,
      );
    }
    ...
    // but do NOT fail the test on it — fleetdb-regression on card counts + API
    // payload (above) is the load-bearing assertion.
    // eslint-disable-next-line no-console
    console.log(`[01-kanban] visual byte-diff ratio=${vis.ratio}`);
  });

  test("drag-drop status change routes through fleet-db", async ({
    tabs,
    fleetSpy,
  }) => {
    test.fail(!SEED_FIXTURE.children.length, "no seed fixture available");
    await gotoViews(tabs, "kanban");
    // Issue cards render as <article aria-label="Issue: <title>"> with no
    // data-id attribute — look up the id via the API by seed title.
    await tabs.fleet.waitForSelector('article[aria-label^="Issue:"]', {
      timeout: 15_000,
    });
    ...
    const cur = await fetch(
      `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/issues/${id}`,
    ).then((r) => r.json());
