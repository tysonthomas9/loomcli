/**
 * 02 Table fleetdb-regression — row count, sort, filter.
 */
import {
  fleetdbTest as test,
  expect,
...
test.describe("02 table fleetdb-regression", () => {
  test("row count + sort + filter match", async ({ tabs }) => {
    ...
    // on; we don't hard-fail on that drift, fleetdb-regression at the API layer
    ...
    // API-level fleetdb-regression: both backends must surface at least the seed
