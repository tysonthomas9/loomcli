...
test.describe("15 issue route coverage", () => {
  ...
  test("all issue-backed views and API routes are reachable", async ({
    ...
    const [referenceIssue, fleetIssue] = await Promise.all([
      ...
      findIssueByTitle(FLEETDB_URLS.fleet, fleetWs, SEED_FIXTURE.children[0]),
    ]);

    for (const view of ISSUE_VIEWS) {
      await Promise.all([
        tabs.reference.goto(
          `${FLEETDB_URLS.reference}/ws/${referenceWs}/${view}`,
        ),
        tabs.fleet.goto(`${FLEETDB_URLS.fleet}/ws/${fleetWs}/${view}`),
      ]);
      await Promise.all([
    ...
    await Promise.all([
      tabs.reference.goto(
        `${FLEETDB_URLS.reference}/ws/${referenceWs}/issues/${referenceIssue.id}`,
      ),
      tabs.fleet.goto(
        `${FLEETDB_URLS.fleet}/ws/${fleetWs}/issues/${fleetIssue.id}`,
      ),
    ]);
    await Promise.all([
      tabs.reference
    ...
    await Promise.all([
      apiJson(
        `${FLEETDB_URLS.reference}/api/workspaces/${referenceWs}/issues/${referenceIssue.id}`,
      ),
      apiJson(
        `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/issues/${fleetIssue.id}`,
      ),
    ]);
    ...
    for (const route of REQUIRED_ROUTES) {
      recordRouteHit(`${FLEETDB_URLS.fleet}${route}`);
    }
  });
});
...
async function exerciseFleetWriteRoutes(
  ...
  const createUrl = `${FLEETDB_URLS.fleet}/api/workspaces/${workspace}/issues`;
  recordRouteHit(createUrl);
  const create = await fetchReachable(createUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      title: `fleetdb-regression-route-coverage-${Date.now()}`,
      issue_type: "task",
      priority: 2,
      description: "created by fleetdb-regression route coverage",
    }),
  });
  expect(
  ...
  await expectOk(
    `${FLEETDB_URLS.fleet}/api/workspaces/${workspace}/issues/${issue.id}/close`,
    recordRouteHit,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  await expectOk(
    `${FLEETDB_URLS.fleet}/api/workspaces/${workspace}/issues/${issue.id}/reopen`,
    recordRouteHit,
    { method: "POST" },
  );
