...
import { composeRun, composeRuntime, FLEETDB_CONTAINER_PREFIX } from "./compose";
...
async function runAllChecks(): Promise<PreflightResult> {
  ...
  if (fleetOnly) {
    ...
    try {
      const j = await httpJson(`${FLEETDB_URLS.reference}/api/config`);
  ...
  // 2. fleet config
  try {
    const j = await httpJson(`${FLEETDB_URLS.fleet}/api/config`);
    const backend = j.issue_backend ?? j.backend ?? "(missing)";
    checks.push({
  ...
  try {
    const ps = execSync(
      `${composeRuntime()} ps --filter name=${FLEETDB_CONTAINER_PREFIX} --format '{{.Names}}|{{.Status}}'`,
      {
        cwd: path.resolve(__dirname, "../../../.."),
        encoding: "utf-8",
        timeout: 10_000,
  ...
  try {
    ...
    const probe = await httpPost(
      ...
      {
        title: `fleetdb-regression-preflight-${Date.now()}`,
        issue_type: "task",
        priority: 3,
    ...
    const inboundPath = `/api/v1/${FLEETDB_URLS.workspace}/issues`;
  ...
  // 7. fleet-db has workspace FLEETDB.
  ...
  try {
    ...
    const has = keys.includes("FLEETDB");
    checks.push({
      name: "fleet-db has workspace FLEETDB",
      expected: "yes",
...
function preflightError(r: PreflightResult): Error {
  ...
  return new Error(
    `FleetDB Regression preflight failed — cannot run suite.\n\n` +
      ...
      `The docker-compose.regression.yml stack must be up and healthy. Run:\n` +
      `${setup}\n\n` +
