...
// Routes the regression suite must exercise at least once. The
...
const coverageHitsPath = path.join(
  ARTIFACTS_DIR,
  "reports",
  "coverage-hits.ndjson",
);
...
export function normalizeUrlToRoute(url: string): string | null {
  try {
    ...
    p = p.replace(
      /\/issues\/[^/]+\/(close|reopen|labels|comments|dependencies|deps|events)$/,
      "/issues/:id/$1",
    );
    ...
    const known = REQUIRED_ROUTES.find(
      (r) => r === p || r === p.replace(/\/$/, ""),
    );
...
export async function writeCoverageReport(): Promise<{
  ...
  for (const r of allRecords)
    for (const rt of r.routes_exercised) covered.add(rt);
