/**
 * Dual-backend drivers: helpers to open both reference (:8081) and fleet (:8082)
 ...
 * recorder + request intercept without polluting the reference tab.
 */
...
export type Backend = "reference" | "fleet";
...
function urlFor(b: Backend): string {
  if (b === "reference" && isFleetOnlyMode()) return FLEETDB_URLS.fleet;
  return b === "reference" ? FLEETDB_URLS.reference : FLEETDB_URLS.fleet;
}
 ...
 * and test/fleetdb/Caddyfile for the intended path.
 */
export async function openDualTabs(
  browser: Browser,
  testId: string,
): Promise<DualTabs> {
  ...
  const referenceCtx = await browser.newContext({
    recordHar: { path: path.join(harDir, "reference.har"), mode: "full" },
  ...
  const reference = await referenceCtx.newPage();
  ...
  return {
    ...
    testId: safeId,
    close: async () => {
      await Promise.allSettled([referenceCtx.close(), fleetCtx.close()]);
    },
  };
}
...
export async function gotoBoth(tabs: DualTabs, relPath: string): Promise<void> {
  ...
  await Promise.all([
    tabs.reference.goto(`${urlFor("reference")}${relPath}`),
    tabs.fleet.goto(`${urlFor("fleet")}${relPath}`),
  ]);
}

/**
 * Go to the fleetdb-regression workspace's kanban/table/etc. page on both tabs and
 ...
 * strings like "default"/"FLEETDB" never match an actual workspace.
 */
export async function gotoViews(
  ...
  const [b, f] = await Promise.all([
    wsReference ?? discoverWorkspaceId(urlFor("reference")),
    wsFleet ?? discoverWorkspaceId(urlFor("fleet")),
  ]);
  await Promise.all([
    tabs.reference.goto(`${urlFor("reference")}/ws/${b}/${view}`),
  ...
  await Promise.all([
    tabs.reference.waitForLoadState("domcontentloaded").catch(() => undefined),
...
export async function gotoIssueDetail(
  ...
  await Promise.all([
    tabs.reference.goto(
      `${urlFor("reference")}/ws/${b}/issues/${issueIdReference}`,
    ),
...
export async function resetBothBackends(
  opts: { reseed?: boolean } = {},
): Promise<void> {
  ...
  resetInFlight = (async () => {
    ...
    if (isFleetOnlyMode()) {
      await deleteAllIssues(FLEETDB_URLS.fleet, fleetWs);
    } else {
      const referenceWs = await discoverWorkspaceId(FLEETDB_URLS.reference);
      await Promise.all([
        deleteAllIssues(FLEETDB_URLS.reference, referenceWs),
        deleteAllIssues(FLEETDB_URLS.fleet, fleetWs),
      ]);
    }
...
async function resetFleetOnlyFixture(): Promise<void> {
  const actorHeaders = { "X-Actor": "fleetdb-regression-harness@fixture.local" };
  await deleteFleetDBIssues(actorHeaders);
  await fetch(
    `${FLEETDB_URLS.fleetDB}/api/v1/admin/workspaces/${encodeURIComponent(FLEETDB_URLS.workspace)}?force=true`,
    { method: "DELETE", headers: actorHeaders },
  ).catch(() => undefined);

  const createWorkspace = await fetch(
    `${FLEETDB_URLS.fleetDB}/api/v1/admin/workspaces`,
    {
      method: "POST",
      headers: { ...actorHeaders, "Content-Type": "application/json" },
      body: JSON.stringify({
        key: FLEETDB_URLS.workspace,
        name: "FleetDB Regression Fixture",
      }),
    },
  );
  if (!createWorkspace.ok && createWorkspace.status !== 409) {
    throw new Error(
      `fleet-only workspace reset failed: HTTP ${createWorkspace.status} ${await createWorkspace.text()}`,
    );
  }

  for (const issue of FLEET_SEED_ISSUES) {
    const response = await fetch(
      `${FLEETDB_URLS.fleetDB}/api/v1/${encodeURIComponent(FLEETDB_URLS.workspace)}/issues`,
      {
        method: "POST",
        headers: { ...actorHeaders, "Content-Type": "application/json" },
    ...
    if (!response.ok) {
      throw new Error(
        `fleet-only seed failed for ${issue.title}: HTTP ${response.status} ${await response.text()}`,
      );
    }
  }
}

async function deleteFleetDBIssues(
  actorHeaders: Record<string, string>,
): Promise<void> {
  const list = await fetch(
    `${FLEETDB_URLS.fleetDB}/api/v1/${encodeURIComponent(FLEETDB_URLS.workspace)}/issues?limit=1000`,
  ...
  for (let i = 0; i < issues.length; i += batchSize) {
    await Promise.all(
      issues.slice(i, i + batchSize).map((issue) => {
        ...
        return fetch(
          `${FLEETDB_URLS.fleetDB}/api/v1/${encodeURIComponent(FLEETDB_URLS.workspace)}/issues/${encodeURIComponent(id)}?force=true`,
          { method: "DELETE", headers: actorHeaders },
        ).catch(() => undefined);
      }),
    );
...
async function runSeedScript(): Promise<void> {
  ...
  const seedHostPath = path.resolve(REPO_ROOT, "test/fleetdb/seed.sh");
  const image = FLEETDB_SEED_FLEET_IMAGE;
  const env: string[] = [
    "-e",
    `FLEET_URL=http://fleet-db:8080`,
    "-e",
    `FLEET_WORKSPACE=${FLEETDB_URLS.workspace}`,
  ];
  const envFlags = env.join(" ");
  const cmd = `${composeRuntime()} run --rm --network ${FLEETDB_NETWORK} ${envFlags} -v ${seedHostPath}:/seed.sh:ro --entrypoint /bin/sh ${image} /seed.sh`;
  try {
    execSync(cmd, {
      cwd: REPO_ROOT,
      encoding: "utf-8",
...
export async function snapshotState(
  ...
  reference: BackendState;
  fleet: BackendState;
  label: "before" | "after";
}> {
  const [reference, fleet] = await Promise.all([
    captureOne("reference"),
    captureOne("fleet"),
  ]);
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ reference, fleet }, null, 2),
  );
  return { reference, fleet, label };
}
...
async function captureOne(b: Backend): Promise<BackendState> {
  ...
  const [issuesR, statsR, configR] = await Promise.allSettled([
    fetch(`${base}/api/workspaces/${ws}/issues`).then((r) =>
      r.ok ? r.json() : null,
    ),
    fetch(`${base}/api/workspaces/${ws}/stats`).then((r) =>
      r.ok ? r.json() : null,
    ),
 ...
 * a literal "default" or "FLEETDB" string in the URL — the actual ID is
...
export async function discoverWorkspaceId(baseUrl: string): Promise<string> {
  ...
  // Primary: /api/workspaces (loom-reference populates this correctly).
  ...
  // the workspace name (e.g. "FLEETDB"). We accept that as the workspace id
...
export async function findFleetIssueByTitle(
  ...
  const wsId = await discoverWorkspaceId(FLEETDB_URLS.fleet);
  const r = await fetch(`${FLEETDB_URLS.fleet}/api/workspaces/${wsId}/issues`);
  if (!r.ok) {
    throw new Error(
