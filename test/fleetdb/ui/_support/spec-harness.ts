 ...
 *
 * Usage:
 *   import { fleetdbTest } from "./_support/spec-harness";
 *   fleetdbTest("does X", async ({ tabs, recordRouteHit }) => {
 *       ...
 *   });
 */
...
export interface FleetDBFixtures {
  tabs: DualTabs;
...
export const fleetdbTest = base.extend<FleetDBFixtures>({
  tabs: async ({ browser }, use, testInfo) => {
    ...
    tabs.reference.on("request", collect);
    ...
    try {
      ...
      if (testInfo.status !== testInfo.expectedStatus) {
        await saveForensics(tabs.testId, tabs.reference, tabs.fleet);
      }
      await tabs.close();
    }
  },
 ...
 */
export function useFleetDBHooks() {
  fleetdbTest.beforeAll(async () => {
    // Skip sanity gate on operator request — see global-setup.ts.
    if (process.env.FLEETDB_SKIP_PREFLIGHT === "1") {
      return;
    }
  ...
  fleetdbTest.beforeEach(async ({}, testInfo) => {
    // The reseed runs `podman compose run --rm fleetdb-regression-seed`, which
