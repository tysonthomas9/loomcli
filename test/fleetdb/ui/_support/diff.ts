...
const TOLERATED_MISMATCH_FIELDS = new Set([
  // Known legitimate cross-backend differences per the regression report
  "created_by",
  "source_repo",
  "repo",
...
export function diffIssueLists(
  ...
  for (const i of referenceList) bMap.set(keyFor(i), i);
  const fMap = new Map<string, any>();
  for (const i of fleetList) fMap.set(keyFor(i), i);

  if (referenceList.length !== fleetList.length) {
    diffs.push({
      field: "__count__",
  ...
  return {
    endpoint: "/api/issues",
    count_reference: referenceList.length,
 ...
 * "issues/ready". When `wsReference` / `wsFleet` are omitted, each backend's
 * workspace ID is discovered at call time via `/api/workspaces` — loom
 * uses UUID workspace IDs, so literals like "default" or "FLEETDB" never
...
export async function apiResponseDiff(
  ...
  const [bWs, fWs] = await Promise.all([
    wsReference ?? discoverWorkspaceId(FLEETDB_URLS.reference),
    wsFleet ?? discoverWorkspaceId(FLEETDB_URLS.fleet),
  ]);
  const [b, f] = await Promise.all([
    fetchJson(
      `${FLEETDB_URLS.reference}/api/workspaces/${bWs}/${endpointSuffix}`,
    ),
    fetchJson(`${FLEETDB_URLS.fleet}/api/workspaces/${fWs}/${endpointSuffix}`),
  ]);
...
export function stateSyncDiff(
  before: { reference: BackendState; fleet: BackendState },
  after: { reference: BackendState; fleet: BackendState },
  label: string,
): StateSyncDiff {
  const bCount = after.reference.issues.length;
  const fCount = after.fleet.issues.length;
  const notes: string[] = [];
  if (bCount !== fCount) {
    notes.push(`issue count diverged: reference=${bCount} fleet=${fCount}`);
  }
  const referenceDelta = bCount - before.reference.issues.length;
  const fd = fCount - before.fleet.issues.length;
  if (referenceDelta !== fd) {
    notes.push(`issue delta diverged: reference+${referenceDelta} fleet+${fd}`);
  }
  // Stats fleetdb-regression: compare open/in_progress/closed counts when both sides
  // expose them.
  const bs = before.reference.stats ?? {};
 ...
 * ceiling instead of the strict 2x ratio — a 9ms-vs-800ms reference/fleet
...
export async function timingAssert(
  action: string,
  times: { reference: number; fleet: number },
): Promise<TimingResult> {
  const fastest = Math.min(times.reference, times.fleet);
  const slowest = Math.max(times.reference, times.fleet);
  let within: boolean;
  ...
  return {
    action,
    reference_ms: times.reference,
    fleet_ms: times.fleet,
    within_2x: within,
  };
}
