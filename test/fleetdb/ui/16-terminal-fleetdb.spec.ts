...
test.describe("16 terminal fleet-db acceptance", () => {
  ...
  test("terminal route, metadata, state, and websocket attach work on fleet", async ({
    ...
    await Promise.all([
      tabs.reference.goto(
        `${FLEETDB_URLS.reference}/ws/${referenceWs}/terminal`,
      ),
      tabs.fleet.goto(`${FLEETDB_URLS.fleet}/ws/${fleetWs}/terminal`),
    ]);
    await Promise.all([
    ...
    const session = `fleetdb-regression-terminal-${Date.now()}`;
    const tabUrl = `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/terminal/tabs/${session}`;

    const initialTabs = await apiJson(
      `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/terminal/tabs`,
      recordRouteHit,
    );
    ...
    const created = await apiJson(tabUrl, recordRouteHit, {
      ...
      body: JSON.stringify({
        label: "FleetDB Regression terminal",
        sort_order: 1,
        notes: "created by fleetdb-regression 16",
        pinned: false,
      }),
    });
    ...
    expect(readBack.data?.label, "terminal tab label").toBe("FleetDB Regression terminal");

    const patched = await apiJson(tabUrl, recordRouteHit, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        notes: "patched by fleetdb-regression 16",
        pinned: true,
        issue_id: issueId,
      }),
    });
    ...
    const byIssue = await apiJson(
      `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/terminal/sessions/by-issue`,
      recordRouteHit,
    );
    ...
    await apiJson(
      `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/terminal/state`,
      recordRouteHit,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    const tokenBody = await apiJson(
      `${FLEETDB_URLS.fleet}/api/workspaces/${fleetWs}/terminal/token?session=${encodeURIComponent(session)}`,
      recordRouteHit,
    );
