...
# Run every shared storetest conformance suite against memstore and a real
# embedded fleet-db. This lane fails closed when fleet-db is unavailable.
conformance:
	@echo "Running store conformance against memstore and embedded fleet-db..."
	@LOOM_RUN_EMBEDDED_SMOKE=1 LOOM_REQUIRE_EMBEDDED_SMOKE=1 \
		go test -count=1 ./internal/infra/memstore ./internal/infra/fleetdb -run Conformance
