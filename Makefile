...
LOCAL_MODE_POSTGRES_COMPOSE_ARGS = -p $(LOCAL_MODE_COMPOSE_PROJECT) -f test/local-mode/docker-compose.yml -f test/local-mode/docker-compose.postgres.yml $(LOCAL_MODE_COMPOSE_EXTRA)
...
# PostgreSQL committed-workspace bootstrap with the standard deterministic backend.
local-mode-postgres-up: local-mode-frontend-dist
	@echo "Starting PostgreSQL local-mode stack ($(LOCAL_MODE_COMPOSE_PROJECT)) on http://localhost:$${LOCAL_MODE_UI_PORT:-8283}/ws/LOCALMODE/kanban..."
	@set -e; \
	$(LOCAL_MODE_COMPOSE_SELECT); \
...
local-mode-postgres-down:
	@set -e; \
	$(LOCAL_MODE_COMPOSE_SELECT); \
	$$compose $(LOCAL_MODE_POSTGRES_COMPOSE_ARGS) down -v --remove-orphans
