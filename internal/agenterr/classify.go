...
func classifyHarnessMarkers(text string) *classifyResult {
	...
	if runTurnDeadlineRe.MatchString(text) {
		return &classifyResult{
			...
			// the residual timeout pattern this arm exists to outrank — which
			// is the whole point of the arm, and unreadable without a record
			// saying which one fired. The Detail tail carries the resolved
			// deadline the emitter names.
