...
// classifyFromText recognizes it before any per-backend pattern so the
// supervisor records a retryable SpawnFailure carrying the real reason, instead
// of falling through to Unknown / "unclassified error (exit code 1)" — the
// generic message that previously hid this cause from the operator and UI.
//
