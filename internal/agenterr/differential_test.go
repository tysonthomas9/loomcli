...
// taken from this branch's own base, loom/PUPPET-442 at 3fb769731 — the commit
// BEFORE the move, with the wrapper pin it had (v0.8.5).
//
...
const (
	...
	differentialGolden = "testdata/differential_puppet442_3fb769731.golden.json"
)
...
var declaredDeviations = map[string]deviation{
	...
	"marker run-turn deadline": {becomes: "", why: "" +
		"The run-turn-deadline arm gains an Evidence record " +
		"(source=harness_marker, rule=RunTurnDeadlineMarker), which local/union " +
		"already carries. That arm exists to OUTRANK the residual timeout " +
		"pattern — a bare `context deadline exceeded` would otherwise read as " +
		"`connection timeout` and point an operator at the network instead of " +
		"the role's time budget — and which of the two fired is unreadable " +
		"without a record saying so. The verdict does not move."},

	"marker outranks residual": {becomes: "", why: "" +
		"Same arm, same record: this row is the one that PROVES the marker " +
		"beats the residual timeout pattern when both are in the text, so it " +
		"gains the same evidence. The verdict does not move."},
...
func TestDifferentialParityAgainstBranchBase(t *testing.T) {
...
func TestDifferentialCorpusCoverage(t *testing.T) {
	...
	// Computed from the CURRENT pipeline, not from the golden. The golden is a
	// record of the past, and an arm that gained evidence in this change would
	// read as uncovered if the baseline is what gets counted — which is exactly
	// backwards: the question is whether the gate still exercises every arm
	// AFTER the change.
	...
	for _, in := range corpus {
		e := ClassifyFromOutput(in.Text, in.ExitCode, in.Backend)
		rules[e.Evidence.Rule]++
		sources[e.Evidence.Source]++
		backends[in.Backend]++
	}
