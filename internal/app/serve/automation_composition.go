// Automation composition binds the capability core, named application
// workflows, typed authority providers, and runtime registrations inside the
// serve root. It exposes only narrow ports to HTTP composition; no adapter
// receives the core's issuer or the process-wide FleetDB client.
package serve
...
func composeAutomationCapability(config automationCapabilityConfig, dependencies automationCapabilityDependencies) (*AutomationCapability, error) {
	...
	service := automation.New(
		...
		config.catalog.effectiveVersions,
		newAutomationCatalogAuthorityProvider(config.catalog.effectiveVersionAuthority),
		...
		automation.WithEventTrustPolicy(automation.EventPolicy{}),
...
func validateAutomationCapabilityConfig(config automationCapabilityConfig) error {
	if config.catalog.issuer == nil || config.catalog.effectiveVersions == nil ||
		config.catalog.effectiveVersionAuthority == nil ||
		config.catalog.operatorResolver == nil {
