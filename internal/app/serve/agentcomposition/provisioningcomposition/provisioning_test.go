...
type agentProvisioningAgentsStub struct{}

func (agentProvisioningAgentsStub) EnsureManagedRole(
	context.Context,
...
func (agentProvisioningAgentsStub) EnsureManagedAgent(
	context.Context,
	authority.SystemAuthority,
...
type agentProvisioningBindingsStub struct{}

func (agentProvisioningBindingsStub) EnsureManagedBinding(
	context.Context,
	authority.SystemAuthority,
...
type agentProvisioningGrantsStub struct{}

func (agentProvisioningGrantsStub) EnsureGrant(
	context.Context,
	authority.SystemAuthority,
	connectors.EnsureGrantCommand,
...
func TestAgentProvisioningCompositionPublishesOnlyCommandsAuthorityAndRecovery(t *testing.T) {
	now := time.Now()
	agentsIssuer := fixedAuthorityIssuer(t, now)
	automationIssuer := fixedAuthorityIssuer(t, now)
	connectorsIssuer := fixedAuthorityIssuer(t, now)
	...
	capability, err := composeAgentProvisioningCapability(agentProvisioningDependencies{
		...
		agents:   agentProvisioningAgentsStub{}, bindings: agentProvisioningBindingsStub{},
		grants:       agentProvisioningGrantsStub{},
	...
	if auth.Subject() != operatorauth.LocalOpenOperatorSubject ||
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if test.auth.Action() != test.action ||
				...
				t.Fatalf("authority = subject:%q workspace:%q action:%q", test.auth.Subject(), test.auth.Workspace(), test.auth.Action())
			}
...
func TestAgentProvisioningCompositionFailsClosedWhenAnyOwnerIsMissing(t *testing.T) {
	now := time.Now
	...
	base := agentProvisioningDependencies{
		...
		operatorResolver: resolver, workspaceKey: "WS", now: now,
	}
	tests := []struct {
		name   string
		...
		{"agents", func(value *agentProvisioningDependencies) { value.agents = nil }},
		{"bindings", func(value *agentProvisioningDependencies) { value.bindings = nil }},
		{"grants", func(value *agentProvisioningDependencies) { value.grants = nil }},
...
func fixedAuthorityIssuer(t *testing.T, now time.Time) *authority.Issuer {
	t.Helper()
