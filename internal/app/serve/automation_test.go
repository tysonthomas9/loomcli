...
func TestComposeAutomationCapabilityExposesOnlyNarrowPortsAndTwoComponents(t *testing.T) {
	...
	catalog := &WorkflowCatalogCapability{
		catalog: workflowcatalog.New(nil, nil, nil), issuer: authority.NewIssuer(),
	...
	_, err = capability.WebhookWorkflow().Ingest(t.Context(), webhookingestion.IngestRequest{
		...
		EventType: eventpolicy.RunFinishedEventType, SubjectRef: "child", ActorRef: "system",
...
func TestAutomationRuntimeAuthorityExpiresWithServerClock(t *testing.T) {
	...
	if !value.ExpiresAt().Equal(now.Add(externalOperatorAuthorityTTL)) {
		t.Fatalf("expiry = %s, want %s", value.ExpiresAt(), now.Add(externalOperatorAuthorityTTL))
	}
}
