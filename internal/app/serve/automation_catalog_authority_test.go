...
func TestAutomationCatalogAuthorityProviderIssuesOnlyEffectiveVersionAuthority(t *testing.T) {
	catalog := &WorkflowCatalogCapability{issuer: authority.NewIssuer()}
	provider := newAutomationCatalogAuthorityProvider(catalog)
...
func TestAutomationCatalogAuthorityProviderFailsClosed(t *testing.T) {
	...
	catalog := &WorkflowCatalogCapability{issuer: authority.NewIssuer()}
	provider = &automationCatalogAuthorityProvider{catalog: catalog}
