...
func TestEnsureDetachedGitWorktreeAtRefRequiresAndPinsSourceControlReceipt(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not available")
	}
	...
	writeFile(t, filepath.Join(repo, "README.md"), "advanced\n")
	git(t, repo, "add", "README.md")
	git(t, repo, "commit", "-m", "advanced")
	advancedCommit := gitOutput(t, repo, "rev-parse", "HEAD")

	const baseRef = "refs/loom/task-runs/run-1/base"
	git(t, repo, "update-ref", baseRef, advancedCommit)
	if err := EnsureDetachedGitWorktreeAtRef(
		t.Context(),
		repo,
		target,
		baseRef,
		...
		t.Fatalf("drifted task base error = %v", err)
	}
	if _, err := os.Stat(filepath.Join(target, ".git")); !os.IsNotExist(err) {
		t.Fatalf("drifted receipt materialized a task worktree: %v", err)
	}

	git(t, repo, "update-ref", baseRef, baseCommit)
	if err := EnsureDetachedGitWorktreeAtRef(
		t.Context(),
		repo,
		target,
		...
		t.Fatalf("materialize pinned task base: %v", err)
	}
	if got := gitOutput(t, target, "rev-parse", "HEAD"); got != baseCommit {
		t.Fatalf("task worktree HEAD = %q, want %q", got, baseCommit)
	}
}
