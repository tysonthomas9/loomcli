...
func RunCodexLeadRuntime(ctx context.Context, cfg CodexLeadRuntimeConfig) error {
	...
	appCmd, appErr, cancelApp, logFile, err := startCodexAppServer(ctx, cfg, runtimeHome, codexHome, sqliteHome, endpoint)
	if err != nil {
		return err
	}
	...
	tuiErr := runCodexRemoteTUI(ctx, cfg, endpoint, codexHome)
...
func startCodexAppServer(
	...
	appCmd.Env = withCodexHome(os.Environ(), codexHome)
	configureCodexAppServerProcess(appCmd)
...
func runCodexRemoteTUI(ctx context.Context, cfg CodexLeadRuntimeConfig, endpoint, codexHome string) error {
	...
	tuiCmd.Env = withCodexHome(os.Environ(), codexHome)
...
// prepareCodexLeadHome carries the authenticated, user-configured portion of
// the Codex profile into a lead-scoped home. Historical sessions and databases
// are intentionally excluded: current Codex app-server versions reconcile that
// state into every fresh sqlite_home before binding their listener, which can
// make lead startup take minutes for a large profile.
func prepareCodexLeadHome(sourceHome, leadHome string) error {
	if err := os.MkdirAll(leadHome, 0700); err != nil {
		return err
	}
	if filepath.Clean(sourceHome) == filepath.Clean(leadHome) {
		return nil
	}
	for _, name := range []string{"config.toml", "auth.json", "AGENTS.md"} {
		sourcePath := filepath.Join(sourceHome, name)
		data, err := os.ReadFile(sourcePath) //nolint:gosec // sourceHome is the user's configured Codex home.
		...
		if err := atomicfile.WriteFile(filepath.Join(leadHome, name), data, 0600); err != nil {
...
func stopCodexAppServer(cmd *exec.Cmd, appErr <-chan error, cancel context.CancelFunc) error {
	...
	defer poll.Stop()
	for {
		if waitCh == nil && !codexAppServerProcessGroupAlive(cmd) {
			return nil
		}
		select {
		case <-waitCh:
		...
		case <-poll.C:
		case <-deadline.C:
			killErr := killCodexAppServerProcess(cmd)
			if waitCh == nil {
				return killErr
			}
			select {
			...
			case <-time.After(2 * time.Second):
				return errors.New("codex app-server did not exit after process-group kill")
			}
		}
	}
}
