...
type CodexLeadRuntimeConfig struct {
	...
	Prompt    string
	CodexPath string
	// ModelPin, when non-empty, is passed to the app server as
	// `-c model="<pin>"` so the session starts on the model the profile was
...
func startCodexAppServer(
	...
	appCmd := exec.CommandContext(appCtx, cfg.CodexPath, codexAppServerArgs(endpoint, sqliteHome, cfg.ModelPin)...)
...
// codexAppServerArgs builds the app-server argv. Extracted so the argument
// shape is assertable without starting a server.
//
// modelPin, when non-empty, becomes `-c model="<pin>"`: the same `-c` config
// overlay sqlite_home already uses, quoted the same way because the value
// portion is parsed as TOML. Verified against codex-cli 0.153.2 — a thread
// started by an app-server launched with the override reports the overridden
// model, so the pin actually decides what the lead's session runs on rather
// than only what its config file says.
func codexAppServerArgs(endpoint, sqliteHome, modelPin string) []string {
	args := []string{"app-server", "--listen", endpoint, "-c", "sqlite_home=" + strconv.Quote(sqliteHome)}
	if pin := strings.TrimSpace(modelPin); pin != "" {
		args = append(args, "-c", "model="+strconv.Quote(pin))
	}
	return args
}
