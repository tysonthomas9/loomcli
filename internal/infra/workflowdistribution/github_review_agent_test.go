...
func TestBuiltinWorkflowRegistryListsAllBuiltins(t *testing.T) {
	...
	wantFiles := map[string]int{
		BuiltinBugFixAgentWorkflowName:       4, // + local/daytona runners + internal Daytona host
		BuiltinEpicRunnerWorkflowName:        5,
	...
	if len(builtinWorkflows) != len(names) {
		t.Fatalf("embedded builtin specs = %d, canonical catalog identities = %d", len(builtinWorkflows), len(names))
	}
	...
	for name := range builtinWorkflows {
		if !IsBuiltinWorkflow(name) {
			t.Fatalf("embedded spec %q is absent from the canonical Workflow Catalog registry", name)
		}
	}
}
...
func TestBuiltinEpicRunnerDeclaresSiblingTaskRunners(t *testing.T) {
	...
	for _, name := range names {
		if name == "openshell-task-runner" || name == "daytona-provider-host" {
			t.Fatalf("internal/deprecated workflow entrypoints must not be derived as runners, got %v", names)
		}
	}
	if _, ok := spec.Files["workflows/daytona-provider-host.ts"]; !ok {
		t.Fatal("epic-runner bundle is missing the internal Daytona provider host")
	}
}
...
func TestGitHubReviewAgentSourceDigestIsDeterministic(t *testing.T) {
	...
	first := mustSourceDigest(t, spec.Files)
	second := mustSourceDigest(t, spec.Files)
	if first != second {
...
func TestWorkflowRunnerSpecsDeriveSiblingWorkflowFiles(t *testing.T) {
	runners := DeriveWorkflowRunnerSpecs("workflows/epic-runner.ts", map[string]string{
...
func TestDaytonaTaskRunnerSourceContract(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(source, forbidden) {
			t.Fatalf("daytona-task-runner source reaches forbidden credential/provider path %q", forbidden)
		}
	}
}
