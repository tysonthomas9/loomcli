...
func registerEpicRunnerAt(t *testing.T, st store.Store, workDir, sourceDigest string) string {
	...
	registered, err := registerFlueDriverForLegacyTest(context.Background(), st, driverpkg.RegisterFlueOptions{
...
func TestEnsureBuiltinWorkflowRefreshesDigestDrift(t *testing.T) {
	...
	if want := mustSourceDigest(t, spec.Files); version.SourceDigest != want {
...
func TestEnsureBuiltinWorkflowDigestDriftFailsOpenWithoutBuildToolchain(t *testing.T) {
	...
	spec, _ := BuiltinWorkflow(BuiltinEpicRunnerWorkflowName)
	for _, want := range []string{driftedDigest, mustSourceDigest(t, spec.Files)} {
...
func TestEnsureBoundPromptAgentWorkflowsRefreshesExistingBindings(t *testing.T) {
	...
	if want := mustSourceDigest(t, spec.Files); boundVersion.SourceDigest != want {
...
func TestEnsureBuiltinWorkflowMatchingDigestIsSilent(t *testing.T) {
	...
	canonical := mustSourceDigest(t, spec.Files)
...
func TestSourceDigestNormalizesPathSeparators(t *testing.T) {
	slash := mustSourceDigest(t, map[string]string{"workflows/a.ts": "x", "workflows/b.ts": "y"})
	backslash := mustSourceDigest(t, map[string]string{filepath.FromSlash("workflows/a.ts"): "x", "workflows/b.ts": "y"})
	if slash != backslash {
