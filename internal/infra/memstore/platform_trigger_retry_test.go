...
func newRetryTestDelivery(deliveryID string, status domain.TriggerDeliveryStatus, attempt int, nextRetryAt *time.Time) *domain.TriggerDelivery {
	now := time.Now().UTC()
	return &domain.TriggerDelivery{
		WorkspaceKey:     "WS",
...
func TestMemTriggerDeliveryDueMembership(t *testing.T) {
	...
	exhausted := newRetryTestDelivery("d-exhausted", domain.TriggerDeliveryFailed, 3, nil)
	exhausted.ErrorClass = domain.TriggerDeliveryErrorRetriesExhausted
	...
	tests := []struct {
		...
		{"failed with next_retry_at", newRetryTestDelivery("d-failed", domain.TriggerDeliveryFailed, 1, &retryAt), true},
		{"failed nil retry is immediately due", newRetryTestDelivery("d-failed-now", domain.TriggerDeliveryFailed, 1, nil), true},
		{"held enters index", newRetryTestDelivery("d-held", domain.TriggerDeliveryHeld, 1, &retryAt), true},
		{"failed retries_exhausted stays out", exhausted, false},
		{"dispatched stays out", newRetryTestDelivery("d-dispatched", domain.TriggerDeliveryDispatched, 1, nil), false},
		{"rejected stays out", newRetryTestDelivery("d-rejected", domain.TriggerDeliveryRejected, 1, nil), false},
		{"superseded stays out", newRetryTestDelivery("d-superseded", domain.TriggerDeliverySuperseded, 1, nil), false},
		{"accepted stays out", newRetryTestDelivery("d-accepted", domain.TriggerDeliveryAccepted, 1, nil), false},
	}
...
func TestMemListDueTriggerDeliveries(t *testing.T) {
	...
	for _, d := range []*domain.TriggerDelivery{
		newRetryTestDelivery("d-late", domain.TriggerDeliveryFailed, 1, at(2*time.Minute)),
		newRetryTestDelivery("d-now", domain.TriggerDeliveryFailed, 1, nil),
		newRetryTestDelivery("d-soon", domain.TriggerDeliveryHeld, 1, at(30*time.Second)),
		newRetryTestDelivery("d-dispatched", domain.TriggerDeliveryDispatched, 1, nil),
	} {
...
func TestMemUpdateTriggerDeliveryResult(t *testing.T) {
	...
	terminal := func(id string) *domain.TriggerDelivery {
		d := newRetryTestDelivery(id, domain.TriggerDeliveryFailed, 3, nil)
	...
	tests := []struct {
		name           string
		initial        *domain.TriggerDelivery
		update         store.TriggerDeliveryResultUpdate
		wantErr        error
		wantStatus     domain.TriggerDeliveryStatus
		wantAttempt    int
	...
	}{
		{
			...
			initial:     newRetryTestDelivery("d-1", domain.TriggerDeliveryFailed, 1, at(30*time.Second)),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed, Attempt: 2, NextRetryAt: at(2 * time.Minute), ErrorClass: "admission_failed"},
			wantStatus:  domain.TriggerDeliveryFailed,
		...
		},
		{
			...
			initial:     newRetryTestDelivery("d-2", domain.TriggerDeliveryAccepted, 1, nil),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed, Attempt: 1, NextRetryAt: at(time.Minute)},
			wantStatus:  domain.TriggerDeliveryFailed,
			wantAttempt: 1,
		...
		},
		{
			...
			initial:     newRetryTestDelivery("d-3", domain.TriggerDeliveryFailed, 2, at(30*time.Second)),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryDispatched, Attempt: 3, DriverRunID: "run-retry-1"},
			wantStatus:  domain.TriggerDeliveryDispatched,
			wantAttempt: 3, wantRunID: "run-retry-1",
		},
		{
			name:        "held promotion to dispatched removes from index",
			initial:     newRetryTestDelivery("d-4", domain.TriggerDeliveryHeld, 1, at(30*time.Second)),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryDispatched, Attempt: 1, DriverRunID: "run-retry-2"},
			wantStatus:  domain.TriggerDeliveryDispatched,
			wantAttempt: 1, wantRunID: "run-retry-2",
		},
		{
			name:        "supersede removes from index",
			initial:     newRetryTestDelivery("d-5", domain.TriggerDeliveryHeld, 1, nil),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliverySuperseded, Attempt: 1},
			wantStatus:  domain.TriggerDeliverySuperseded,
			wantAttempt: 1,
		},
		{
			name:        "attempt at binding budget is terminal retries_exhausted",
			initial:     newRetryTestDelivery("d-6", domain.TriggerDeliveryFailed, 2, at(30*time.Second)),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed, Attempt: 3, NextRetryAt: at(time.Minute), ErrorClass: "admission_failed"},
			wantStatus:  domain.TriggerDeliveryFailed,
			wantAttempt: 3, wantErrorClass: domain.TriggerDeliveryErrorRetriesExhausted,
		},
		{
			name:    "terminal failed rejects retry transition",
			initial: terminal("d-7"),
			update:  store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryDispatched, Attempt: 4},
			wantErr: domain.ErrInvalidTransition,
		},
		{
			name:        "dispatched re-applied idempotently",
			initial:     newRetryTestDelivery("d-8", domain.TriggerDeliveryDispatched, 2, nil),
			update:      store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryDispatched, Attempt: 2, DriverRunID: "run-retry-3"},
			wantStatus:  domain.TriggerDeliveryDispatched,
			wantAttempt: 2, wantRunID: "run-retry-3",
		},
		{
			name:    "dispatched rejects move back to failed",
			initial: newRetryTestDelivery("d-9", domain.TriggerDeliveryDispatched, 1, nil),
			update:  store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed, Attempt: 2, NextRetryAt: at(time.Minute)},
			wantErr: domain.ErrInvalidTransition,
		},
	}
	for _, tt := range tests {
...
func TestMemUpdateTriggerDeliveryResult_Validation(t *testing.T) {
	...
	t.Run("missing delivery wraps not found", func(t *testing.T) {
		s := newRetryTestStore(t)
		if _, err := s.TriggerDeliveries().UpdateResult(t.Context(), "WS", "d-missing", store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed}); !errors.Is(err, domain.ErrNotFound) {
	...
	t.Run("invalid status rejected", func(t *testing.T) {
		s := newRetryTestStore(t)
		if err := s.deliveries.create(newRetryTestDelivery("d-bad", domain.TriggerDeliveryFailed, 1, nil)); err != nil {
			t.Fatalf("create delivery: %v", err)
		}
	...
	t.Run("vanished binding falls back to default budget", func(t *testing.T) {
		s := newRetryTestStore(t)
		d := newRetryTestDelivery("d-orphan", domain.TriggerDeliveryFailed, 3, nil)
		d.TriggerBindingID = "binding-vanished"
		if err := s.deliveries.create(d); err != nil {
			t.Fatalf("create delivery: %v", err)
		}
		// Attempt 4 < default budget 5: still retryable, not exhausted.
		got, err := s.TriggerDeliveries().UpdateResult(t.Context(), "WS", "d-orphan", store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed, Attempt: 4, NextRetryAt: &retryAt, ErrorClass: "admission_failed"})
		if err != nil {
			t.Fatalf("UpdateResult: %v", err)
		}
		if got.ErrorClass != "admission_failed" || got.NextRetryAt == nil {
			t.Fatalf("delivery = %+v, want retryable below the %d-attempt default", got, domain.DefaultTriggerRetryMaxAttempts)
		}
		// Attempt 5 hits the default budget: terminal.
		got, err = s.TriggerDeliveries().UpdateResult(t.Context(), "WS", "d-orphan", store.TriggerDeliveryResultUpdate{Status: domain.TriggerDeliveryFailed, Attempt: 5, NextRetryAt: &retryAt, ErrorClass: "admission_failed"})
		if err != nil {
			t.Fatalf("UpdateResult: %v", err)
		}
		if got.ErrorClass != domain.TriggerDeliveryErrorRetriesExhausted || got.NextRetryAt != nil {
