...
func TestComputePlan_DropRoot(t *testing.T) {
	...
	ordered := []sl.StackNode{node("T2", ""), node("T3", "T2")} // T1 removed; T2 is new root
	prs := map[string]PR{
...
func TestComputePlan_DropTop(t *testing.T) {
	stack := sl.Stack{ID: sid, RootBase: "main"}
	ordered := []sl.StackNode{node("T1", ""), node("T2", "T1")} // T3 removed
	prs := map[string]PR{
		br("T1"): openPR(1, "T1", "main"),
		br("T2"): openPR(2, "T2", br("T1")),
		br("T3"): openPR(3, "T3", br("T2")),
...
func TestComputePlan_DropMultipleMiddle(t *testing.T) {
	stack := sl.Stack{ID: sid, RootBase: "main"}
	ordered := []sl.StackNode{node("T1", ""), node("T4", "T1")} // T2,T3 removed; T4 onto T1
	prs := map[string]PR{
		br("T1"): openPR(1, "T1", "main"),
		br("T2"): openPR(2, "T2", br("T1")),
		br("T3"): openPR(3, "T3", br("T2")),
...
func TestComputePlan_FullPermutation(t *testing.T) {
	...
	ordered := []sl.StackNode{node("T2", ""), node("T4", "T2"), node("T1", "T4"), node("T3", "T1")}
...
func TestComputePlan_Forest(t *testing.T) {
	stack := sl.Stack{ID: sid, RootBase: "main"}
	ordered := []sl.StackNode{node("A1", ""), node("A2", "A1"), node("B1", ""), node("B2", "B1")}
