...
//
//	loom-<wsShort>-<plan|task>-<agent>-<pid>
//
...
// validTmuxName permits the characters emitted by auto-mode for canonical
// Fleet Agent and Role names. It deliberately excludes tmux target separators
// such as ':' and control/path punctuation.
...
type agentConnection struct {
	connection *AgentTmuxConn
	manager    *AgentTmuxManager
}

func (connection *agentConnection) Read(buffer []byte) (int, error) {
	return connection.connection.PTY.Read(buffer)
}

func (connection *agentConnection) Write(buffer []byte) (int, error) {
	return connection.connection.PTY.Write(buffer)
}

func (connection *agentConnection) ConnectionID() string    { return connection.connection.ConnID }
func (connection *agentConnection) SessionName() string     { return connection.connection.SessionName }
func (connection *agentConnection) Killed() <-chan struct{} { return connection.connection.KillCh() }
func (connection *agentConnection) Resize(connectionID string, cols, rows uint16) error {
	return connection.manager.Resize(connectionID, cols, rows)
}

type agentRuntime struct{ manager *AgentTmuxManager }

func NewAgentRuntime(maxSessions int) (*agentRuntime, error) {
	manager, err := NewAgentTmuxManager(maxSessions)
	if err != nil {
		return nil, err
	}
	return &agentRuntime{manager: manager}, nil
}

func (runtime *agentRuntime) HasSession(name string) bool { return runtime.manager.HasSession(name) }
func (runtime *agentRuntime) PaneDead(name string) bool   { return runtime.manager.PaneDead(name) }
func (runtime *agentRuntime) CapturePane(name string, lines int) string {
	return runtime.manager.CapturePane(name, lines)
}
func (runtime *agentRuntime) FindLatestAgentSession(workspaceKey, agentID string) (string, bool, error) {
	return runtime.manager.FindLatestAgentSession(workspaceKey, agentID)
}
func (runtime *agentRuntime) SessionCount() int { return runtime.manager.SessionCount() }
func (runtime *agentRuntime) MaxSessions() int  { return runtime.manager.MaxSessions() }
func (runtime *agentRuntime) Attach(sessionName string, cols, rows uint16) (interaction.AgentTerminalConnection, error) {
	connection, err := runtime.manager.AttachExistingRaw(sessionName, cols, rows)
	if err != nil {
		return nil, publicRuntimeError(err)
	}
	return &agentConnection{connection: connection, manager: runtime.manager}, nil
}
func (runtime *agentRuntime) Detach(connectionID string) error {
	return runtime.manager.Detach(connectionID)
}
func (runtime *agentRuntime) KillWorkspaceSessions(workspaceKey string) error {
	return runtime.manager.KillWorkspaceSessions(workspaceKey)
}
func (runtime *agentRuntime) Shutdown() error { return runtime.manager.Shutdown() }

var _ interaction.AgentTerminalRuntime = (*agentRuntime)(nil)
...
func (m *AgentTmuxManager) FindLatestAgentSession(wsID, agentName string) (string, bool, error) {
	...
	for _, s := range sessions {
		if !matchesAgentSession(s.name, wsPrefix, agentName) {
			continue
		}
...
func matchesAgentSession(sessionName, workspacePrefix, agentName string) bool {
	if workspacePrefix == "" || !validTmuxName.MatchString(agentName) {
		return false
	}
	pattern := regexp.MustCompile(fmt.Sprintf(
		`^loom-%s-(?:plan|task)-%s-[0-9]+$`,
		regexp.QuoteMeta(workspacePrefix),
		regexp.QuoteMeta(agentName),
	))
	return pattern.MatchString(sessionName)
}
