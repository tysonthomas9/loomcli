...
func replayOutput(events []interaction.TerminalReplayEvent) []byte {
	var output []byte
...
func TestAttach_SpawnsFreshSession(t *testing.T) {
	...
	replay := att.Replay()
	if len(replay) != 1 || !replay[0].IsResize() || replay[0].Columns != 80 || replay[0].Rows != 24 {
		t.Errorf("Replay() on fresh session = %#v", replay)
	}
...
func TestReattachWithinGraceReplaysScrollback(t *testing.T) {
	...
	replay := att2.Replay()
	if len(replay) == 0 || !replay[0].IsResize() {
		t.Errorf("replay missing initial resize; got %#v", replay)
	}
	if output := replayOutput(replay); !bytes.Contains(output, []byte("hello-world")) {
		t.Errorf("replay missing prior output; got %q", string(output))
	}
...
func TestSecondAttachCoexistsAndReceivesScrollback(t *testing.T) {
	...
	if output := replayOutput(att2.Replay()); !bytes.Contains(output, []byte("marker-abc")) {
		t.Errorf("second attach replay missing marker; got %q", string(output))
	}
