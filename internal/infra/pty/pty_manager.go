// Package pty owns process-local PTY and tmux mechanics behind Interaction's
// TerminalRuntime port.
//
// PTYManager owns per-session PTY-backed shells for the
// web terminal.
//
...
// session's ordered resize/output timeline, followed by live output.
//
...
func pinCurrentLoomOnPath(env []string) []string {
	...
	return platformruntime.PinExecutableDirOnPath(env, executable)
}
...
// Callers should apply Attachment.Replay() before consuming live output.
...
// captures output and geometry so a later WebSocket attach can replay it.
...
func (m *PTYManager) spawnSession(key SessionKey, cols, rows uint16, launch *LaunchSpec) (*ptySession, error) {
	...
	sess := newPtySession(key, ptmx, cmd, cols, rows)
	go sess.drain(m)
	return sess, nil
}
