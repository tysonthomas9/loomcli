// Package agentscompatstore is the infrastructure-only Phase 5 persistence adapter for
// legacy startup and local compatibility flows. Only central application
// composition imports this package; consumers receive typed commands from the
// public Agents root and never receive Adapter or the process-wide Store.
...
func New(
	...
	if roles == nil || agentServices == nil || assignments == nil {
		return nil, fmt.Errorf("compose Agents compatibility store adapter: %w", agents.ErrUnavailable)
	}
	return &Adapter{
		roles:       roles,
