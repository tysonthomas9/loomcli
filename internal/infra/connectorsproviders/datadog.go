...
const (
	...
	ActionDatadogMonitorsRead = domain.ActionDatadogMonitorsRead
	// ActionDatadogAlertRead reads one monitor's alert state (read-only).
	ActionDatadogAlertRead = domain.ActionDatadogAlertRead
	...
	// deleted afterwards.
	ActionDatadogIncidentsWrite = domain.ActionDatadogIncidentsWrite
)

// DatadogActions returns the actions the Datadog provider implements (a
// copy).
func DatadogActions() []string {
	return domain.ActionsForSource(domain.ConnectorSourceDatadog)
}
