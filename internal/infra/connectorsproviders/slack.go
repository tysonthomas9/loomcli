...
const (
	...
	ActionSlackChatPost = domain.ActionSlackChatPost
	// ActionSlackConversationsRead reads recent conversation history
	// (read-only).
	ActionSlackConversationsRead = domain.ActionSlackConversationsRead
)

// SlackActions returns the actions the Slack provider implements (a copy).
func SlackActions() []string {
	return domain.ActionsForSource(domain.ConnectorSourceSlack)
}
