...
// the held delivery.
func heldDeliveryFixture(t *testing.T, s *memstore.Store) (blockingRunID string, held *automation.Delivery) {
	t.Helper()
	...
	if leg.Status != automation.DeliveryHeld || leg.RunID != "" {
		t.Fatalf("second dispatch leg = %+v, want held with no run", leg)
	}
	held, err := s.TriggerDeliveries().Get(t.Context(), "WS", leg.DeliveryID)
	if err != nil || held.Status != automation.DeliveryHeld || held.NextRetryAt == nil || held.Attempt != 1 {
...
func TestDeliverySweeperPromotesHeldDelivery(t *testing.T) {
	...
	setupSweeperBinding(t, st, store.TriggerBindingCreate{
		BindingID: "binding-queue", ConcurrencyPolicy: automation.ConcurrencyQueue,
	...
	if err != nil || promoted.Status != automation.DeliveryDispatched {
...
func TestDeliverySweeperHeldStillBusyBackoffDoubling(t *testing.T) {
	...
	for _, want := range []struct {
		...
		if err != nil || held.Status != automation.DeliveryHeld {
...
func TestDeliverySweeperRetriesExhaustedTerminal(t *testing.T) {
	...
	if err != nil || terminal.Status != automation.DeliveryFailed {
		t.Fatalf("terminal delivery = %+v (err %v), want failed", terminal, err)
	}
	if terminal.ErrorClass != automation.TriggerDeliveryErrorRetriesExhausted || terminal.NextRetryAt != nil || terminal.Attempt != 3 {
...
func TestDeliverySweeperRedispatchesFailedDelivery(t *testing.T) {
	...
	failed, err := st.TriggerDeliveries().UpdateResult(ctx, "WS", held.DeliveryID, store.TriggerDeliveryResultUpdate{
		Status: automation.DeliveryFailed, Attempt: held.Attempt,
		NextRetryAt: &retryAt, ErrorClass: "delivery_target_error",
	})
	if err != nil || failed.Status != automation.DeliveryFailed {
	...
	if err != nil || healed.Status != automation.DeliveryDispatched || healed.DriverRunID == "" {
...
func TestDeliverySweeperDispatchErrorBacksOff(t *testing.T) {
	...
	held, err := st.TriggerDeliveries().Get(ctx, "WS", held.DeliveryID)
	if err != nil || held.Status != automation.DeliveryHeld || held.Attempt != 2 {
	...
	promoted, err := st.TriggerDeliveries().Get(ctx, "WS", held.DeliveryID)
	if err != nil || promoted.Status != automation.DeliveryDispatched || promoted.Attempt != 3 {
