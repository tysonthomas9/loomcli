...
func (s *DeliverySweeper) recordRetry(ctx context.Context, ws string, delivery *automation.Delivery, binding *automation.Binding, now time.Time, errorClass string, out *DeliverySweepResult) error {
	attempt := delivery.Attempt + 1
	...
	if exhausted {
		update.Status = automation.DeliveryFailed
	} else {
...
func (s *DeliverySweeper) resolveRouteKey(ctx context.Context, ws string, event *automation.Event) string {
...
func (s *DeliverySweeper) deliveryBinding(ctx context.Context, ws, bindingID string) *automation.Binding {
	if bindingID == "" {
		return nil
	}
...
func redispatchInput(event *automation.Event) store.TriggerRouteDispatch {
...
func findDeliveryLeg(result *store.TriggerRouteDispatchResult, delivery *automation.Delivery) (store.TriggerRouteDelivery, bool) {
	if result == nil {
...
func deliveryStillDue(d *automation.Delivery, now time.Time) bool {
	switch d.Status {
	case automation.DeliveryHeld:
	case automation.DeliveryFailed:
		if d.ErrorClass == automation.TriggerDeliveryErrorRetriesExhausted {
			return false
		}
	default:
		return false
...
func deliveryRetryMaxAttempts(binding *automation.Binding) int {
	if binding == nil || binding.RetryMaxAttempts <= 0 {
		return automation.DefaultTriggerRetryMaxAttempts
	}
...
func deliveryRetryBackoff(binding *automation.Binding, attempt int) time.Duration {
