...
func TestGenericDriverRequestsOmitLifecycleActivation(t *testing.T) {
	...
	if _, err := client.Drivers().Create(t.Context(), workflowcatalog.DriverCreate{
		WorkspaceKey: "WS", DriverID: "driver-1", Name: "driver",
		Status: workflowcatalog.DriverStatusDraft, TrustLevel: workflowcatalog.DriverTrustTrusted,
	}); err != nil {
	...
	metadata := map[string]string{"team": "runtime"}
	if _, err := client.Drivers().Update(t.Context(), "WS", "driver-1", workflowcatalog.DriverUpdate{
...
func TestPlatformClientDriverRunLifecycleRoutesAndErrors(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/epics/WS-1/runs":
			...
			writeJSON(t, w, execution.DriverRunRecord{WorkspaceKey: "WS", RunID: req.RunID, DriverID: "driver-1", DriverVersionID: "version-1", EpicID: "WS-1", Status: execution.DriverRunQueued})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs":
			...
			writeJSON(t, w, execution.DriverRunRecord{WorkspaceKey: "WS", RunID: req.RunID, DriverID: req.DriverID, DriverVersionID: req.DriverVersionID, EpicID: req.EpicID, Status: execution.DriverRunQueued})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/WS/driver-runs":
			...
			writeJSON(t, w, map[string]any{"driver_runs": []execution.DriverRunRecord{{WorkspaceKey: "WS", RunID: "run-1", AgentServiceID: "agent-1", Status: execution.DriverRunQueued}}})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs/run-1/claim":
			...
			writeJSON(t, w, execution.DriverRunRecord{WorkspaceKey: "WS", RunID: "run-1", Status: execution.DriverRunRunning, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: 1})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs/run-1/finish":
			...
			decodeJSONBody(t, r, &req)
			if req.NodeID != "node-1" || req.LeaseID != "lease-1" || req.FencingToken != 1 || req.Status != execution.DriverRunCompleted || req.Output["logs_ref"] != "driver-run://run-1/flue-local" {
				t.Fatalf("finish body = %+v", req)
			}
			writeJSON(t, w, execution.DriverRunRecord{WorkspaceKey: "WS", RunID: "run-1", Status: execution.DriverRunCompleted, NodeID: req.NodeID})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs/recover-stale":
			...
			writeJSON(t, w, execution.StaleDriverRunRecoveryResult{WorkspaceKey: "WS", StaleBefore: recoveryBefore, Recovered: 1, SkippedFresh: 1, RecoveredRunIDs: []string{"run-stale"}})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs/run-1/recover-stale-tasks":
			...
			writeJSON(t, w, execution.StaleTaskRunRecoveryResult{WorkspaceKey: "WS", DriverRunID: "run-1", StaleBefore: recoveryBefore, Recovered: 2, Released: 1, SkippedFresh: 1, RecoveredTaskRunIDs: []string{"task-run-1", "task-run-2"}})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs/run-1/steps":
			...
			decodeJSONBody(t, r, &req)
			if req.StepID != "step-1" || req.DriverRunID != "" || req.StepKind != "custom_vendor_gate" || req.Status != execution.DriverStepWaiting || req.InputRef != "artifact://input-1" || req.NodeID != "node-1" || req.LeaseID != "lease-1" || req.FencingToken != 1 {
				t.Fatalf("driver run step create body = %+v", req)
			}
			writeJSON(t, w, execution.DriverStepRecord{WorkspaceKey: "WS", StepID: req.StepID, DriverRunID: "run-1", StepKind: req.StepKind, Status: req.Status, InputRef: req.InputRef})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/WS/driver-runs/run-1/steps":
			...
			writeJSON(t, w, map[string]any{"driver_steps": []execution.DriverStepRecord{{WorkspaceKey: "WS", StepID: "step-1", DriverRunID: "run-1", StepKind: "custom_vendor_gate", Status: execution.DriverStepWaiting}}})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-steps":
			...
			writeJSON(t, w, execution.DriverStepRecord{WorkspaceKey: "WS", StepID: req.StepID, DriverRunID: req.DriverRunID, StepKind: req.StepKind, Status: execution.DriverStepQueued})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/WS/driver-steps":
			...
			writeJSON(t, w, map[string]any{"driver_steps": []execution.DriverStepRecord{{WorkspaceKey: "WS", StepID: "step-1", DriverRunID: "run-1", TaskRunID: "task-run-1", Status: execution.DriverStepCompleted}}})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/WS/driver-steps/step-1":
			writeJSON(t, w, execution.DriverStepRecord{WorkspaceKey: "WS", StepID: "step-1", DriverRunID: "run-1", StepKind: "custom_vendor_gate", Status: execution.DriverStepWaiting, InputRef: "artifact://input-1"})
		case r.Method == http.MethodPatch && r.URL.Path == "/api/v1/WS/driver-steps/step-1":
			var req execution.DriverStepUpdate
			decodeJSONBody(t, r, &req)
			if req.Status == nil || *req.Status != execution.DriverStepCompleted || req.TaskRunID == nil || *req.TaskRunID != "task-run-1" || req.OutputRef == nil || *req.OutputRef != "artifact://output-1" || req.NodeID != "node-1" || req.LeaseID != "lease-1" || req.FencingToken != 1 {
				t.Fatalf("driver step update body = %+v", req)
			}
			writeJSON(t, w, execution.DriverStepRecord{WorkspaceKey: "WS", StepID: "step-1", DriverRunID: "run-1", StepKind: "custom_vendor_gate", Status: execution.DriverStepCompleted, TaskRunID: *req.TaskRunID, OutputRef: *req.OutputRef})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/task-runs":
			var req struct {
				...
				LeaseID          string                           `json:"lease_id"`
				FencingToken     int64                            `json:"fencing_token"`
				SandboxPlacement execution.TaskRunPlacementRecord `json:"sandbox_placement"`
			}
			decodeJSONBody(t, r, &req)
			...
			writeJSON(t, w, execution.TaskRunRecord{WorkspaceKey: "WS", TaskRunID: req.TaskRunID, DriverRunID: req.DriverRunID, DriverStepID: req.DriverStepID, TaskID: req.TaskID, WorkerProfileID: req.WorkerProfileID, Status: execution.TaskRunRecordRunning, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: 42, SandboxPlacement: req.SandboxPlacement})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/task-runs/claim":
			...
			writeJSON(t, w, execution.TaskRunRecord{WorkspaceKey: "WS", TaskRunID: req.TaskRunID, WorkerProfileID: "falcon", Status: execution.TaskRunRecordRunning, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: 42, RunnerPlacement: execution.TaskRunPlacementRecord{Provider: "daemon", NodeID: req.NodeID, RunnerID: req.RunnerID}, SandboxPlacement: req.SandboxPlacement})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/WS/task-runs":
			...
			writeJSON(t, w, map[string]any{"task_runs": []execution.TaskRunRecord{{WorkspaceKey: "WS", TaskRunID: "task-run-1", DriverRunID: "run-1", DriverStepID: "step-1", Status: execution.TaskRunRecordRunning}}})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/task-runs/task-run-1/finish":
			...
			decodeJSONBody(t, r, &req)
			if req.NodeID != "node-1" || req.LeaseID != "task-lease-1" || req.FencingToken != 42 || req.Status != execution.TaskRunRecordCompleted || req.ExitCode == nil || *req.ExitCode != 0 || req.LogsRef != "logs://task-run-1" {
			...
			writeJSON(t, w, execution.TaskRunRecord{WorkspaceKey: "WS", TaskRunID: "task-run-1", Status: execution.TaskRunRecordCompleted, ExitCode: req.ExitCode, InputTokens: req.InputTokens, OutputTokens: req.OutputTokens, CacheReadTokens: req.CacheReadTokens, CacheWriteTokens: req.CacheWriteTokens, EstimatedCostUSD: req.EstimatedCostUSD})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/task-runs/task-run-1/heartbeat":
			...
			writeJSON(t, w, execution.TaskRunRecord{WorkspaceKey: "WS", TaskRunID: "task-run-1", Status: execution.TaskRunRecordRunning, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: req.FencingToken, LogsRef: req.LogsRef})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/task-runs/task-run-1/logs":
			...
			writeJSON(t, w, execution.TaskRunLogEntry{WorkspaceKey: "WS", TaskRunID: "task-run-1", Sequence: 1, Stream: req.Stream, Text: req.Text, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: req.FencingToken})
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/WS/task-runs/task-run-1/logs":
			...
			writeJSON(t, w, map[string]any{"logs": []execution.TaskRunLogEntry{{WorkspaceKey: "WS", TaskRunID: "task-run-1", Sequence: 2, Stream: "stderr", Text: "warning\n"}}, "count": 1, "next_sequence": 2})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/task-runs/task-run-1/complete":
			...
			decodeJSONBody(t, r, &req)
			if req.CompletionID != "completion-1" || req.NodeID != "node-1" || req.LeaseID != "task-lease-1" || req.FencingToken != 42 || req.Status != execution.TaskRunRecordCompleted || !req.RequireArtifacts || !req.CloseTask || len(req.RequiredArtifactIDs) != 1 || req.RequiredArtifactIDs[0] != "artifact-1" || !req.FinishedAt.Equal(logTimestamp) {
			...
			writeJSON(t, w, map[string]any{
				"task_run": execution.TaskRunRecord{WorkspaceKey: "WS", TaskRunID: "task-run-1", TaskID: "WS-1", Status: execution.TaskRunRecordCompleted, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: req.FencingToken, InputTokens: req.InputTokens, OutputTokens: req.OutputTokens, CacheReadTokens: req.CacheReadTokens, CacheWriteTokens: req.CacheWriteTokens, EstimatedCostUSD: req.EstimatedCostUSD},
			})
		default:
	...
	if _, err := client.Drivers().Create(t.Context(), workflowcatalog.DriverCreate{WorkspaceKey: "WS", DriverID: "driver-1", Name: "epic-runner", Status: workflowcatalog.DriverStatusDraft}); err != nil {
		t.Fatalf("Create driver: %v", err)
	}
	if _, err := client.DriverVersions().Create(t.Context(), workflowcatalog.DriverVersionCreate{WorkspaceKey: "WS", DriverID: "driver-1", VersionID: "version-1", Version: 1, SourceDigest: "sha256:source", BundleDigest: "sha256:bundle"}); err != nil {
		t.Fatalf("Create driver version: %v", err)
	}
	if versions, err := client.DriverVersions().List(t.Context(), "WS", workflowcatalog.DriverVersionFilter{ValidationStatus: workflowcatalog.DriverVersionValidationPassed, Limit: 1}); err != nil || len(versions) != 1 {
		t.Fatalf("List driver versions = %+v err=%v, want one", versions, err)
	}
	if binding, err := client.TriggerBindings().Create(t.Context(), automation.TriggerBindingCreate{
		WorkspaceKey:      "WS",
		BindingID:         "binding-1",
		Name:              "Epic runner",
		SourceKind:        "http",
	...
	if bindings, err := client.TriggerBindings().List(t.Context(), "WS", automation.TriggerBindingFilter{TargetAgentServiceID: "lead", Limit: 1}); err != nil || len(bindings) != 1 || bindings[0].TargetAgentServiceID != "lead" {
	...
	enabled := false
	if binding, err := client.TriggerBindings().Update(t.Context(), "WS", "binding-1", automation.TriggerBindingUpdate{Name: &name, Enabled: &enabled}); err != nil || binding.Name != name || binding.Enabled {
		t.Fatalf("Update trigger binding = %+v err=%v, want disabled renamed binding", binding, err)
	}
	if run, err := client.DriverRuns().CreateEpic(t.Context(), "WS", "WS-1", execution.EpicRunCreate{
		RunID:          "run-epic-1",
	...
	if _, err := client.DriverRuns().Create(t.Context(), execution.DriverRunCreate{WorkspaceKey: "WS", RunID: "run-1", DriverID: "driver-1", DriverVersionID: "version-1", EpicID: "WS-1", Payload: json.RawMessage(`{"epicId":"WS-1"}`)}); err != nil {
		t.Fatalf("Create driver run: %v", err)
	}
	if runs, err := client.DriverRuns().List(t.Context(), "WS", execution.DriverRunFilter{EpicID: "WS-1", AgentServiceID: "agent-1", Status: execution.DriverRunQueued}); err != nil || len(runs) != 1 || runs[0].AgentServiceID != "agent-1" {
	...
	if _, err := client.DriverRuns().Claim(t.Context(), "WS", "run-1", "node-1", "lease-1"); !errors.Is(err, persistence.ErrAlreadyClaimed) {
		t.Fatalf("second Claim err = %v, want ErrAlreadyClaimed", err)
	}
	if step, err := client.DriverSteps().CreateForRun(t.Context(), "WS", "run-1", execution.DriverStepCreate{StepID: "step-1", StepKind: "custom_vendor_gate", Status: execution.DriverStepWaiting, InputRef: "artifact://input-1", NodeID: "node-1", LeaseID: "lease-1", FencingToken: 1}); err != nil || step.StepID != "step-1" || step.DriverRunID != "run-1" {
		t.Fatalf("CreateForRun driver step = %+v err=%v, want step-1 under run-1", step, err)
	}
	if steps, err := client.DriverSteps().ListForRun(t.Context(), "WS", "run-1", execution.DriverStepFilter{StepKind: "custom_vendor_gate", Status: execution.DriverStepWaiting}); err != nil || len(steps) != 1 {
		t.Fatalf("ListForRun driver steps = %+v err=%v, want one", steps, err)
	}
	if step, err := client.DriverSteps().Create(t.Context(), execution.DriverStepCreate{WorkspaceKey: "WS", StepID: "step-top", DriverRunID: "run-1", StepKind: "run_agent", NodeID: "node-1", LeaseID: "lease-1", FencingToken: 1}); err != nil || step.StepID != "step-top" {
	...
	completedStep := execution.DriverStepCompleted
	stepTaskRunID := "task-run-1"
	stepOutputRef := "artifact://output-1"
	if step, err := client.DriverSteps().Update(t.Context(), "WS", "step-1", execution.DriverStepUpdate{Status: &completedStep, TaskRunID: &stepTaskRunID, OutputRef: &stepOutputRef, NodeID: "node-1", LeaseID: "lease-1", FencingToken: 1}); err != nil || step.Status != execution.DriverStepCompleted || step.TaskRunID != "task-run-1" {
		t.Fatalf("Update driver step = %+v err=%v, want completed task step", step, err)
	}
	if steps, err := client.DriverSteps().List(t.Context(), "WS", execution.DriverStepFilter{DriverRunID: "run-1", TaskRunID: "task-run-1", Status: execution.DriverStepCompleted, Limit: 1}); err != nil || len(steps) != 1 {
		t.Fatalf("List driver steps = %+v err=%v, want one", steps, err)
	}
	if _, err := client.DriverRuns().Finish(t.Context(), "WS", "run-1", execution.DriverRunFinish{NodeID: "node-1", LeaseID: "lease-1", FencingToken: 1, Status: execution.DriverRunCompleted, Output: map[string]string{"logs_ref": "driver-run://run-1/flue-local"}}); err != nil {
		t.Fatalf("Finish driver run: %v", err)
	}
	if recovered, err := client.DriverRuns().RecoverStale(t.Context(), "WS", execution.StaleDriverRunRecovery{StaleBefore: recoveryBefore, MaxAgeSeconds: 60, ErrorClass: "stale_driver_run", Summary: "operator stale run recovery", Limit: 2}); err != nil || recovered.Recovered != 1 || recovered.SkippedFresh != 1 {
		t.Fatalf("RecoverStale = %+v err=%v, want recovery counts", recovered, err)
	}
	if recovered, err := client.DriverRuns().RecoverStaleTaskRuns(t.Context(), "WS", "run-1", execution.StaleTaskRunRecovery{StaleBefore: recoveryBefore, MaxAgeSeconds: 60, ErrorClass: "stale_task_run", ErrorMessage: "operator recovery"}); err != nil || recovered.Recovered != 2 || recovered.Released != 1 || recovered.SkippedFresh != 1 {
		t.Fatalf("RecoverStaleTaskRuns = %+v err=%v, want recovery counts", recovered, err)
	}

	taskRun, err := client.TaskRuns().Create(t.Context(), execution.TaskRunCreate{WorkspaceKey: "WS", TaskRunID: "task-run-1", DriverRunID: "run-1", DriverStepID: "step-1", TaskID: "WS-1", WorkerProfileID: "falcon", Status: execution.TaskRunRecordRunning, NodeID: "node-1", LeaseID: "task-lease-1", SandboxPlacement: execution.TaskRunPlacementRecord{Provider: "daytona", CWD: "/workspace"}})
	if err != nil {
		t.Fatalf("Create task run: %v", err)
	}
	taskRun, err = client.TaskRuns().ClaimQueued(t.Context(), "WS", execution.TaskRunClaim{TaskRunID: "task-run-1", NodeID: "node-1", RunnerID: "runner-1", LeaseID: "task-lease-1", LeaseToken: "claim-token", SupportedProviders: []string{"daytona"}, Capabilities: []string{"git"}, WorkerProfileIDs: []string{"falcon"}, SandboxPlacement: execution.TaskRunPlacementRecord{Provider: "daytona", SandboxID: "sandbox-1", CWD: "/workspace"}})
	if err != nil {
	...
	if taskRuns, err := client.TaskRuns().List(t.Context(), "WS", execution.TaskRunFilter{DriverRunID: "run-1", DriverStepID: "step-1", WorkerProfileID: "falcon", Status: execution.TaskRunRecordRunning}); err != nil || len(taskRuns) != 1 {
		t.Fatalf("List task runs = %+v err=%v, want one", taskRuns, err)
	}
	exitCode := 0
	if heartbeat, err := client.TaskRuns().Heartbeat(t.Context(), "WS", "task-run-1", execution.TaskRunHeartbeat{NodeID: taskRun.NodeID, LeaseID: taskRun.LeaseID, LeaseToken: "task-run-token", FencingToken: taskRun.FencingToken, LogsRef: "logs://task-run-1", RuntimeMetadata: map[string]string{"phase": "running"}}); err != nil || heartbeat.Status != execution.TaskRunRecordRunning {
		t.Fatalf("Heartbeat task run = %+v err=%v, want running", heartbeat, err)
	}
	if entry, err := client.TaskRuns().AppendLog(t.Context(), "WS", "task-run-1", execution.TaskRunLogAppend{RequestID: "task-run-log-1", NodeID: taskRun.NodeID, LeaseID: taskRun.LeaseID, LeaseToken: "task-run-token", FencingToken: taskRun.FencingToken, Stream: "stdout", Text: "starting\n", Timestamp: logTimestamp}); err != nil || entry.Sequence != 1 {
		t.Fatalf("AppendLog = %+v err=%v, want sequence 1", entry, err)
	}
	if logs, err := client.TaskRuns().ListLogs(t.Context(), "WS", "task-run-1", execution.TaskRunLogFilter{AfterSequence: 1, Limit: 10}); err != nil || len(logs) != 1 || logs[0].Sequence != 2 {
		t.Fatalf("ListLogs = %+v err=%v, want one sequence 2", logs, err)
	}
	if finished, err := client.TaskRuns().Finish(t.Context(), "WS", "task-run-1", execution.TaskRunFinish{NodeID: taskRun.NodeID, LeaseID: taskRun.LeaseID, LeaseToken: "task-run-token", FencingToken: taskRun.FencingToken, Status: execution.TaskRunRecordCompleted, ExitCode: &exitCode, LogsRef: "logs://task-run-1", InputTokens: 11, OutputTokens: 7, CacheReadTokens: 5, CacheWriteTokens: 3, EstimatedCostUSD: 0.125}); err != nil {
	...
	if completed, err := client.TaskRuns().Complete(t.Context(), "WS", "task-run-1", execution.TaskRunComplete{CompletionID: "completion-1", NodeID: taskRun.NodeID, LeaseID: taskRun.LeaseID, LeaseToken: "task-run-token", FencingToken: taskRun.FencingToken, Status: execution.TaskRunRecordCompleted, RequiredArtifactIDs: []string{"artifact-1"}, RequireArtifacts: true, InputTokens: 23, OutputTokens: 19, CacheReadTokens: 13, CacheWriteTokens: 2, EstimatedCostUSD: 0.25, CloseTask: true, CloseReason: "done", FinishedAt: logTimestamp}); err != nil || completed.Status != execution.TaskRunRecordCompleted {
...
func TestPlatformClientFinishesDriverRunWithNeedsReviewStatus(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		decodeJSONBody(t, r, &req)
		if req.NodeID != "node-1" || req.LeaseID != "lease-1" || req.FencingToken != 1 || req.Status != execution.DriverRunNeedsReview {
			t.Fatalf("finish needs_review body = %+v", req)
		}
		writeJSON(t, w, execution.DriverRunRecord{WorkspaceKey: "WS", RunID: "run-1", Status: execution.DriverRunNeedsReview, NodeID: req.NodeID, LeaseID: req.LeaseID, FencingToken: req.FencingToken})
	}))
	defer ts.Close()
	...
	finished, err := client.DriverRuns().Finish(t.Context(), "WS", "run-1", execution.DriverRunFinish{
		NodeID:       "node-1",
		LeaseID:      "lease-1",
		FencingToken: 1,
		Status:       execution.DriverRunNeedsReview,
		Summary:      "blocked",
	...
	if finished.Status != execution.DriverRunNeedsReview {
