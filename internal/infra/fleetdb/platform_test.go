...
func TestPlatformClientDriverRunLifecycleRoutesAndErrors(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/WS/driver-runs/run-1/finish":
			...
			decodeJSONBody(t, r, &req)
			if req.NodeID != "node-1" || req.LeaseID != "lease-1" || req.FencingToken != 1 || req.Status != domain.DriverRunCompleted {
	...
	if _, err := client.DriverRuns().Finish(t.Context(), "WS", "run-1", store.DriverRunFinish{NodeID: "node-1", LeaseID: "lease-1", FencingToken: 1, Status: domain.DriverRunCompleted}); err != nil {
