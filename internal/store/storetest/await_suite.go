...
// hour-long deadline.
func awaitReg(runID string, n int, pattern string) store.AwaitRegistration {
	return store.AwaitRegistration{
		InstanceKey: domain.AwaitInstanceKey(runID, n),
		RunID:       runID,
		Pattern:     pattern,
...
func mustRegister(t *testing.T, ctx context.Context, h *AwaitHarness, in store.AwaitRegistration) *store.AwaitResult {
	t.Helper()
...
func mustListPattern(t *testing.T, ctx context.Context, h *AwaitHarness, pattern string) []*domain.AwaitInstance {
	t.Helper()
...
func assertNoSatisfiedRow(t *testing.T, ctx context.Context, h *AwaitHarness, instanceKey string) {
	t.Helper()
	if _, err := h.Awaits.GetSatisfiedAwait(ctx, h.Workspace, instanceKey); !errors.Is(err, domain.ErrNotFound) {
...
func testPendingRegistrationIdempotent(t *testing.T, h *AwaitHarness) {
	...
	if again.Satisfied || again.Instance.Status != domain.AwaitPending {
...
func testRegistrationValidation(t *testing.T, h *AwaitHarness) {
	...
	cases := []struct {
		name    string
		mutate  func(*store.AwaitRegistration)
		wantErr error
	}{
		{"missing deadline", func(r *store.AwaitRegistration) { r.Deadline = time.Time{} }, domain.ErrAwaitTimeoutRequired},
		{"past deadline", func(r *store.AwaitRegistration) { r.Deadline = time.Now().Add(-time.Minute) }, domain.ErrAwaitTimeoutRequired},
		{"bare event type pattern", func(r *store.AwaitRegistration) { r.Pattern = "pr.approved" }, domain.ErrAwaitPatternUnscoped},
		{"empty subject pattern", func(r *store.AwaitRegistration) { r.Pattern = "pr.approved:" }, domain.ErrAwaitPatternUnscoped},
		{"malformed instance key", func(r *store.AwaitRegistration) { r.InstanceKey = "run-a#await-0" }, domain.ErrAwaitInstanceKeyMalformed},
		{"key belongs to another run", func(r *store.AwaitRegistration) { r.RunID = "run-b" }, domain.ErrAwaitInstanceKeyMalformed},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !errors.Is(err, domain.ErrInvalid) {
				t.Fatalf("err = %v, want it to wrap domain.ErrInvalid", err)
			}
		})
	}
}
...
func testResolveFirstWinnerPersistsPayload(t *testing.T, h *AwaitHarness) {
	...
	if won.Instance.Status != domain.AwaitSatisfied || won.Instance.ResumedAt == nil {
...
func testResolveTimeoutLandsTimedOut(t *testing.T, h *AwaitHarness) {
	...
	res := mustRegister(t, ctx, h, awaitReg("run-a", 1, "pr.approved:repo-1"))
	key := res.Instance.InstanceKey
	timeoutEvent := domain.AwaitTimeoutEventIDPrefix + "deadline-1"
	...
	if out.Instance.Status != domain.AwaitTimedOut {
		t.Fatalf("timeout resolution status = %q, want timed_out", out.Instance.Status)
	}
	row, err := h.Awaits.GetSatisfiedAwait(ctx, h.Workspace, key)
	if err != nil || row.Status != domain.AwaitTimedOut || row.SatisfiedByEventID != timeoutEvent {
...
func testResolveMissingNotFound(t *testing.T, h *AwaitHarness) {
	ctx := t.Context()
	if _, err := h.Awaits.ResolveAwait(ctx, h.Workspace, "run-x#await-1", "event-1", nil, "alice"); !errors.Is(err, domain.ErrNotFound) {
...
func testResolvePayloadCapRejected(t *testing.T, h *AwaitHarness) {
	...
	res := mustRegister(t, ctx, h, awaitReg("run-a", 1, "pr.approved:repo-1"))
	key := res.Instance.InstanceKey
	oversize := json.RawMessage(`"` + string(bytes.Repeat([]byte("x"), domain.DefaultAwaitResumePayloadCap)) + `"`)
...
func testDueDeadlinesOrderedAndCapped(t *testing.T, h *AwaitHarness) {
	...
	wantOrder := []string{domain.AwaitInstanceKey("run-a", 2), domain.AwaitInstanceKey("run-a", 3)}
	if len(due) != 2 {
...
func claimRun(t *testing.T, ctx context.Context, h *AwaitHarness, runID string) *domain.DriverRun {
	t.Helper()
...
func testSuspendRequiresRunningOwner(t *testing.T, h *AwaitHarness) {
	...
	if _, err := runs.Suspend(ctx, "run-a", "", "", 0); !errors.Is(err, domain.ErrInvalidTransition) {
		t.Fatalf("suspend of queued run: err = %v, want ErrInvalidTransition", err)
	}
	claimed := claimRun(t, ctx, h, "run-a")
	if _, err := runs.Suspend(ctx, "run-a", claimed.NodeID, "lease-stale", claimed.FencingToken); !errors.Is(err, domain.ErrNotOwner) {
		t.Fatalf("suspend with wrong lease: err = %v, want ErrNotOwner", err)
	}
	if _, err := runs.Suspend(ctx, "run-a", claimed.NodeID, claimed.LeaseID, claimed.FencingToken+1); !errors.Is(err, domain.ErrNotOwner) {
		t.Fatalf("suspend with wrong fencing token: err = %v, want ErrNotOwner", err)
	}
	if _, err := runs.Suspend(ctx, "run-missing", "node-1", "lease-1", 1); !errors.Is(err, domain.ErrNotFound) {
...
func testSuspendReleasesSlotIdempotently(t *testing.T, h *AwaitHarness) {
	...
	if suspended.Status != domain.DriverRunSuspendedAwaitingEvent || suspended.SuspendedAt == nil {
...
func testClaimRejectsSuspended(t *testing.T, h *AwaitHarness) {
	...
	if _, err := runs.Store.Claim(ctx, h.Workspace, "run-b", "node-2", "lease-2"); !errors.Is(err, domain.ErrInvalidTransition) {
...
func testStaleSweepSkipsSuspended(t *testing.T, h *AwaitHarness) {
	...
	result, err := runs.Store.RecoverStale(ctx, h.Workspace, store.StaleDriverRunRecovery{
...
func testResumeOnlyFromSuspendedFirstWinner(t *testing.T, h *AwaitHarness) {
	...
	if _, err := runs.Resume(ctx, "run-a", "event-10"); !errors.Is(err, domain.ErrInvalidTransition) {
