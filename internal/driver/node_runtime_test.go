...
func TestResolveProcessNodePath(t *testing.T) {
	...
	t.Run("packaged resource wins before PATH", func(t *testing.T) {
		dir := t.TempDir()
	...
	t.Run("executable sibling is not a runtime location", func(t *testing.T) {
		dir := t.TempDir()
		node := filepath.Join(dir, "node")
		if err := os.WriteFile(node, []byte("unowned executable sibling"), 0o755); err != nil {
			t.Fatalf("write executable sibling: %v", err)
		}
		got := resolveProcessNodePath("", filepath.Join(dir, "loom"))
		if got != "node" {
			t.Fatalf("resolveProcessNodePath() = %q, want PATH lookup", got)
		}
	})

	t.Run("non executable sibling uses PATH", func(t *testing.T) {
		dir := t.TempDir()
		node := filepath.Join(dir, "node")
