...
// Await-event driver-op logic (ARCHITECTURE-PROPOSAL §7 step 8, chunk AW9).
//
...
// AwaitMaxTimeoutEnvVar overrides the maximum await timeout (milliseconds).
// Part of the env-tunable timeout budget decision, together with the per-run
// await budget and total-suspend cap below (landed with the deadline
// sweeper, AW8).
const AwaitMaxTimeoutEnvVar = "LOOM_AWAIT_MAX_TIMEOUT_MS"

// DefaultAwaitMaxTimeout bounds timeoutMs when no override is configured.
const DefaultAwaitMaxTimeout = 14 * 24 * time.Hour

// AwaitMaxPerRunEnvVar overrides the per-run await budget: the highest
// awaitIndex a single run may register (the index is the dense 1-based
// ordinal, RULE 3, so the index IS the count).
const AwaitMaxPerRunEnvVar = "LOOM_AWAIT_MAX_PER_RUN"

// DefaultAwaitMaxPerRun bounds awaitIndex when no override is configured.
const DefaultAwaitMaxPerRun = 100

// AwaitTotalSuspendCapEnvVar overrides the per-run total-suspend cap
// (milliseconds): the time a run has already spent suspended across its
// prior awaits plus the new await's timeout may never exceed it, so a run's
// total suspension is bounded even within the per-await maximum.
const AwaitTotalSuspendCapEnvVar = "LOOM_AWAIT_TOTAL_SUSPEND_CAP_MS"

// DefaultAwaitTotalSuspendCap bounds a run's cumulative suspended time when
...
// Await listing for re-entry context rebuilding (GET driver/events/awaits).
const (
	// maxAwaitListProbe caps the dense instance-key probe.
	maxAwaitListProbe = 1000
	// awaitPendingScanLimit caps the pending-deadline scan backing the list.
	awaitPendingScanLimit = 10000
	// awaitPendingScanHorizon is how far past now the pending scan looks —
	// far beyond any admissible await deadline.
	awaitPendingScanHorizon = 24 * time.Hour * 365 * 10
)

// ListRunAwaits returns the run's awaits in index order for re-entry context
// rebuilding: terminal rows (with their recorded events and inline payloads)
// and pending rows. store.AwaitStore has no per-run listing (the AW5 storage
// deviation), so this composes the two reads it does have: pending rows from
// the deadline feed filtered to the run, terminal rows by probing the dense
// runID#await-{n} keys (RULE 3 — sequential awaits use increasing n from 1).
// The probe stops at the first index that is neither pending nor
// satisfied/timed_out, so a cancelled await ends the listing at its index.
func ListRunAwaits(ctx context.Context, st store.Store, ws, runID string) ([]*domain.AwaitInstance, error) {
	due, err := st.Awaits().ListDueAwaitDeadlines(ctx, ws,
	...
	for _, inst := range due {
		if inst.RunID == runID {
			pending[inst.InstanceKey] = inst
		}
	}
	out := []*domain.AwaitInstance{}
	for n := 1; n <= maxAwaitListProbe; n++ {
		key := domain.AwaitInstanceKey(runID, n)
		if inst, ok := pending[key]; ok {
			out = append(out, inst)
			continue
		}
		...
		if err != nil {
			return nil, fmt.Errorf("get await %s: %w", key, err)
		}
		out = append(out, inst)
	}
	return out, nil
