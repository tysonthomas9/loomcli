...
func checkedInSnapshotViolations(report Report) []string {
	...
	checkCount("composite Store files", len(report.CompositeStoreFiles), 12)
	...
	checkCount("direct persistence-write rows", report.DirectPersistenceWrites, 77)
	checkCount("runtime components", report.RuntimeComponents, 71)
	checkCount("runtime goroutine launches", report.RuntimeGoroutineLaunches, 77)
