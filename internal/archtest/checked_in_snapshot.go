...
func checkedInSnapshotViolations(report Report) []string {
	...
	checkCount("direct persistence-write rows", report.DirectPersistenceWrites, 90)
	checkCount("runtime components", report.RuntimeComponents, 70)
	checkCount("runtime goroutine launches", report.RuntimeGoroutineLaunches, 79)
