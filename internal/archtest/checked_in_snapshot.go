...
func checkedInSnapshotViolations(report Report) []string {
	...
	checkCount("composite Store files", len(report.CompositeStoreFiles), 64)
	checkCount("outside-composition Store files", len(report.CompositeStoreOutside), 54)
	...
	checkCount("mutation commands", report.MutationCommands, 106)
	checkCount("direct persistence-write rows", report.DirectPersistenceWrites, 122)
