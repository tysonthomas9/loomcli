...
func validateInventories(values []InventoryBaseline) error {
	if len(values) == 0 {
		return errors.New("baseline inventories must not be empty")
	}
	...
	for _, value := range values {
		...
		if value.Status != "complete" && value.Status != "deferred" {
			return fmt.Errorf("inventory %s has unsupported status %q", value.ID, value.Status)
		}
...
func validateDecisions(values []DecisionBaseline) error {
	...
	for _, value := range values {
		if value.Owner == "" {
			return fmt.Errorf("decision %s has no owner", value.ID)
		}
...
func (g CapabilityGraph) Validate() error {
	...
	if g.Status != "proposed" && g.Status != "approved" {
		return fmt.Errorf("capability graph status %q is unsupported", g.Status)
	}
	if g.Status == "approved" {
		return fmt.Errorf("analyzer %s cannot approve the capability graph until app, platform, legacy-path, and durable-cycle enforcement is implemented", AnalyzerVersion)
	}
	if !safeInternalRoot(g.ModuleRoot) {
		return fmt.Errorf("capability graph module_root must be a safe path under internal, got %q", g.ModuleRoot)
	}
...
func validateCapabilities(values []Capability) (map[string]Capability, error) {
	if len(values) == 0 {
		return nil, errors.New("capability graph must declare at least one capability")
	}
	...
	for _, edge := range edges {
		...
		for _, kind := range edge.Kinds {
			if kind != "import" && kind != "command" && kind != "query" && kind != "durable_event" {
				return fmt.Errorf("edge %s -> %s has unsupported kind %q", edge.From, edge.To, kind)
			}
		}
	}
	return nil
...
func (m AnalysisMatrix) Validate() error {
	...
	for _, profile := range append(append([]AnalysisProfile{}, m.Release...), m.Tagged...) {
		if profile.Enforced {
			return fmt.Errorf("analysis profile %s cannot be marked enforced by analyzer %s", profile.Name, AnalyzerVersion)
		}
	}
