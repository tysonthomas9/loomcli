...
func TestCheckedInDirectWriteInventoryStrictCounts(t *testing.T) {
	...
	if len(inventory.Writes) != 72 {
		t.Fatalf("direct-write rows = %d, want current source-backed ratchet of 72", len(inventory.Writes))
	}
	totalSites := 0
	...
	if totalSites != 86 {
		t.Fatalf("direct-write sites = %d, want current source-backed ratchet of 86", totalSites)
	}
