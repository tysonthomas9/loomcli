...
func TestCheckedInDirectWriteInventoryStrictCounts(t *testing.T) {
	...
	if len(inventory.Writes) != 89 {
		t.Fatalf("direct-write rows = %d, want current migration ratchet of 89", len(inventory.Writes))
	}
	totalSites := 0
	...
	if totalSites != 98 {
		t.Fatalf("direct-write sites = %d, want current migration ratchet of 98", totalSites)
	}
