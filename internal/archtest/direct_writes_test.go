...
func TestCheckedInDirectWriteInventoryStrictCounts(t *testing.T) {
	...
	if len(inventory.Writes) != 93 {
		t.Fatalf("direct-write rows = %d, want current migration ratchet of 93", len(inventory.Writes))
	}
	totalSites := 0
	...
	if totalSites != 102 {
		t.Fatalf("direct-write sites = %d, want current migration ratchet of 102", totalSites)
	}
