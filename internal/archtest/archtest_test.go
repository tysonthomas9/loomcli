...
func TestCheckedInManifestsAndRepository(t *testing.T) {
	...
	if got, want := report.MutationCommands, 108; got != want {
		t.Fatalf("mutation commands = %d, want %d", got, want)
	}
	if got, want := report.DirectPersistenceWrites, 72; got != want {
...
func TestCheckedInPhase7ArchitectureContracts(t *testing.T) {
	...
	if len(ledger.Commands) != 108 {
		t.Fatalf("mutation commands = %d, want 108", len(ledger.Commands))
	}
}
...
func TestRetiredSourceCompatibilityAPIsCannotReturn(t *testing.T) {
	...
	for _, relative := range []string{
		...
		"internal/infra/workflowdistribution/builtin_authoring.go",
		"internal/infra/workflowdistribution/native_authoring.go",
		"internal/infra/workflowdistribution/legacy_authoring_test.go",
		"internal/infra/workflowdistribution/legacy_compat_test.go",
