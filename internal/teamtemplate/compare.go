...
func compareRole(tpl TemplateRole, existing *domain.Role) []string {
	...
	return divergedFields([]fieldCheck{
		...
		fieldCheck{name: "read_only", diverged: tpl.ReadOnly != existing.ReadOnly},
		fieldCheck{name: "denied_tools", diverged: !sameSet(tpl.DeniedTools, existing.DeniedTools)},
		fieldCheck{name: "allowed_tools", diverged: !sameSet(tpl.AllowedTools, existing.AllowedTools)},
