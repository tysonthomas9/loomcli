...
func TestRunApplyProvisionsSingleRepoAgentWorktree(t *testing.T) {
	...
	writeDefFile(t, root, ".loom/agents/nova.ts", `import { createAgent, runtime } from '@loom/runtime';
...
export default createAgent({
  ...
  instructions: 'Work on Slack-clone tasks.',
	...
	st := memstore.New()
	const workspace = "TSWT"
	if _, err := st.Workspaces().Create(ctx, store.WorkspaceCreate{Key: workspace, Name: "TypeScript Worktree"}); err != nil {
		t.Fatalf("create workspace: %v", err)
	}
	...
	withTSFirstGlobals(t, func() {
		applyDir = root
		applyStart = true
		if err := runApply(nil, []string{"nova"}); err != nil {
			t.Fatalf("runApply() error = %v", err)
		}
	})
	...
	if _, err := os.Stat(filepath.Join(worktreePath, ".git")); err != nil {
		t.Fatalf("applied worktree .git = %v, want provisioned worktree at %s", err, worktreePath)
	}
}
