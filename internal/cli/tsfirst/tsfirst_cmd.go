...
func invokeLocalAgent(ctx context.Context, plan *defspkg.Plan, agent defspkg.AgentModule, prompt, message string, stream io.Writer, resumeProviderSessionID string) (localInvocationResult, error) {
	...
	backendName, backend, err := lookupLocalBackend(agent)
	if err != nil {
		return localInvocationResult{}, err
	}
...
func lookupLocalBackend(agent defspkg.AgentModule) (string, cli.Backend, error) {
	...
	if !ok {
		return "", nil, fmt.Errorf("backend %q is not registered; use backend \"echo\" for offline local connect or install/configure the backend", backendName)
	}
	return backendName, backend, nil
}

type streamingLocalInvocation struct {
	backendName        string
	backend            cli.Backend
...
func invokeStreamingLocalAgent(ctx context.Context, in streamingLocalInvocation) (localInvocationResult, error) {
	rc, err := openLocalStreamingResponse(ctx, in)
	if err != nil {
		return localInvocationResult{}, err
	}
...
func openLocalStreamingResponse(ctx context.Context, in streamingLocalInvocation) (io.ReadCloser, error) {
	if in.resume != nil {
		if resumable, ok := in.backend.(backendcaps.ResumableStreamingBackend); ok {
			rc, err := resumable.InvokeStreamingResumed(ctx, in.workDir, in.prompt, in.agentName, in.resume.PriorProviderSessionID)
			if err != nil {
				return nil, err
			}
			markResumeApplied(in.resume, connectResumeMethodStreamingResumed)
			return rc, nil
		}
		if setBackendResumeSessionID(in.backend, in.resume.PriorProviderSessionID) {
			markResumeApplied(in.resume, connectResumeMethodSetter)
		} else {
			markResumeUnsupported(in.resume, in.backendName)
		}
	}
	return in.streamer.InvokeStreaming(ctx, in.workDir, in.prompt, in.agentName)
}

func invokeNonStreamingLocalAgentWithResume(backendName string, backend cli.Backend, workDir, prompt, agentName string, stream io.Writer, resume *connectResume, appliedToolRuntime *connectToolRuntime) (localInvocationResult, error) {
	result, err := runNonStreamingLocalAgent(backendName, backend, workDir, prompt, agentName, stream, resume)
	result = finishLocalInvocationResult(result, backend, workDir, resume, appliedToolRuntime)
	return result, err
}

func runNonStreamingLocalAgent(backendName string, backend cli.Backend, workDir, prompt, agentName string, stream io.Writer, resume *connectResume) (localInvocationResult, error) {
	if resume == nil {
		return invokeNonStreamingLocalAgent(backend, backendName, workDir, prompt, agentName, stream)
	}
	if resumable, ok := backend.(backendcaps.ResumableNonInteractiveBackend); ok {
		result, err := invokeNonStreamingLocalAgentWithRunner(backendName, agentName, stream, func(shutdown <-chan struct{}, collector *usage.Collector) error {
			return resumable.InvokeNonInteractiveResumed(workDir, prompt, agentName, resume.PriorProviderSessionID, shutdown, collector)
		})
		markResumeApplied(resume, connectResumeMethodNonInteractiveResumed)
		return result, err
	}
	if setBackendResumeSessionID(backend, resume.PriorProviderSessionID) {
...
func finishLocalInvocationResult(result localInvocationResult, backend cli.Backend, workDir string, resume *connectResume, appliedToolRuntime *connectToolRuntime) localInvocationResult {
	...
	return fillBackendSessionID(result, backend, workDir)
}
