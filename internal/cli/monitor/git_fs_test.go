...
func makeTempRepo(t *testing.T) string {
	...
	if _, err := cli.RunGitCommand(tmpDir, "config", "user.email", "test@test.com"); err != nil {
		t.Fatalf("git config email failed: %v", err)
	}
	if _, err := cli.RunGitCommand(tmpDir, "config", "user.name", "Test"); err != nil {
	...
	if _, err := cli.RunGitCommand(tmpDir, "add", "."); err != nil {
		t.Fatalf("git add failed: %v", err)
	}
	if _, err := cli.RunGitCommand(tmpDir, "commit", "-m", "init"); err != nil {
...
func TestReadBranchFromFS_MatchesGitCLI(t *testing.T) {
	...
	cliResult, err := cli.RunGitCommand(repoRoot, "branch", "--show-current")
	if err != nil {
...
func TestReadRefSHA_MatchesGitCLI(t *testing.T) {
	...
	cliSHA, err := cli.RunGitCommand(repoRoot, "rev-parse", "refs/heads/"+branch)
	if err != nil {
...
func TestReadBranchFromFS_DetachedHead(t *testing.T) {
	...
	// Detach HEAD
	sha, err := cli.RunGitCommand(tmpDir, "rev-parse", "HEAD")
	if err != nil {
		t.Fatalf("git rev-parse HEAD: %v", err)
	}
	if _, err := cli.RunGitCommand(tmpDir, "checkout", "--detach", strings.TrimSpace(sha)); err != nil {
...
func TestReadRefSHA_TempRepo(t *testing.T) {
	...
	cliSHA, err := cli.RunGitCommand(tmpDir, "rev-parse", "refs/heads/"+branch)
	if err != nil {
