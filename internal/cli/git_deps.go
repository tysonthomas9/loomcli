...
func gitFetch(deps *Deps, dir string) error {
	fmt.Println("Fetching from origin...")
	return runGitOutput(deps, dir, "fetch", "origin")
}

func gitCheckout(deps *Deps, dir, branch string) error {
...
func gitPull(deps *Deps, dir, branch string) error {
	if err := validateGitRef(branch); err != nil {
		return err
	}
...
func gitMerge(deps *Deps, dir, branch, message string) error {
	if err := validateGitRef(branch); err != nil {
		return err
	}
...
func gitMergeOrigin(deps *Deps, dir, branch, message string) error {
	if err := validateGitRef(branch); err != nil {
		return err
	}
...
func gitPush(deps *Deps, dir, branch string) error {
	if err := validateGitRef(branch); err != nil {
		return err
	}
...
func gitPushForce(deps *Deps, dir, branch string) error {
	if err := validateGitRef(branch); err != nil {
		return err
	}
...
func gitReset(deps *Deps, dir, ref string) error {
...
func gitClean(deps *Deps, dir string) error {
	fmt.Println("Cleaning untracked files...")
	return runGitOutput(deps, dir, "clean", "-fd")
}

func gitCleanDryRun(deps *Deps, dir string) (string, error) {
	return runGit(deps, dir, "clean", "-fdn")
}

func gitCleanExclude(deps *Deps, dir string, excludes []string) error {
...
func gitCleanDryRunExclude(deps *Deps, dir string, excludes []string) (string, error) {
...
func getConflictedFilesDeps(deps *Deps, dir string) ([]string, error) {
...
func hasCommitsBetweenDeps(deps *Deps, dir, target, source string) (bool, error) {
...
func isCleanWorkingTreeDeps(deps *Deps, dir string) (bool, error) {
...
func gitFetchRemote(deps *Deps, dir, remote string) error {
...
func gitMergeRemote(deps *Deps, dir, remote, branch, message string) error {
	r := resolveRemote(remote)
	if err := validateGitRef(r); err != nil {
		return err
	}
...
func gitPushRemote(deps *Deps, dir, remote, branch string) error {
	r := resolveRemote(remote)
	if err := validateGitRef(r); err != nil {
		return err
	}
...
func gitPullRemote(deps *Deps, dir, remote, branch string) error {
	r := resolveRemote(remote)
	if err := validateGitRef(r); err != nil {
		return err
	}
...
func hasCommitsBetweenRemoteDeps(deps *Deps, dir, remote, target, source string) (bool, error) {
	r := resolveRemote(remote)
	if err := validateGitRef(r); err != nil {
		return false, err
	}
...
func isRefCheckedOutInWorktreeDeps(deps *Deps, dir, branch string) (bool, string, error) {
...
func gitCheckoutDetached(deps *Deps, dir, ref string) error {
	if err := validateGitRef(ref); err != nil {
		return err
	}
...
func gitCreateBranchFromHead(deps *Deps, dir, name string) error {
...
func gitDeleteBranch(deps *Deps, dir, name string, force bool) error {
	if err := validateGitRef(name); err != nil {
		return err
	}
	flag := "-d"
...
func gitPushRefspec(deps *Deps, dir, remote, localRef, remoteRef string) error {
	r := resolveRemote(remote)
	if err := validateGitRef(r); err != nil {
		return err
	}
...
func getStashCountDeps(deps *Deps, dir string) (int, error) {
...
func gitStash(deps *Deps, dir string) (bool, error) {
...
func gitStashPop(deps *Deps, dir string) error {
	fmt.Println("Restoring stashed changes...")
	return runGitOutput(deps, dir, "stash", "pop")
}

func branchExistsLocallyDeps(deps *Deps, dir, branch string) (bool, error) {
	if err := validateGitRef(branch); err != nil {
		return false, err
	}
...
func remoteBranchExistsDeps(deps *Deps, dir, remote, branch string) (bool, error) {
	r := resolveRemote(remote)
	if err := validateGitRef(r); err != nil {
		return false, err
	}
...
func gitCheckoutNewFromRef(deps *Deps, dir, branch, startPoint string) error {
	if err := validateGitRef(branch); err != nil {
		return err
	}
...
func gitMergeAbort(deps *Deps, dir string) error {
	_, err := runGit(deps, dir, "merge", "--abort")
	return err
}

func hasUnmergedFilesDeps(deps *Deps, dir string) (bool, error) {
