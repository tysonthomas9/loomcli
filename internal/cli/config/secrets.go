...
type OnePasswordBackend struct {
	...
	execCtx     func(ctx context.Context, dir, name string, args ...string) (stdout, stderr string, err error)
}

// NewOnePasswordBackend creates a OnePasswordBackend using exec.LookPath and exec.CommandContext.
func NewOnePasswordBackend() *OnePasswordBackend {
	_, err := exec.LookPath("op")
	return &OnePasswordBackend{
		opAvailable: err == nil,
		lookPath:    exec.LookPath,
		execCtx:     defaultOpExec,
	}
}

// NewOnePasswordBackendWithExec creates a OnePasswordBackend with custom exec functions.
func NewOnePasswordBackendWithExec(lookPath func(string) (string, error), execCtx func(ctx context.Context, dir, name string, args ...string) (stdout, stderr string, err error)) *OnePasswordBackend {
	_, err := lookPath("op")
	return &OnePasswordBackend{
		opAvailable: err == nil,
		lookPath:    lookPath,
...
func (b *OnePasswordBackend) Resolve(name string) (string, bool, error) {
	...
	stdout, stderr, err := b.execCtx(ctx, "", "op", "read", name)
	if err != nil {
		return "", false, fmt.Errorf("op read %s: %s", redactOpURI(name), strings.TrimSpace(stderr))
	}
	return strings.TrimSpace(stdout), true, nil
}

// defaultOpExec runs a command using exec.CommandContext.
func defaultOpExec(ctx context.Context, dir, name string, args ...string) (string, string, error) {
