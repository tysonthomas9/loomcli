...
type Daemon struct {
	...
	repos         []cfgpkg.RepoConfig  // workspace repos for resolveAgentRepos; nil outside workspace mode
...
func (d *Daemon) configSnapshot() *config.DaemonConfig {
	d.reconcileMu.RLock()
	cfg := d.config
...
// findRepoConfig looks up a cfgpkg.RepoConfig by name in d.repos.
// Returns nil if repoName is empty or not found (non-workspace mode).
func (d *Daemon) findRepoConfig(repoName string) *cfgpkg.RepoConfig {
	if repoName == "" {
		return nil
	}
	for i := range d.repos {
...
func NewDaemon(config *config.DaemonConfig, projectDir string, eventBus events.Emitter, issueBackend backend.IssueBackend) (*Daemon, error) {
	if config == nil {
