...
func TestWriteStateFile_CarriesTheHoldAndGatedFlag(t *testing.T) {
	...
	if err := writeStateFile(path, time.Now(), agents, nil, nil, nil, nil, 3, stateExtras{Hold: hold}); err != nil {
	...
	if err := writeStateFile(path, time.Now(), agents, nil, nil, nil, nil, 3); err != nil {
