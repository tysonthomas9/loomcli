...
// or liveness watchdog timeout). Extracted so tests can exercise the exit-
// code policy without spinning up a real daemon and its sockets/files.
...
// timeout). Callers should propagate the non-zero code to os.Exit so external
// process supervisors see the failure.
