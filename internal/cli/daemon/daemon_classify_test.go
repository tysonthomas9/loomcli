...
func writeLockFile(t *testing.T, dir string, info *LockInfo) {
	t.Helper()
...
func TestHandleAgentCheckpoint_YieldExit(t *testing.T) {
	...
	writeLockFile(t, tmpDir, &LockInfo{
		PID:       os.Getpid(),
		Command:   "task",
		AgentName: "falcon",
		TaskID:    "bd-yield-100",
	...
	d.handleAgentCheckpoint(ap, 0)

	// Verify checkpoint was saved with yield metadata
	lockDir := ResolveLockDir(tmpDir)
	cp, err := LoadCheckpoint(lockDir)
	if err != nil {
...
func TestHandleAgentCheckpoint_NormalSuccess(t *testing.T) {
	...
	if err := SaveCheckpoint(lockDir, oldCp); err != nil {
...
func TestHandleAgentCheckpoint_CrashWithYieldFile(t *testing.T) {
	...
	d := &Daemon{config: &DaemonConfig{}}
	ap := &AgentProcess{
		entry:        AgentEntry{Worktree: "hawk"},
		worktreePath: tmpDir,
	}

	// Call handleAgentCheckpoint with exit code 1 (crash path)
	d.handleAgentCheckpoint(ap, 1)
