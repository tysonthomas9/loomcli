...
func TestReadWorkspacePIDFile_MissingAndGarbage(t *testing.T) {
	...
	if got, ok := readWorkspacePIDFile(filepath.Join(dir, "absent.pid")); ok || got.PID != 0 || !got.StartedAt.IsZero() {
	...
	if got, ok := readWorkspacePIDFile(garbage); ok || got.PID != 0 || !got.StartedAt.IsZero() {
