...
const AgentProfilesDirName = harnessprofile.DirName
...
const ProfileManifestName = harnessprofile.ManifestName

// ── The per-agent harness profile policy lives in internal/harnessprofile ──
//
// It used to live here. It moved DOWN because a third caller needed it and
// could not reach it: this package imports internal/cli/agent (spawn.go,
// classify.go, resume_recovery.go), so the standalone `loom agent` path can
// never import this one, and interposing a helper package only lengthens the
// cycle. `loom lead` got away with calling in here only because it is a
// sibling package, not `agent` itself.
//
// The names below stay exported from the supervisor so every existing caller
// and test keeps compiling. They are aliases and one-line delegations, never
// copies: the sentinels must remain the SAME values (errors.Is is used across
// packages) and the version cache must remain the SAME instance (tests reset
// it through here).
...
var (
	ErrProfileManifestMissing     = harnessprofile.ErrProfileManifestMissing
	ErrProfileManifestUnreadable  = harnessprofile.ErrProfileManifestUnreadable
	ErrProfileFingerprintMismatch = harnessprofile.ErrProfileFingerprintMismatch
	ErrProfileVersionDrift        = harnessprofile.ErrProfileVersionDrift
	ErrProfileVersionUnknown      = harnessprofile.ErrProfileVersionUnknown
	ErrProfileTokenUnreadable     = harnessprofile.ErrProfileTokenUnreadable
)

// ProfileHarnesses returns the harnesses a profile root can be provisioned for.
func ProfileHarnesses() []string { return harnessprofile.ProfileHarnesses() }
...
func ProfileHarnessBinary(harness string) string {
	return harnessprofile.ProfileHarnessBinary(harness)
}

// ProfileEnvVar returns the environment variable a harness profile root is
// exported as, or "" for an unknown harness.
func ProfileEnvVar(harness string) string { return harnessprofile.ProfileEnvVar(harness) }

// ProfileHarnessEnv resolves one harness profile root for an agent, verifies
// it, and returns the KEY=VALUE assignments that export it.
func ProfileHarnessEnv(projectDir, agent, harness string) (string, []string, error) {
	return harnessprofile.ProfileHarnessEnv(projectDir, agent, harness)
}

// ProfileSecretEnv returns the assignments exporting the credential a harness
// profile root carries of its own, or nothing when it carries none.
func ProfileSecretEnv(dir, harness string) ([]string, error) {
	return harnessprofile.ProfileSecretEnv(dir, harness)
}
...
// credential that root carries of its own. This is the spawn path's entry
// point into the policy.
func AppendProfileEnv(env []string, projectDir, agent string) ([]string, error) {
	return harnessprofile.AppendProfileEnv(env, projectDir, agent)
}

// VerifyProfileManifest applies the spawn path's verify-or-refuse rule to a
// profile root for a caller outside the daemon.
func VerifyProfileManifest(dir, binary string) error {
	return harnessprofile.VerifyProfileManifest(dir, binary)
}

// ResetHarnessVersionCache drops every cached probe. For testing only — and it
// must reach the same cache the injector reads, which is why it delegates
// rather than clearing a local copy.
func ResetHarnessVersionCache() { harnessprofile.ResetHarnessVersionCache() }
