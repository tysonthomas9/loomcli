...
// ── pre-spawn skill and hook materialization ─────────────────────────────
//
// These live beside the backend gate rather than in spawn.go because that
// file is at the package's 1000-line LOC ceiling, and the supervisor package
// is at its file-count ceiling, so they can be neither there nor in a file of
// their own. The gate and these are the same phase of the spawn anyway: what
// must be true on disk before the agent process starts.
