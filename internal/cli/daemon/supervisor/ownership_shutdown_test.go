...
func TestStopSweepsOwnershipLeasesWhenSuperviseGoroutineNeverExits(t *testing.T) {
	...
	oldRelease := shutdownLeaseReleaseTimeout
	shutdownLeaseReleaseTimeout = 50 * time.Millisecond
	t.Cleanup(func() { shutdownLeaseReleaseTimeout = oldRelease })
	...
	go func() {
		defer close(done)
		// A budget small enough that the goroutine wait gives up at once.
		s.StopWithBudget(100 * time.Millisecond)
	}()
	select {
	case <-done:
