	...
	t.Run("increments count and returns true when under limit", func(t *testing.T) {
		s := &Supervisor{
			ConfigSnapshot: func() *cfgpkg.DaemonConfig {
				return &cfgpkg.DaemonConfig{Daemon: cfgpkg.DaemonSettings{RestartPolicy: cfgpkg.RestartPolicy{MaxRetries: cfgpkg.IntPtr(3), BackoffInitial: cfgpkg.IntPtr(1), BackoffMax: cfgpkg.IntPtr(1)}}}
			},
			Shutdown:      make(chan struct{}),
			StoppedAgents: make(map[string]struct{}),
			Agents:        make([]*AgentProcess, 0),
		...
		result := s.handleRestartAfterError(ap)
		if !result {
...
func TestHandleRestartAfterError_ShutdownDuringBackoff(t *testing.T) {
	s := &Supervisor{
		ConfigSnapshot: func() *cfgpkg.DaemonConfig {
			return &cfgpkg.DaemonConfig{Daemon: cfgpkg.DaemonSettings{RestartPolicy: cfgpkg.RestartPolicy{MaxRetries: cfgpkg.IntPtr(10), BackoffInitial: cfgpkg.IntPtr(60), BackoffMax: cfgpkg.IntPtr(60)}}}
		},
		Shutdown:      make(chan struct{}),
		StoppedAgents: make(map[string]struct{}),
		Agents:        make([]*AgentProcess, 0),
...
func TestHandleRestartAfterError_SequentialEscalation(t *testing.T) {
	s := &Supervisor{
		ConfigSnapshot: func() *cfgpkg.DaemonConfig {
			return &cfgpkg.DaemonConfig{Daemon: cfgpkg.DaemonSettings{RestartPolicy: cfgpkg.RestartPolicy{
				MaxRetries:     cfgpkg.IntPtr(3),
				BackoffInitial: cfgpkg.IntPtr(0), // zero backoff for fast test
				BackoffMax:     cfgpkg.IntPtr(0),
			}}}
		},
		Shutdown:      make(chan struct{}),
	...
	ap := &AgentProcess{
		Entry:        cfgpkg.AgentEntry{Worktree: "test-escalation"},
		RestartCount: 0,
	}
...
func TestHandleRestartAfterError_ActualBackoffDelay(t *testing.T) {
	s := &Supervisor{
		ConfigSnapshot: func() *cfgpkg.DaemonConfig {
			return &cfgpkg.DaemonConfig{Daemon: cfgpkg.DaemonSettings{RestartPolicy: cfgpkg.RestartPolicy{
				MaxRetries:     cfgpkg.IntPtr(10),
				BackoffInitial: cfgpkg.IntPtr(1), // 1 second
				BackoffMax:     cfgpkg.IntPtr(1),
			}}}
		},
		Shutdown:      make(chan struct{}),
	...
	ap := &AgentProcess{
		Entry:        cfgpkg.AgentEntry{Worktree: "test-delay"},
		RestartCount: 0,
	}
