...
// listCountingAgentSessionStore counts List calls so the once-per-process guard can
// be asserted directly.
type listCountingAgentSessionStore struct {
	store.AgentSessionStore
	lists atomic.Int64
}

func (c *listCountingAgentSessionStore) List(ctx context.Context, ws string, filter store.AgentSessionFilter) ([]*domain.AgentSession, error) {
	c.lists.Add(1)
...
func TestRecordAbandonedRunsForAgentRunsOncePerProcess(t *testing.T) {
	...
	s.IssueBackend = mock
	counting := &listCountingAgentSessionStore{AgentSessionStore: base.AgentSessions()}
