...
func TestHasActiveEphemeralTask_RequestedTask(t *testing.T) {
	s := &Supervisor{
		Agents: []*AgentProcess{{
	...
	if !s.HasActiveEphemeralTask("worker") {
		t.Fatal("HasActiveEphemeralTask = false, want true for requested ephemeral task")
	}
}

func TestHasActiveEphemeralTask_DoneReturnsFalse(t *testing.T) {
	done := make(chan struct{})
	close(done)
	s := &Supervisor{
		Agents: []*AgentProcess{{
	...
	if s.HasActiveEphemeralTask("worker") {
		t.Fatal("HasActiveEphemeralTask = true, want false after supervisor goroutine is done")
	}
}

func TestHasActiveEphemeralTask_ServiceAgentReturnsFalse(t *testing.T) {
	s := &Supervisor{
		Agents: []*AgentProcess{{
			Entry:          cfgpkg.AgentEntry{Worktree: "worker", Role: "task", Mode: domain.AgentModeService},
			AssignedTaskID: "TASK-1",
			Done:           make(chan struct{}),
		}},
	}
	...
	if s.HasActiveEphemeralTask("worker") {
		t.Fatal("HasActiveEphemeralTask = true, want false for service agent")
	}
}
