...
//
// RunCritical performs its own Wg.Add(1). Callers that must coordinate Wg
// growth with a mutex should use RecoverAndSignal directly inside an inline
// goroutine instead.
...
// RangeTicks iterates over registered tick slots, invoking fn for each. fn
// must not call RecordTick or RegisterTick on the same name (sync.Map allows
// concurrent reads but RangeTicks should remain side-effect free).
