...
func TestCheckAgentHealth_Watchdog(t *testing.T) {
	...
	t.Run("uses latest activity when transcript is stale but log is fresh", func(t *testing.T) {
		...
		cmd := exec.Command("sleep", "30") //nolint:norawexec // test fixture: spawns a benign sleep child to exercise process-group kill
...
// TestBuildCommand_RoutingEnvVars and TestBuildCommand_NoRoutingEnvVars moved
// to routing_env_test.go to keep this file under the LOC test-file ceiling.
