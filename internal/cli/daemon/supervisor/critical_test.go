...
// newHarnessSupervisor returns a Supervisor configured for harness tests: an
// initialized Shutdown channel and FatalCh buffered to 1. This avoids pulling
// in NewDaemon / NewAgent which need a real workspace.
...
func TestRunCriticalPanicSignalsFatal(t *testing.T) {
	...
	// Goroutine must have exited so Wg drains.
...
func TestRunCriticalReturnWithoutShutdownSignalsFatal(t *testing.T) {
	s := newHarnessSupervisor()
	// Critical loop returns immediately — supervisor still expects it to run.
	s.RunCritical("test_early_return", func() {
		// no-op: returns immediately without observing s.Shutdown
	})
	...
	case err := <-s.FatalChannel():
		...
		if !strings.Contains(err.Error(), "returned without shutdown") {
			t.Errorf("error missing returned-without-shutdown reason: %v", err)
		}
	case <-time.After(2 * time.Second):
...
func TestRunCriticalNormalShutdownDoesNotSignalFatal(t *testing.T) {
	...
	// Close Shutdown — the goroutine should exit cleanly without firing FatalCh.
	close(s.Shutdown)
...
func TestSignalFatalFiresOnlyOnce(t *testing.T) {
	...
	select {
	case err := <-s.FatalChannel():
		t.Fatalf("second error leaked to FatalChannel: %v", err)
...
func TestRegisterAndRecordTick(t *testing.T) {
	...
	// Unregistered name is silent and ungettable.
