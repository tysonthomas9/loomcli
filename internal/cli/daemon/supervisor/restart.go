...
func (s *Supervisor) shouldRestart(ap *AgentProcess) bool {
	...
	// Ephemeral mode exits cleanly after one successful task cycle; stop the
	// supervisor instead of restarting it. A NoWork exit still falls through
...
// stopAfterEphemeralTask reports whether an ephemeral-mode agent has just
// completed one successful task cycle and should exit its supervisor instead
// of restarting. A NoWork exit (no assigned task) returns false so the agent
// keeps polling until a task arrives. Caller holds ap.Mu.
