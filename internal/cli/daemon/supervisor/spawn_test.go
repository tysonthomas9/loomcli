...
func TestSetupAgentLogFile_LogStartOffset(t *testing.T) {
	...
	t.Run("empty log starts at zero", func(t *testing.T) {
		...
		if ap.LogFileStartOffset != 0 {
			t.Errorf("LogFileStartOffset = %d, want 0 for a fresh log", ap.LogFileStartOffset)
		}
	})
	...
	t.Run("append picks up the pre-existing size", func(t *testing.T) {
		...
		if second.LogFileStartOffset != info.Size() {
			t.Errorf("LogFileStartOffset = %d, want %d (pre-existing log size)", second.LogFileStartOffset, info.Size())
		}
		if second.LogFileStartOffset != int64(len(prior)) {
			t.Errorf("LogFileStartOffset = %d, want %d", second.LogFileStartOffset, len(prior))
		}
	})
}
