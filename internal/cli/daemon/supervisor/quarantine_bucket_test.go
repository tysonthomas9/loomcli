...
// TestDeadlineCount_SurvivesADaemonRestart pins the JSON tag on DeadlineCount.
//
// The ledger is persisted precisely because the failure mode that produces
// boomeranging tasks is a daemon that PM2 restarts, and an untagged second
// counter would silently reset to zero on exactly that path — the deadline
// bucket could then never reach its (higher) threshold on the host it matters
// on, with nothing anywhere reporting a problem.
func TestDeadlineCount_SurvivesADaemonRestart(t *testing.T) {
	dir := t.TempDir()

	sA := newPersistSupervisor(dir, nil)
	ap := newExpiredAgent(t, "falcon", "T-D9")
	sA.recordTaskExitForQuarantine(ap, 1)
	sA.recordTaskExitForQuarantine(ap, 1)
	sA.recordTaskExitForQuarantine(ap, 1)

	if got := deadlineCount(sA, "T-D9"); got != 3 {
		t.Fatalf("setup: DeadlineCount = %d, want 3", got)
	}

	sB := newPersistSupervisor(dir, nil)
	after := record(sB, "T-D9")
	if after == nil {
		t.Fatal("record did not survive the restart")
	}
	if after.DeadlineCount != 3 {
		t.Errorf("DeadlineCount after restart = %d, want 3 (the field must be json-tagged)", after.DeadlineCount)
	}
	if after.Count != 0 {
		t.Errorf("Count after restart = %d, want 0 — expiries must not reload onto the crash counter", after.Count)
	}
}
