...
func TestResolveWorkspaceByName_LegacyMode(t *testing.T) {
	...
	if r.Mode != cli.ModeLegacy {
		t.Fatalf("expected cli.ModeLegacy, got %d", r.Mode)
	}
...
func TestEnsureRepoWorktree_BranchExists(t *testing.T) {
	...
	// Create branch first
	cli.RunGitCommand(repoPath, "branch", "agent1")
...
func TestWorktreeCompletion_WorkspaceMode(t *testing.T) {
	...
	completions, directive := cli.WorktreeCompletion(nil, []string{}, "")
