...
- If approved, run `loom data update <id> --status open --remove-label needs-revision`.
  Clearing the label is what ends the review loop: the planner selects issues that
  carry `needs-revision` and the worker skips them, so a status-only approve sends
  the plan straight back for another review.
