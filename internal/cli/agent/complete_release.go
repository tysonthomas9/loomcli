...
//   - the lock has no TaskID (agent exited before claiming, no-op)
//   - the active issue backend does not implement backend.ClaimReleaser (no-op)
...
func releaseClaimOnComplete(worktreePath string) {
	...
	defer cancel()
	if err := releaser.ReleaseClaim(ctx, info.TaskID); err != nil {
