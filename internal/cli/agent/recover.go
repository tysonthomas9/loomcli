...
// On this path the supervisor has ALREADY claimed the task it is about to run,
// so full recovery would hand that claim straight back. A stale lock left in
// this worktree may still name an orphaned task; the next cycle's ordinary
// pre-flight recovery, which runs against the carried placement, releases it
// with the task id in hand.
//
// A lock whose process is still ALIVE is left alone: another agent is working
// there, and this is not the path that arbitrates that.
