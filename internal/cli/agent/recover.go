...
func releaseFleetIssueLock(deps *cli.Deps, agentName, taskID string) {
	...
	if err := deps.IssueBackend.ReleaseIssueLock(ctx, taskID, agentName); err != nil {
		// KindNotFound here means the lock was already gone — that's the
		// happy path for an idempotent release. Anything else is logged so
		// operators can see why the lock might still be held.
