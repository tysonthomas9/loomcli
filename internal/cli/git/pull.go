...
func runPull(cmd *cobra.Command, args []string) error {
	...
	if all {
		...
		return pullAllWorkspaces(deps, sourceBranch, !noPush)
	}
...
func pullAllWorkspaces(deps *cli.Deps, sourceBranch string, pushAfterPull bool) error {
	resolver, err := cli.NewResolver()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error creating resolver: %v\n", err)
		os.Exit(1)
	...
	for _, wsName := range wsNames {
		...
		notInSync += summaryFailures(pullWorkspaceWorktrees(deps, worktrees, sourceBranch, pushAfterPull))
		fmt.Println("")
	}
...
func pullWorkspaceRepo(deps *cli.Deps, resolver *cli.Resolver, worktreeName, sourceBranch string, pushAfterPull bool) {
	...
	if _, err := pullRepoWorktree(deps, matched.Path, matched.Branch, source, remote, pushAfterPull); err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}
}

func pullWorkspaceWorktrees(deps *cli.Deps, worktrees []cli.WorktreeInfo, sourceBranch string, pushAfterPull bool) []pullOutcome {
	return pullWorkspaceWorktreesWithCoverage(deps, worktrees, sourceBranch, pushAfterPull, nil)
}
...
func pullWorkspaceWorktreesWithCoverage(deps *cli.Deps, worktrees []cli.WorktreeInfo, sourceBranch string, pushAfterPull bool, notCovered []string) []pullOutcome {
	...
	for _, wt := range worktrees {
		...
		outcome, _ := pullRepoWorktree(deps, wt.Path, wt.Branch, source, remote, pushAfterPull)
...
//
// When pushAfterPull is true a clean merge is published by pushing
// currentBranch to the remote. With pushAfterPull=false the function performs
// NO remote-writing operation: it fetches and merges only. That is what
// `loom sync --pull-only` needs — the flag promises that nothing is published,
// and this push is the one that used to break that promise by publishing every
// worktree's current branch.
func pullRepoWorktree(deps *cli.Deps, repoPath, currentBranch, sourceBranch, remote string, pushAfterPull bool) (pullOutcome, error) {
