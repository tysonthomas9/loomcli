...
func pullWorkspaceWorktrees(deps *cli.Deps, worktrees []cli.WorktreeInfo, sourceBranch string, pushAfterPull bool) []pullOutcome {
	return pullWorkspaceWorktreesWithCoverage(deps, worktrees, sourceBranch, nil, pushAfterPull)
}
...
func pullWorkspaceWorktreesWithCoverage(deps *cli.Deps, worktrees []cli.WorktreeInfo, sourceBranch string, notCovered []string, pushAfterPull bool) []pullOutcome {
...
// flag promises that nothing is published, and the push is what used to break
// that promise. Verification still runs, because being in sync with the source
// branch does not depend on having published the merge.
