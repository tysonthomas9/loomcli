...
func validateWorkerFlags() (string, string) {
	...
	if workerBackend != "" {
		if err := SetBackend(workerBackend); err != nil {
...
func runWorker(cmd *cobra.Command, args []string) {
	...
	// Use the token returned from registration if one was provided
	authToken := workerToken
	if reg.Token != "" {
		authToken = reg.Token
	}

	// Step 2: Set up HTTP-backed interfaces
	lockBridge := &HTTPLockBridge{
		ControlPlaneURL: workerControlPlane,
	...
	// Redirect stdout/stderr to log forwarder as well as local output
	multiOut := io.MultiWriter(os.Stdout, logForwarder)
	log.SetOutput(multiOut)

	// Step 3: Set up shutdown handling
	...
	// Step 4: Acquire lock and run auto mode loop
	if err := AcquireLock(worktreePath, "worker", workerAgent); err != nil {
