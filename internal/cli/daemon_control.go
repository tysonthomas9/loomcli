...
func (d *Daemon) handleAgentControlStart(name string) DaemonControlResponse {
	...
	// Look up the AgentEntry from current config
...
// findAgentEntry looks up an AgentEntry by worktree name in the current config.
func (d *Daemon) findAgentEntry(name string) (AgentEntry, bool) {
	cfg := d.configSnapshot()
	for _, agent := range cfg.Agents {
		if agent.Worktree == name {
			return agent, true
