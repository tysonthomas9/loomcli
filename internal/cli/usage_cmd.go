...
func renderUsageTable(records []usage.SessionUsage, f usage.Filter) {
	...
	sb.WriteString(renderBoxLine(centerText("USAGE SUMMARY", dashboardWidth-4)))

	// Date range line
	dateRange := formatDateRange(f, records)
	sb.WriteString(renderBoxLine(centerText(dateRange, dashboardWidth-4)))

	// Totals section
	sb.WriteString(renderBoxSeparator())
	sb.WriteString(renderBoxLine(" TOTALS"))
	sb.WriteString(renderBoxLine(fmt.Sprintf("   Input tokens:  %s    Output tokens:    %s",
		padRight(formatTokenCount(agg.TotalInput), 12), formatTokenCount(agg.TotalOutput))))
	sb.WriteString(renderBoxLine(fmt.Sprintf("   Cache reads:   %s    Cache writes:     %s",
		padRight(formatTokenCount(agg.TotalCacheRead), 12), formatTokenCount(agg.TotalCacheWrite))))
	sb.WriteString(renderBoxLine(fmt.Sprintf("   Estimated cost:  %-12s  Sessions:  %d",
		formatCost(agg.TotalCost), agg.SessionCount)))

	// By Agent section
	sb.WriteString(renderBoxSeparator())
	sb.WriteString(renderBoxLine(" BY AGENT"))
	for _, a := range agg.ByAgent {
		line := fmt.Sprintf("   %-12s %s  %2d sessions   input: %s  output: %s",
			truncateToWidth(a.Name, 12),
			padRight(formatCost(a.Cost), 8),
			a.Sessions,
	...
	// By Backend section
	sb.WriteString(renderBoxSeparator())
	sb.WriteString(renderBoxLine(" BY BACKEND"))
	for _, b := range agg.ByBackend {
		line := fmt.Sprintf("   %-12s %s  %2d sessions",
			truncateToWidth(b.Name, 12),
			padRight(formatCost(b.Cost), 8),
			b.Sessions)
		sb.WriteString(renderBoxLine(line))
	}
	...
	if usageVerbose {
		sb.WriteString(renderBoxSeparator())
		sb.WriteString(renderBoxLine(" SESSIONS"))
		...
		for _, rec := range sorted {
			...
			line := fmt.Sprintf("   %s  %-10s %-8s %-12s %s  %s  exit:%d",
				...
				truncateToWidth(rec.AgentName, 10),
