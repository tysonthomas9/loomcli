...
// `loom recover <worktree>` (which now releases the fleet-db lock) or wait
...
func checkOrphanedFleetLocks(deps *cli.Deps) CheckResult {
	...
	if err != nil {
		// Don't fail doctor on listing errors — other checks already cover
		// backend availability. Skip silently.
		return CheckResult{}
	}
	if len(issues) == 0 {
		return CheckResult{}
	...
	return CheckResult{
		...
		Detail:  strings.Join(orphans, "\n") + "\nremediation: run `loom recover <worktree>` to release the fleet-db lock, or wait ~5min for TTL expiry.",
	}
}
