...
func createWorkspace(ctx context.Context, req service.WorkspaceCreateRequest) (service.WorkspaceCreateResult, error) {
	...
	err := config.WithConfigLock(func() error {
		...
		wsDir := req.Path
		if wsDir == "" {
			wsDir = config.GetWorkspaceDir(req.Name)
		}
		...
		switch req.Type {
		case "empty":
			result, err = createEmptyWorkspace(ctx, cfg, req.Name, wsDir, branch, req.Repos, config.SaveConfigUnlocked)
			return err
		case "clone":
			...
			result, err = createCloneWorkspace(ctx, cfg, req.Name, wsDir, cloneURLs, config.SaveConfigUnlocked)
			return err
		default:
