...
func startServePlatformRuntime(
	ctx context.Context,
	...
	startIssueJournalBridge(
		...
		callbacks.repositoryRequiredBlocker, issueJournalSource,
	)
	...
	executionRuntime, err := serveadapter.BuildExecutionRuntimeContributor(
		executionPasses, cfg.ExecutionCapability, driverAutomationWorkspaceScope(),
	...
	runtimeHost, err := serveadapter.BuildServeRuntimeHost(
		...
		storeHandle.Store.Workspaces(), runOutcomes, driverAutomationWorkspaceScope(),
		cfg.ExecutionCapability, executionRuntime, automationCapability,
	)
	if err != nil {
