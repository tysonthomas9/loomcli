...
func TestLegacyWorkflowTargetPreparationProjectsActivatedTarget(t *testing.T) {
	state := memstore.New()
	...
	factory := newLegacyWorkflowTargetPreparation(state)
	if factory == nil {
		t.Fatal("legacy preparation factory is nil")
	}
	...
	if _, err := prepare(t.Context(), "TEST", "missing"); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("missing workflow error = %v, want %v", err, domain.ErrNotFound)
	}
	if newLegacyWorkflowTargetPreparation(nil) != nil {
		t.Fatal("nil legacy catalog constructed a preparation factory")
	}
}
