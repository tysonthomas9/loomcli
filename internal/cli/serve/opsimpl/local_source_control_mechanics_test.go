...
func TestResolveAgentWorktreeForRepo_StoreBackedFleetDB(t *testing.T) {
	...
	if _, err := g.ResolveAgentWorktreeForRepo("WS1", "nova", "docs"); !errors.Is(err, sourcecontrol.ErrAgentRepoNotAllowed) {
		t.Fatalf("nova/docs err = %v, want ErrAgentRepoNotAllowed", err)
	}
	if _, err := g.ResolveAgentWorktreeForRepo("WS1", "nova", "missing"); !errors.Is(err, sourcecontrol.ErrAgentRepoNotAllowed) {
		t.Fatalf("nova/missing err = %v, want ErrAgentRepoNotAllowed", err)
	}
	if _, err := g.ResolveAgentWorktreeForRepo("WS1", "any", "api"); !errors.Is(err, sourcecontrol.ErrAgentWorktreeNotFound) {
...
func TestResolveAgentWorktree_BrokenGitMetadataReturnsUnknownBranch(t *testing.T) {
	...
	for name, resolve := range map[string]func() (*sourcecontrol.Worktree, error){
		...
		"ResolveAgentWorktreeForRepo": func() (*sourcecontrol.Worktree, error) {
...
func TestGitShowFileAtRevPreservesNotFoundKind(t *testing.T) {
	...
	_, err := NewLocalSourceControlMechanics().GitShowFileAtRev(context.Background(), repo, "HEAD", "untracked.txt", 1024)
