...
func TestResolveWorkspaceRoot_StoreBacked(t *testing.T) {
	...
	g := NewLocalSourceControlMechanics().WithWorkspaceProjection(testWorkspaceProjection{store: st})
...
func TestResolveWorkspaceRoot_EmptyID(t *testing.T) {
	if _, err := NewLocalSourceControlMechanics().ResolveWorkspaceRoot(""); err == nil {
