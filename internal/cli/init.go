...
This command helps new users set up loom agents:
  ...
  - Explains the loom workflow

Flags:
  -y, --yes              Non-interactive mode with defaults
  --worktrees-dir DIR    Override worktrees directory (default: from env or "worktrees")
  --names NAMES          Comma-separated worktree names for non-interactive mode

Examples:
  loom init                           # Interactive guided setup
  loom init --yes                     # Non-interactive with defaults (falcon, nova)
  loom init --yes --names="a,b,c"     # Create worktrees a, b, c`,
	Args: cobra.NoArgs,
	Run:  runInit,
}
