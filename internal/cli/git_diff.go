...
func ResolveMergeBase(worktreePath, branch string) (string, error) {
	...
	out, err := RunGitCommand(worktreePath, "merge-base", branch, "HEAD")
	if err != nil {
...
func DiffCommits(worktreePath, mergeBase string, limit int) ([]ops.DiffCommitResult, error) {
	...
	out, err := RunGitCommand(worktreePath, args...)
	if err != nil {
...
func DiffFiles(worktreePath, from, to string) ([]ops.DiffFileResult, error) {
	...
	// Get file statuses
	nameStatusOut, err := RunGitCommand(worktreePath, "diff", "--name-status", diffRange)
	if err != nil {
		return nil, fmt.Errorf("diff name-status: %w", err)
	}

	// Get line stats
	numstatOut, err := RunGitCommand(worktreePath, "diff", "--numstat", diffRange)
	if err != nil {
...
func DiffFilePatch(worktreePath, from, to, path string) (*ops.DiffFilePatchResult, error) {
	...
	numstatOut, err := RunGitCommand(worktreePath, "diff", "--numstat", diffRange, "--", path)
	if err != nil {
	...
	patchOut, err := RunGitCommand(worktreePath, "diff", diffRange, "--", path)
	if err != nil {
