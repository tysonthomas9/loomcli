...
func handleFileTree(svc FileService) http.HandlerFunc {
...
func handleFileRead(svc FileService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		agentName := r.PathValue("name")
		wsID := middleware.WorkspaceFromContext(r.Context())
		reqPath := r.URL.Query().Get("path")
...
func handleFileWrite(svc FileService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		agentName := r.PathValue("name")
		wsID := middleware.WorkspaceFromContext(r.Context())
		reqPath := r.URL.Query().Get("path")
