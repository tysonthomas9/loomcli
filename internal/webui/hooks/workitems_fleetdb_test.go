...
func newTestWorkItemsFleetDBHook(t *testing.T) *WorkItemsFleetDBHook {
	t.Helper()
	return NewWorkItemsFleetDBHook(testFleetURL, "test-key", "test-actor", slog.Default())
}

func TestWorkItemsFleetDBHook_Name(t *testing.T) {
	hook := newTestWorkItemsFleetDBHook(t)
	if got := hook.Name(); got != "workitems-fleetdb" {
		t.Errorf("Name() = %q, want %q", got, "workitems-fleetdb")
	}
}

func TestWorkItemsFleetDBHook_Critical(t *testing.T) {
	hook := newTestWorkItemsFleetDBHook(t)
	if hook.Critical() {
...
func TestWorkItemsFleetDBHook_OnRegister_CreatesBackend(t *testing.T) {
	hook := newTestWorkItemsFleetDBHook(t)
	...
	res, ok := ctx.Resolve(coordinator.ResourceKeyWorkItemsFleetDB)
	if !ok {
		t.Fatal("expected ResourceKeyWorkItemsFleetDB to be provided after OnRegister")
	}
...
func TestWorkItemsFleetDBHook_OnRegisterScopesBackendToRegisteredWorkspace(t *testing.T) {
	var gotPath string
	...
	hook := NewWorkItemsFleetDBHook(srv.URL, "", "test-actor", slog.Default())
...
func TestWorkItemsFleetDBHook_V2MutationWaitCarriesAPIKey(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("X-API-Key") != serviceCredential || r.Header.Get("X-Fleet-API-Key") != serviceCredential {
			t.Error("v2 mutation wait did not carry the WorkItemsFleetDBHook API key")
	...
	hook := NewWorkItemsFleetDBHook(server.URL, serviceCredential, "local-service", slog.Default())
	ctx := regCtx("SECURE", "/tmp/secure")
	if err := hook.OnRegister(ctx); err != nil {
		t.Fatalf("OnRegister: %v", err)
	}
	resource, ok := ctx.Resolve(coordinator.ResourceKeyWorkItemsFleetDB)
	if !ok {
		t.Fatal("expected Work Items FleetDB adapter resource")
	}
...
func TestWorkItemsFleetDBHook_EmptyWorkspaceRequiresExplicitWorkspace(t *testing.T) {
	hook := NewWorkItemsFleetDBHook(testFleetURL, "", "test-actor", slog.Default())
	ctx := regCtx("", "/tmp/demo")

	if err := hook.OnRegister(ctx); err == nil {
		t.Fatal("OnRegister returned nil, want missing workspace error")
	}
	if _, ok := ctx.Resolve(coordinator.ResourceKeyWorkItemsFleetDB); ok {
		t.Fatal("unexpected fleet backend resource for empty workspace")
	}
}

func TestWorkItemsFleetDBHook_OnRegister_Error(t *testing.T) {
	// Empty baseURL causes fleet.New to fail.
	hook := NewWorkItemsFleetDBHook("", "test-key", "test-actor", slog.Default())
	...
	if _, ok := ctx.Resolve(coordinator.ResourceKeyWorkItemsFleetDB); ok {
		t.Error("expected ResourceKeyWorkItemsFleetDB to NOT be provided after error")
	}
}

func TestWorkItemsFleetDBHook_OnDeregister_RunsCleanly(t *testing.T) {
	hook := newTestWorkItemsFleetDBHook(t)
...
func TestWorkItemsFleetDBHook_OnRollback_RunsCleanly(t *testing.T) {
	hook := newTestWorkItemsFleetDBHook(t)
...
func TestWorkItemsFleetDBHook_MultipleWorkspaces(t *testing.T) {
	hook := newTestWorkItemsFleetDBHook(t)
	...
	for _, ws := range workspaces {
		res, ok := ws.ctx.Resolve(coordinator.ResourceKeyWorkItemsFleetDB)
		if !ok {
	...
	for _, wsID := range []string{"ws-alpha", "ws-gamma"} {
		for _, ws := range workspaces {
			if ws.id == wsID {
				if _, ok := ws.ctx.Resolve(coordinator.ResourceKeyWorkItemsFleetDB); !ok {
