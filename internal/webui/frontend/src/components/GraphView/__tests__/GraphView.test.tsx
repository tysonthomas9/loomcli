...
vi.mock("@xyflow/react", () => ({
  ReactFlow: vi.fn(
    ({ children, onNodeClick, onNodeMouseEnter, onNodeMouseLeave, nodes }) => (
  ...
  GraphControls: vi.fn(
    ({
      ...
      <div
        ...
        data-dep-type-filter={
          dependencyTypeFilter ? JSON.stringify([...dependencyTypeFilter]) : ""
        }
        className={className}
      >
        <input
...
describe("GraphView", () => {
  ...
  describe("show closed filtering", () => {
    ...
    it("filters out closed issues when showClosed is false", () => {
      ...
      const closedIssue = callArgs.find(
        (issue: Issue) => issue.id === "issue-closed",
      );
    ...
    it("updates filtering when show closed toggle is clicked", () => {
      ...
      const openOnlyNodes = createTestNodes(
        issues.filter((i) => i.status !== "closed"),
      );
      ...
      const closedIssueInLastCall = lastCall[0].find(
        (issue: Issue) => issue.id === "issue-closed",
      );
  ...
  describe("node click callback", () => {
    it("calls onNodeClick with correct issue when node is clicked", () => {
      const issues = [
        createTestIssue({ id: "issue-click", title: "Clickable Issue" }),
      ];
      const nodes = createTestNodes(issues);
    ...
    it("does not call onNodeClick when callback is not provided", () => {
      const issues = [
        createTestIssue({ id: "issue-no-click", title: "Non-clickable" }),
      ];
      const nodes = createTestNodes(issues);
  ...
  describe("tooltip integration", () => {
    it("shows NodeTooltip component on node mouse enter", () => {
      const issues = [
        createTestIssue({ id: "issue-hover", title: "Hover Issue" }),
      ];
      const nodes = createTestNodes(issues);
    ...
    it("hides NodeTooltip on node mouse leave", () => {
      const issues = [
        createTestIssue({ id: "issue-leave", title: "Leave Issue" }),
      ];
      const nodes = createTestNodes(issues);
    ...
    it("passes correct issue and position to NodeTooltip", () => {
      const issues = [
        createTestIssue({ id: "issue-position", title: "Position Issue" }),
      ];
      const nodes = createTestNodes(issues);
    ...
    it("still calls external onNodeMouseEnter callback when provided", () => {
      const issues = [
        createTestIssue({ id: "issue-callback", title: "Callback Issue" }),
      ];
      const nodes = createTestNodes(issues);
  ...
  describe("show closed toggle", () => {
    ...
    it("filters out closed issues when showClosed is false", () => {
      ...
      expect(callArgs.some((issue: Issue) => issue.id === "closed-1")).toBe(
        false,
      );
    });
  ...
  describe("status filter integration", () => {
    ...
    it('filters to only closed issues when statusFilter is "closed"', () => {
      ...
      const callArgs = (useGraphData as Mock).mock.calls[0][0];
      expect(callArgs).toHaveLength(2);
      expect(callArgs.every((issue: Issue) => issue.status === "closed")).toBe(
        true,
      );
    });
  ...
  describe("dependency type filter integration", () => {
    ...
    it("persists dependency type filter to localStorage when changed", () => {
      ...
      const nonBlockingCheckbox = screen.getByTestId(
        "dep-type-non-blocking-checkbox",
      );
    ...
    it("updates includeDependencyTypes when filter is toggled", () => {
      ...
      const parentChildCheckbox = screen.getByTestId(
        "dep-type-parent-child-checkbox",
      );
    ...
    it("checkbox reflects correct checked state for blocking", () => {
      ...
      expect(
        screen.getByTestId("dep-type-parent-child-checkbox"),
      ).not.toBeChecked();
      expect(
        screen.getByTestId("dep-type-non-blocking-checkbox"),
      ).not.toBeChecked();
    });
    ...
    it("checkbox reflects correct checked state when none selected", () => {
      ...
      expect(
        screen.getByTestId("dep-type-blocking-checkbox"),
      ).not.toBeChecked();
      expect(
        screen.getByTestId("dep-type-parent-child-checkbox"),
      ).not.toBeChecked();
