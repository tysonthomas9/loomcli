...
describe("SwimLane", () => {
  ...
  describe("rendering", () => {
    ...
    it("renders all status columns", () => {
      ...
      expect(
        screen.getByRole("region", { name: "Closed issues" }),
      ).toBeInTheDocument();
      expect(
  ...
  describe("backlog column (Pending→Backlog rename)", () => {
    it('passes columnType="backlog" to StatusColumn for backlog column', () => {
      const columns: KanbanColumnConfig[] = [
        ...columnsFromStatuses(["open", "in_progress"]),
        {
          ...
          label: "Backlog",
          filter: (issue: Issue) =>
            issue.status === "blocked" || issue.status === "deferred",
      ...
      const backlogSection = container.querySelector(
        '[data-column-type="backlog"]',
      );
    ...
    it("passes isBacklog=true to DraggableIssueCard in backlog column", () => {
      ...
      const card = screen
        .getByText("Blocked Issue")
        .closest('[data-in-backlog="true"]');
  ...
  describe("card limiting", () => {
    ...
    it("handles multiple columns with different issue counts independently", () => {
      ...
      expect(
        within(progressColumn).queryByText(/Show all/),
      ).not.toBeInTheDocument();
