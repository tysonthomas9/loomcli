...
import type { LogChunk } from "@/hooks/terminal";
...
describe("LogViewer", () => {
  ...
  it("writes new chunks incrementally", () => {
    ...
    const { rerender } = render(
      <LogViewer chunks={first} connectionState="connected" />,
    );
    ...
    expect(lastTerminalInstance?.write).toHaveBeenNthCalledWith(
      2,
      new TextEncoder().encode("\rdef"),
      expect.any(Function),
    );
  });
  ...
  it("resets terminal when resetVersion changes", () => {
    ...
    rerender(
      <LogViewer chunks={[]} connectionState="connected" resetVersion={1} />,
    );
  ...
  it("loads fit addon and fits terminal", () => {
    render(<LogViewer chunks={[]} connectionState="connected" />);
    expect(lastTerminalInstance?.loadAddon).toHaveBeenCalledWith(
      lastFitAddonInstance,
    );
  ...
  it("keeps loading banner mounted and only toggles visibility", () => {
    ...
    rerender(
      <LogViewer chunks={[]} connectionState="connected" isLoadingMore />,
    );
