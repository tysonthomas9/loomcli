...
describe("AssigneePrompt", () => {
  ...
  describe("Rendering", () => {
    ...
    it("renders title and subtitle", () => {
      ...
      expect(screen.getByText("Who is working on this?")).toBeInTheDocument();
      expect(
        screen.getByText("Enter your name to claim this task"),
      ).toBeInTheDocument();
    });
    ...
    it("renders Assign and Skip buttons", () => {
      ...
      expect(screen.getByTestId("assignee-confirm-button")).toHaveTextContent(
        "Assign",
      );
      expect(screen.getByTestId("assignee-skip-button")).toHaveTextContent(
        "Skip",
      );
    });
    ...
    it('renders "Or enter a new name" label when recent names exist', () => {
      render(
        <AssigneePrompt {...defaultProps} recentNames={["Alice", "Bob"]} />,
      );
  ...
  describe("Recent names", () => {
    it("shows recent names dropdown when recentNames is not empty", () => {
      ...
      expect(screen.getByTestId("recent-name-Alice")).toHaveTextContent(
        "Alice",
      );
      expect(screen.getByTestId("recent-name-Bob")).toHaveTextContent("Bob");
      expect(screen.getByTestId("recent-name-Charlie")).toHaveTextContent(
        "Charlie",
      );
    });
  ...
  describe("Skip behavior", () => {
    ...
    it("does not call onSkip on Escape when closed", () => {
      const onSkip = vi.fn();
      render(
        <AssigneePrompt {...defaultProps} isOpen={false} onSkip={onSkip} />,
      );
  ...
  describe("Focus behavior", () => {
    it("focuses input when modal opens", () => {
      ...
      expect(document.activeElement).toBe(
        screen.getByTestId("assignee-name-input"),
      );
    });

    it("resets input value when modal opens", () => {
      const { rerender } = render(
        <AssigneePrompt {...defaultProps} isOpen={true} />,
      );
  ...
  describe("Edge cases", () => {
    ...
    it("cleans up event listeners on unmount", () => {
      ...
      const { unmount } = render(
        <AssigneePrompt {...defaultProps} onSkip={onSkip} />,
      );
  ...
  describe("Props changes", () => {
    it("handles isOpen changing from false to true", () => {
      const { rerender } = render(
        <AssigneePrompt {...defaultProps} isOpen={false} />,
      );
    ...
    it("handles recentNames prop changes", () => {
      const { rerender } = render(
        <AssigneePrompt {...defaultProps} recentNames={["Alice"]} />,
      );
