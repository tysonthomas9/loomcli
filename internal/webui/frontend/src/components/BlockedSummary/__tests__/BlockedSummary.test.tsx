...
describe("BlockedSummary", () => {
  ...
  describe("rendering", () => {
    ...
    it("renders badge with non-zero count", () => {
      setupMocks({
        data: [createBlockedIssue(), createBlockedIssue({ id: "issue-2" })],
      });
      render(<BlockedSummary />);
  ...
  describe("issue click", () => {
    it("calls onIssueClick with correct ID when issue is clicked", () => {
      ...
      // Click on issue
      const issueItem = screen.getByRole("menuitem", {
        name: /clicked-issue/i,
      });
    ...
    it('calls onIssueClick with "__show_all_blocked__" when footer is clicked', () => {
      ...
      const footerButton = screen.getByRole("menuitem", {
        name: /show all blocked/i,
      });
  ...
  describe("error state", () => {
    it("shows error message when error occurs", () => {
      ...
      expect(
        screen.getByText("Failed to load blocked issues"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("handles very long issue IDs", () => {
      const longId =
        "very-long-issue-id-that-might-cause-display-issues-12345678";
