...
describe("RedirectToWorkspace", () => {
  describe("happy path — default workspace redirect", () => {
    it("navigates to the default workspace when no lastId is stored", async () => {
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith("/ws/ws-default/", {
          replace: true,
        });
      });
    });
    ...
    it("navigates to the first workspace when no default is set", async () => {
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith("/ws/ws-first/", {
          replace: true,
        });
      });
    });
  ...
  describe("stale localStorage — valid default present", () => {
    ...
    it("navigates directly to lastId when it exists in the workspace list", async () => {
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith("/ws/ws-known/", {
          replace: true,
        });
  ...
  describe("failedWorkspaceId filtering — redirect-loop fix", () => {
    it("excludes failedWorkspaceId and navigates to the remaining workspace", async () => {
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith("/ws/ws-2/", {
          replace: true,
        });
  ...
  describe("no failedWorkspaceId in state", () => {
    it("uses the full workspace list when location.state is null", async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockNavigate).toHaveBeenCalledWith("/ws/ws-b/", {
          replace: true,
        });
      });
    });
    ...
    it("uses the full workspace list when location.state has no failedWorkspaceId key", async () => {
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith("/ws/ws-only/", {
          replace: true,
        });
      });
    });
