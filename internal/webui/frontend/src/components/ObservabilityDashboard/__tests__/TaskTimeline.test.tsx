...
describe('TaskTimeline', () => {
  describe('rendering bars', () => {
    ...
    it('does not render failed segment when failed is 0', () => {
      const buckets = [
        createBucket({ hour: '2026-03-05T10:00:00Z', completed: 10, failed: 0 }),
      ];
  ...
  describe('empty state', () => {
    it('shows empty state when hourlyCompletions is empty', () => {
      ...
      expect(screen.getByText('No task completions in the last 24 hours')).toBeInTheDocument();
    });
  ...
  describe('hour labels', () => {
    it('shows hour labels every 3rd column (i % 3 === 0)', () => {
      ...
      const { container } = render(<TaskTimeline hourlyCompletions={buckets} />);
  ...
  describe('bar height calculation', () => {
    it('renders bar heights proportional to max value', () => {
      ...
      const completedBars = container.querySelectorAll('[class*="barCompleted"]');
