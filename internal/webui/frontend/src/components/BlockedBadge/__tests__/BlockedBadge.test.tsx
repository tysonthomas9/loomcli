...
describe("BlockedBadge", () => {
  ...
  describe("tooltip behavior", () => {
    ...
    it("tooltip has correct header for blocks variant", () => {
      render(
        <BlockedBadge count={1} issueIds={["blocked-id"]} variant="blocks" />,
      );
  ...
  describe("edge cases", () => {
    ...
    it("handles very large count", () => {
      ...
      expect(
        screen.getByLabelText("Blocked by 999 issues"),
      ).toBeInTheDocument();
    });

    it("handles long issue IDs in tooltip", () => {
      const longId =
        "very-long-blocker-id-that-might-cause-display-issues-12345678";
  ...
  describe("issueDetails prop", () => {
    it('shows "id: title" format in tooltip when issueDetails is provided', () => {
      ...
      expect(
        screen.getByText("ISSUE-2: Update dependencies"),
      ).toBeInTheDocument();
    });

    it("truncates titles longer than 45 characters with ellipsis", () => {
      const longTitle =
        "This is a very long issue title that exceeds the forty-five character limit";
      ...
      expect(
        screen.queryByText(`ISSUE-1: ${longTitle}`),
      ).not.toBeInTheDocument();
    });

    it("falls back to showing just the ID when issueDetails has an empty title", () => {
      const details: BlockerRef[] = [{ id: "ISSUE-1", title: "", priority: 1 }];
      render(
        <BlockedBadge
          count={1}
          issueIds={["ISSUE-1"]}
    ...
    it('shows first 5 items with "and N more..." when issueDetails has >5 items', () => {
      ...
      render(
        <BlockedBadge count={8} issueIds={issueIds} issueDetails={details} />,
      );
      ...
      for (let i = 1; i <= 5; i++) {
        expect(
          screen.getByText(`ISSUE-${i}: Task number ${i}`),
        ).toBeInTheDocument();
      }
