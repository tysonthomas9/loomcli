...
describe("ProjectHealthPanel", () => {
  ...
  describe("bottleneck detection", () => {
    it("detects bottlenecks from issues blocking multiple others", () => {
      const blockedIssues = [
        createBlockedIssue({
          id: "blocked-1",
          blocked_by: ["bottleneck-1", "other-1"],
        }),
        createBlockedIssue({ id: "blocked-2", blocked_by: ["bottleneck-1"] }),
        createBlockedIssue({
          id: "blocked-3",
          blocked_by: ["bottleneck-1", "bottleneck-2"],
        }),
    ...
    it("sorts bottlenecks by blocking count descending", () => {
      const blockedIssues = [
        ...
        createBlockedIssue({
          id: "blocked-2",
          blocked_by: ["small-blocker", "big-blocker"],
        }),
  ...
  describe("loading state", () => {
    ...
    it('does not show "No bottlenecks detected" while loading', () => {
      ...
      expect(
        screen.queryByText("No bottlenecks detected"),
  ...
  describe("edge cases", () => {
    ...
    it("handles issue blocked by multiple blockers where only one is a bottleneck", () => {
      const blockedIssues = [
        createBlockedIssue({
          id: "blocked-1",
          blocked_by: ["bottleneck", "single-1"],
        }),
        createBlockedIssue({
          id: "blocked-2",
          blocked_by: ["bottleneck", "single-2"],
        }),
      ];
    ...
    it("uses blocked_by_details title when available", () => {
      const blockedIssues = [
        createBlockedIssue({
          ...
          blocked_by: ["bn-1"],
          blocked_by_details: [
            { id: "bn-1", title: "Critical API Bug", priority: 0 },
          ],
        }),
        createBlockedIssue({
          id: "blocked-2",
    ...
    it("handles blocker with empty title by using ID as fallback", () => {
      const blockedIssues = [
        createBlockedIssue({
          ...
          blocked_by_details: [
            { id: "bn-empty-title", title: "", priority: 2 },
          ],
        }),
        createBlockedIssue({
          id: "blocked-2",
