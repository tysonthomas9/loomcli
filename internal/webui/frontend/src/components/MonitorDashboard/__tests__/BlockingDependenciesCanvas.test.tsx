...
describe("BlockingDependenciesCanvas", () => {
  ...
  describe("empty state", () => {
    ...
    it('renders with testid "blocking-dependencies-canvas"', () => {
      ...
      expect(
        screen.getByTestId("blocking-dependencies-canvas"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("interactions", () => {
    ...
    it("calls onExpandClick when expand button is clicked", () => {
      ...
      const expandButton = screen.getByRole("button", {
        name: /expand to full graph view/i,
      });
  ...
  describe("props", () => {
    it("applies custom className", () => {
      render(
        <BlockingDependenciesCanvas issues={[]} className="custom-class" />,
      );
  ...
  describe("filtering", () => {
    it("excludes closed issues from visualization", () => {
      ...
      expect(
        callArgs.some((issue: Issue) => issue.id === "in-progress-1"),
      ).toBe(true);
  ...
  describe("edge cases", () => {
    it("handles node click when node data has no issue", () => {
      ...
      const onNodeClick = vi.fn();
      render(
        <BlockingDependenciesCanvas issues={[]} onNodeClick={onNodeClick} />,
      );
