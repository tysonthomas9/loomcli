...
describe("TypeDropdown", () => {
  ...
  describe("Dropdown behavior", () => {
    ...
    it("renders all 5 type options in menu", () => {
      ...
      expect(screen.getByTestId("type-option-feature")).toHaveTextContent(
        "Feature",
      );
      ...
      expect(screen.getByTestId("type-option-chore")).toHaveTextContent(
        "Chore",
      );
    });
  ...
  describe("Error handling", () => {
    ...
    it("displays error message on save failure", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("type-error")).toHaveTextContent(
          "Network error",
        );
      });
    });
  ...
  describe("Keyboard navigation", () => {
    ...
    it("navigates down with ArrowDown key", () => {
      ...
      expect(screen.getByTestId("type-option-bug")).toHaveAttribute(
        "data-focused",
        "true",
      );

      fireEvent.keyDown(trigger, { key: "ArrowDown" });
      expect(screen.getByTestId("type-option-feature")).toHaveAttribute(
        "data-focused",
        "true",
      );

      fireEvent.keyDown(trigger, { key: "ArrowDown" });
      expect(screen.getByTestId("type-option-task")).toHaveAttribute(
        "data-focused",
        "true",
      );
    });
    ...
    it("navigates up with ArrowUp key", () => {
      ...
      expect(screen.getByTestId("type-option-chore")).toHaveAttribute(
        "data-focused",
        "true",
      );

      fireEvent.keyDown(trigger, { key: "ArrowUp" });
      expect(screen.getByTestId("type-option-epic")).toHaveAttribute(
        "data-focused",
        "true",
      );
  ...
  describe("Props sync", () => {
    it("syncs optimistic type when prop changes", () => {
      const { rerender } = render(
        <TypeDropdown {...defaultProps} type="task" />,
      );
      expect(screen.getByTestId("type-dropdown-trigger")).toHaveTextContent(
        "Task",
      );
    ...
    it("updates checkmark when type prop changes", () => {
      ...
      expect(screen.getByTestId("type-option-task")).not.toHaveAttribute(
        "data-selected",
      );
    });
  ...
  describe("Edge cases", () => {
    it("handles rapid type changes", () => {
      const { rerender } = render(
        <TypeDropdown {...defaultProps} type="bug" />,
      );
    ...
    it("handles custom/unknown type strings", () => {
      render(
        <TypeDropdown {...defaultProps} type={"custom_type" as IssueType} />,
      );
      const trigger = screen.getByTestId("type-dropdown-trigger");
