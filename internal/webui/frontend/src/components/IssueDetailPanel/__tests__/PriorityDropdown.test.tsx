...
describe("PriorityDropdown", () => {
  ...
  describe("Dropdown behavior", () => {
    ...
    it("renders all 5 priority options in menu", () => {
      ...
      expect(screen.getByTestId("priority-option-0")).toHaveTextContent(
        "P0 - Critical",
      );
      expect(screen.getByTestId("priority-option-1")).toHaveTextContent(
        "P1 - High",
      );
      expect(screen.getByTestId("priority-option-2")).toHaveTextContent(
        "P2 - Medium",
      );
      expect(screen.getByTestId("priority-option-3")).toHaveTextContent(
        "P3 - Normal",
      );
      expect(screen.getByTestId("priority-option-4")).toHaveTextContent(
        "P4 - Backlog",
      );
    });
  ...
  describe("Selection", () => {
    ...
    it("calls onSave for each different priority value", async () => {
      ...
      for (const targetPriority of priorities) {
        ...
        await act(async () => {
          fireEvent.click(
            screen.getByTestId(`priority-option-${targetPriority}`),
          );
        });
  ...
  describe("Error handling", () => {
    it("reverts to previous priority on save failure", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId("priority-dropdown-trigger"),
        ).toHaveTextContent("P2 - Medium");
      });
    });
  ...
  describe("Keyboard navigation", () => {
    ...
    it("navigates down with ArrowDown key", () => {
      ...
      expect(screen.getByTestId("priority-option-0")).toHaveAttribute(
        "data-focused",
        "true",
      );

      fireEvent.keyDown(trigger, { key: "ArrowDown" });
      expect(screen.getByTestId("priority-option-1")).toHaveAttribute(
        "data-focused",
        "true",
      );

      fireEvent.keyDown(trigger, { key: "ArrowDown" });
      expect(screen.getByTestId("priority-option-2")).toHaveAttribute(
        "data-focused",
        "true",
      );
    });
    ...
    it("navigates up with ArrowUp key", () => {
      ...
      expect(screen.getByTestId("priority-option-4")).toHaveAttribute(
        "data-focused",
        "true",
      );

      fireEvent.keyDown(trigger, { key: "ArrowUp" });
      expect(screen.getByTestId("priority-option-3")).toHaveAttribute(
        "data-focused",
        "true",
      );
  ...
  describe("Props sync", () => {
    it("syncs optimistic priority when prop changes", () => {
      const { rerender } = render(
        <PriorityDropdown {...defaultProps} priority={2} />,
      );
      expect(screen.getByTestId("priority-dropdown-trigger")).toHaveTextContent(
        "P2 - Medium",
      );
    ...
    it("updates checkmark when priority prop changes", () => {
      ...
      expect(screen.getByTestId("priority-option-2")).not.toHaveAttribute(
        "data-selected",
      );
    });
  ...
  describe("Edge cases", () => {
    it("handles rapid priority changes", () => {
      const { rerender } = render(
        <PriorityDropdown {...defaultProps} priority={0} />,
      );
