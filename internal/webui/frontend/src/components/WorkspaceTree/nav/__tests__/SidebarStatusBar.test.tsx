...
describe("SidebarStatusBar", () => {
  ...
  it("renders correct counts for mixed agent statuses", () => {
    const agents = [
      makeAgent("a1", "working: loom-123 (5m)"),
      makeAgent("a2", "working: loom-456 (2m)"),
      makeAgent("a3", "review: loom-789 (1m)"),
      makeAgent("a4", "ready"),
      makeAgent("a5", "idle"),
      makeAgent("a6", "done: loom-111 (10m)"),
    ];
  ...
  it("categorizes planning as working", () => {
    const agents = [
      makeAgent("a1", "planning: loom-123 (3m)"),
      makeAgent("a2", "ready"),
    ];
  ...
  it("categorizes error, dirty, and changes as idle", () => {
    const agents = [
      makeAgent("a1", "error: loom-123 (1m)"),
      makeAgent("a2", "dirty"),
  ...
  it("updates counts when agents prop changes", () => {
    const agents1 = [makeAgent("a1", "working: loom-123 (5m)")];
    const { rerender } = render(<SidebarStatusBar agents={agents1} />);
    expect(screen.getByText(/1 working/)).toBeInTheDocument();

    const agents2 = [
      makeAgent("a1", "working: loom-123 (5m)"),
      makeAgent("a2", "review: loom-456 (2m)"),
    ];
