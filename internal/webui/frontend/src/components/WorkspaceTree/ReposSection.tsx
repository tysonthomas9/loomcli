 ...
 * Shows repo name, branch, and whether a local checkout is registered.
 */
  ...
  return (
    <div className={styles.section}>
      ...
      <div className={styles.list}>
        {repos.map((repo) => {
          const hasLocalCheckout = repo.path.trim().length > 0;
          const title = hasLocalCheckout
            ? repo.path
            : `No local checkout path registered for ${repo.name}`;

          return (
            <div key={repo.name} className={styles.row} title={title}>
              <svg
