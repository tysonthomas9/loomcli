...
describe("columnConfigs", () => {
  ...
  describe("Column style configuration", () => {
    ...
    it('all non-backlog/blocked columns have style "normal"', () => {
      const actionable = DEFAULT_COLUMNS.filter(
        (c) => c.id !== "backlog" && c.id !== "blocked",
      );
  ...
  describe("createColumns()", () => {
    ...
    describe("no arguments (default behavior)", () => {
      ...
      it("still allows non-epic issues through all columns", () => {
        const columns = createColumns();
        const taskOpen = createMockIssue({
          issue_type: "task",
          status: "open",
        });
        const taskDeferred = createMockIssue({
          issue_type: "task",
          status: "deferred",
        });
        const taskBlocked = createMockIssue({
          issue_type: "task",
          status: "open",
        });
        const taskInProgress = createMockIssue({
          issue_type: "task",
          status: "in_progress",
        });
        const taskReview = createMockIssue({
          issue_type: "task",
          status: "review",
        });
        const taskClosed = createMockIssue({
          issue_type: "task",
          status: "closed",
        });

        expect(
          getColumnFrom(columns, "ready").filter(taskOpen, notBlocked),
        ).toBe(true);
        expect(
          getColumnFrom(columns, "backlog").filter(taskDeferred, notBlocked),
        ).toBe(true);
        expect(
          getColumnFrom(columns, "blocked").filter(taskBlocked, blocked),
        ).toBe(true);
        expect(
          getColumnFrom(columns, "in_progress").filter(
            taskInProgress,
            notBlocked,
          ),
        ).toBe(true);
        expect(
          getColumnFrom(columns, "review").filter(taskReview, notBlocked),
        ).toBe(true);
        expect(
          getColumnFrom(columns, "done").filter(taskClosed, notBlocked),
        ).toBe(true);
      });
    });
    ...
    describe("with includeEpics: true", () => {
      ...
      it("still respects other filter logic (status matching)", () => {
        ...
        const epicOpen = createMockIssue({
          issue_type: "epic",
          status: "open",
        });
        expect(
          getColumnFrom(columns, "backlog").filter(epicOpen, notBlocked),
        ).toBe(false);
      });
    ...
    describe("column structure consistency", () => {
      ...
      it("returns columns with the same IDs regardless of includeEpics", () => {
        ...
        const withEpicsIds = createColumns({ includeEpics: true }).map(
          (c) => c.id,
        );
        const withoutEpicsIds = createColumns({ includeEpics: false }).map(
          (c) => c.id,
        );
  ...
  describe("DEFAULT_COLUMNS backward compatibility", () => {
    it("DEFAULT_COLUMNS is defined as createColumns() with no args", () => {
      ...
      expect(DEFAULT_COLUMNS.map((c) => c.id)).toEqual(
        freshDefault.map((c) => c.id),
      );
      expect(DEFAULT_COLUMNS.map((c) => c.label)).toEqual(
        freshDefault.map((c) => c.label),
      );
    });

    it("DEFAULT_COLUMNS filters epics from all columns except Done", () => {
      const epicOpen = createMockIssue({ issue_type: "epic", status: "open" });
      const epicDeferred = createMockIssue({
        issue_type: "epic",
        status: "deferred",
      });
      const epicInProgress = createMockIssue({
        issue_type: "epic",
        status: "in_progress",
      });
      const epicReview = createMockIssue({
        issue_type: "epic",
        status: "review",
      });
      const epicClosed = createMockIssue({
        issue_type: "epic",
        status: "closed",
      });
      ...
      expect(getColumn("in_progress").filter(epicInProgress, notBlocked)).toBe(
        false,
      );
