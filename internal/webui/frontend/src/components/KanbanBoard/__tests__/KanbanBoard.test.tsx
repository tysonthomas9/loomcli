...
describe("KanbanBoard", () => {
  ...
  describe("rendering", () => {
    ...
    it("renders custom status columns", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Deferred" }),
      ).toBeInTheDocument();
      expect(screen.getByRole("heading", { name: "Open" })).toBeInTheDocument();
      ...
      expect(
        screen.queryByRole("heading", { name: "Closed" }),
      ).not.toBeInTheDocument();
    });
    ...
    it("renders correct issue count in each column", () => {
      ...
      expect(
        within(inProgressColumn).getByLabelText("1 issue"),
      ).toBeInTheDocument();
      ...
      expect(
        within(closedColumn).getByLabelText("2 issues"),
      ).toBeInTheDocument();
    });
    ...
    it("groups issues by status", () => {
      ...
      expect(
        within(progressColumn).getByText("Progress Issue 1"),
      ).toBeInTheDocument();
      ...
      expect(
        within(closedColumn).getByText("Closed Issue 1"),
      ).toBeInTheDocument();
    });
  ...
  describe("DndContext", () => {
    it("DndContext provider wraps columns", () => {
      ...
      expect(
        document.querySelector('[data-droppable-id="open"]'),
      ).toBeInTheDocument();
      expect(
        document.querySelector('[data-droppable-id="in_progress"]'),
      ).toBeInTheDocument();
      expect(
        document.querySelector('[data-droppable-id="closed"]'),
      ).toBeInTheDocument();
    });
    ...
    it("sensors are configured for drag-and-drop", () => {
      ...
      const draggableWrapper = document.querySelector(
  ...
  describe("edge cases", () => {
    ...
    it("issues without status default to open", () => {
      ...
      const openColumn = screen.getByRole("region", { name: "Open issues" });
      expect(
        within(openColumn).getByText("No Status Issue"),
      ).toBeInTheDocument();
    ...
    it("issues with unknown status are not displayed if status not in columns", () => {
      ...
      expect(
        screen.queryByText("Unknown Status Issue"),
      ).not.toBeInTheDocument();
    ...
    it("handles large number of issues", () => {
      ...
      expect(
        within(openColumn).getByLabelText("34 issues"),
      ).toBeInTheDocument();
      expect(
        within(progressColumn).getByLabelText("33 issues"),
      ).toBeInTheDocument();
      expect(
        within(closedColumn).getByLabelText("33 issues"),
      ).toBeInTheDocument();
    });
    ...
    it("handles status undefined fallback correctly", () => {
      ...
      const openColumn = screen.getByRole("region", { name: "Open issues" });
      expect(
        within(openColumn).getByText("Undefined Status"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("filtering", () => {
    ...
    it("updates when filters change", () => {
      ...
      // Apply filter
      rerender(
        <KanbanBoard issues={issues} filters={{ type: "bug" as IssueType }} />,
      );
    ...
    it("handles issues with undefined optional fields", () => {
      ...
      render(
        <KanbanBoard issues={issuesWithMissingFields} filters={filters} />,
      );

      expect(screen.getByText("Issue With Labels")).toBeInTheDocument();
      expect(
        screen.queryByText("Issue Without Labels"),
      ).not.toBeInTheDocument();
    });
    ...
    it("handles issues with missing labels when label filter applied", () => {
      ...
      expect(
        screen.queryByText("Issue With Empty Labels"),
      ).not.toBeInTheDocument();
    });
    ...
    it("column counts reflect filtered issues", () => {
      ...
      expect(
        within(progressColumn).getByLabelText("1 issue"),
      ).toBeInTheDocument();
    ...
    it("filters work with all four filter types combined", () => {
      ...
      expect(
        screen.queryByText("Perfect Match But Wrong Priority"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("Perfect Match But Wrong Type"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("Perfect Match But Wrong Labels"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("Something Else Entirely"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe("blocked issues filtering", () => {
    ...
    function createBlockedIssuesMap(
      ...
      const map = new Map<
        string,
        { blockedByCount: number; blockedBy: string[] }
      >();
    ...
    it("hides multiple blocked issues when showBlocked=false", () => {
      ...
      const blockedIssues = createBlockedIssuesMap([
        "blocked-1",
        "blocked-2",
        "blocked-3",
      ]);
  ...
  describe("Done column display limiting", () => {
    ...
    it("Done column count header shows total count, not truncated count", () => {
      ...
      const doneColumn = screen.getByRole("region", { name: "Done issues" });
      expect(
        within(doneColumn).getByLabelText("15 issues"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("blockedIssues prop passed to cards", () => {
    ...
    it("passes correct blockedBy array to BlockedBadge", () => {
      ...
      >([
        [
          "blocked-issue",
          { blockedByCount: 2, blockedBy: ["blocker-abc", "blocker-xyz"] },
        ],
      ]);
