...
import { useWorkspaceOnboarding } from "@/app/onboarding/useWorkspaceOnboarding";
...
function App() {
  ...
  const openOnboardingAgentCreator = useCallback(() => {
  ...
  const onboarding = useWorkspaceOnboarding({
    ...
    hasOnboardingRepository: hasOnboardingRepo,
    issueCount: issues.length,
    agentTaskCount: agentStats?.total ?? 0,
    backends: aiBackends,
    defaultBackend: onboardingBackendConfig?.backend,
    backendsLoading: aiBackendsLoading,
    backendConfigLoading: onboardingBackendConfigLoading,
    backendsError: aiBackendsError,
    savingDefaultBackend: isSavingOnboardingBackend,
    updateDefaultBackend: updateOnboardingBackend,
    refetchBackends: refetchAiBackends,
    refetchIssues: refetch,
    refetchWorkspace,
    closeAllPanels,
    navigateToView,
    showToast,
    openAgentCreator: openOnboardingAgentCreator,
  });
  const shouldShowWorkspaceOnboarding = onboarding.shouldShow;
  const shouldPrefillOnboardingAgent =
    hasOnboardingRepo && !onboarding.hasPlanner;
  const workspaceOnboardingSteps = onboarding.steps;
  const handleOnboardingDismiss = onboarding.dismiss;
      ...
      <CreateAgentModal
        ...
        onSuccess={(agent) => {
          ...
          if (
            ...
            onboarding.beginAgentConfirmation();
            void (async () => {
              try {
                ...
                if (mountedRef.current) {
                  onboarding.finishAgentConfirmation();
                }
              }
            })();
          } else {
