...
export async function fetchObservabilityMetrics(): Promise<MetricsSnapshot> {
  const response = await fetchWithTimeout(`${LOOM_SERVER_URL}/api/observability/metrics`, {
    method: 'GET',
    headers: buildLoomHeaders(),
  });

  if (!response.ok) {
    throw new Error(`Observability metrics: ${response.status} ${response.statusText}`);
  }
