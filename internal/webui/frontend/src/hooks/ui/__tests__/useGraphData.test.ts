...
describe("useGraphData", () => {
  ...
  describe("Single node", () => {
    ...
    it("builds issueIdToNodeId map correctly", () => {
      ...
      expect(result.current.issueIdToNodeId.get("my-issue")).toBe(
        "node-my-issue",
      );
    });
  });
  ...
  describe("Linear chain A -> B -> C", () => {
    ...
    it("edges have correct source and target", () => {
      ...
      const edgeAB = result.current.edges.find(
        (e) => e.id === "edge-A-B-blocks",
      );
      const edgeBC = result.current.edges.find(
        (e) => e.id === "edge-B-C-blocks",
      );
  ...
  describe("Counts accuracy", () => {
    it("dependencyCount matches outgoing edges", () => {
      ...
      const nodeA = result.current.nodes.find((n) => n.id === "node-A");
      const outgoingEdges = result.current.edges.filter(
        (e) => e.source === "node-A",
      );
    ...
    it("dependentCount matches incoming edges", () => {
      ...
      const incomingEdges = result.current.edges.filter(
        (e) => e.target === "node-D",
      );
    ...
    it("totalDependencies equals number of edges", () => {
      ...
      expect(result.current.totalDependencies).toBe(
        result.current.edges.length,
      );
      expect(result.current.totalDependencies).toBe(3);
    });
  });
  ...
  describe("Edge cases", () => {
    ...
    it("handles large graph with many nodes", () => {
      ...
      for (let i = 0; i < 100; i++) {
        const deps =
          i > 0 ? [createDependency(`issue-${i}`, `issue-${i - 1}`)] : [];
  ...
  describe("isReady computation", () => {
    ...
    it("correctly computes isReady for multiple issues", () => {
      ...
      const inProgressNotBlocked = createTestIssue({
        id: "D",
        status: "in_progress",
      });
  ...
  describe("isClosed flag", () => {
    ...
    it("correctly sets isClosed for multiple issues with mixed statuses", () => {
      ...
      const inProgressIssue = createTestIssue({
        id: "C",
        status: "in_progress",
      });
  ...
  describe("Orphan edges", () => {
    ...
    it("creates ghost nodes for orphan targets", () => {
      ...
      const ghostNode = result.current.nodes.find(
        (n) => n.id === "node-non-existent",
      );
    ...
    it("updates issueIdToNodeId map with ghost node IDs", () => {
      ...
      expect(result.current.issueIdToNodeId.get("ghost-target")).toBe(
        "node-ghost-target",
      );
    });
    ...
    it("ghost nodes break cycles naturally (no outgoing edges)", () => {
      ...
      const edgesFromGhost = result.current.edges.filter(
        (e) => e.source === "node-ghost",
      );
