...
describe("useTerminalMetadata", () => {
  ...
  describe("readiness (loadedFor / unavailable)", () => {
    ...
    it("does not report readiness while the workspace is unresolved", async () => {
      ...
      expect(result.current.loadedFor).toBeNull();
      // `isLoading` is deliberately NOT asserted here: an unresolved workspace
      // stays loading (see "stays loading with no fetch while the workspace is
      // unresolved" below), because consumers that still gate on `!isLoading`
      // must not read an empty list as authoritative. Readiness is `loadedFor`.
    });
  });
