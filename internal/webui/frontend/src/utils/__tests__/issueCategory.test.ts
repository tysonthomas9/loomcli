...
describe('hasNeedsRevision', () => {
  ...
  it('returns true when needs-revision is among other labels', () => {
    expect(hasNeedsRevision({ labels: ['bug', 'needs-revision', 'urgent'] })).toBe(true);
  });
...
describe('getOpenStatus', () => {
  ...
  describe('needs_plan status', () => {
    ...
    it('returns "needs_plan" when design exists but has needs-revision label (bug fix)', () => {
      expect(getOpenStatus({ design: 'plan text', labels: ['needs-revision'] })).toBe('needs_plan');
    });

    it('returns "needs_plan" when design exists and needs-revision among other labels', () => {
      expect(getOpenStatus({ design: 'plan', labels: ['bug', 'needs-revision'] })).toBe('needs_plan');
    });
  });
});
...
describe('getReviewType', () => {
  describe('plan review', () => {
    it('returns "plan" when status is review with no external_ref', () => {
      expect(getReviewType({ title: 'Design auth flow', status: 'review' })).toBe('plan');
    });

    it('returns "plan" when status is review with non-PR external_ref', () => {
      expect(getReviewType({ title: 'Task', status: 'review', external_ref: 'JIRA-123' })).toBe('plan');
    });

    it('returns "plan" when status is review with null external_ref', () => {
      expect(getReviewType({ title: 'Task', status: 'review', external_ref: null })).toBe('plan');
    });

    it('returns "plan" when status is review with empty string external_ref', () => {
      expect(getReviewType({ title: 'Task', status: 'review', external_ref: '' })).toBe('plan');
    });
  });
  ...
  describe('help review', () => {
    ...
    it('returns null when status is "blocked" without notes', () => {
      expect(getReviewType({ title: 'Blocked task', status: 'blocked' })).toBeNull();
    });

    it('returns null when status is "blocked" with empty string notes', () => {
      expect(getReviewType({ title: 'Blocked task', status: 'blocked', notes: '' })).toBeNull();
    });
  });

  describe('no review type', () => {
    it('returns null for regular issues', () => {
      expect(getReviewType({ title: 'Regular task', status: 'open' })).toBeNull();
    });

    it('returns null for in_progress status', () => {
      expect(getReviewType({ title: 'Working on it', status: 'in_progress' })).toBeNull();
    });

    it('returns null for closed status', () => {
      expect(getReviewType({ title: 'Done task', status: 'closed' })).toBeNull();
    });
  ...
  describe('edge cases', () => {
    ...
    it('returns "plan" for review status even with [Need Review] in title', () => {
      expect(getReviewType({ title: '[Need Review]', status: 'review' })).toBe('plan');
    });

    it('does not detect review from title alone (no status)', () => {
      expect(getReviewType({ title: '[Need Review] My feature plan' })).toBeNull();
    });
  });
});
