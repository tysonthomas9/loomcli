...
describe("AuthContext", () => {
  ...
  describe("ExternalAuthProvider", () => {
    ...
    it("resets tokenFetchedForSession on empty token allowing retry", async () => {
      ...
      // Same session.id across rerenders: without the fix, the guard on
      // tokenFetchedForSession.current would block the retry. The fix resets
      // that ref to null in the empty-token branch, allowing the second fetch.
      mockSessionData = {
        data: {
      ...
      // New session object reference but SAME session.id — this exercises
      // the ref-reset path specifically (not just the sessionId guard).
      mockSessionData = {
        data: {
          user: { id: "u1", name: "Faye", email: "faye@test.com" },
          session: { id: "s7" },
