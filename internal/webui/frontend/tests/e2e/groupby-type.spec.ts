...
async function navigateAndWait(
  page: Page,
  path = `/ws/${WORKSPACE_ID}/kanban?groupBy=type`,
) {
...
async function showMoreFilters(page: Page) {
  ...
  await expect(page.getByLabel("Group issues by")).toBeVisible();
}
...
test.describe("groupBy Type Swim Lanes", () => {
  ...
  test.describe("Drag and Drop Status Changes", () => {
    test("drag within type lane changes status", async ({ page }) => {
      ...
      await setupMocks(page, mockIssuesAllTypes, patchCalls);
      await navigateAndWait(page);
      ...
      const patchResponse = page.waitForResponse(
        (res) =>
          new URL(res.url()).pathname === `${WS_API}/issues/bug-1` &&
  ...
  test.describe("Edge Cases", () => {
    ...
    test("URL param ?groupBy=type shows type swim lanes", async ({ page }) => {
      await setupMocks(page);
      await page.goto(`/ws/${WORKSPACE_ID}/kanban?groupBy=type`);
      ...
      await expect(page.getByTestId("swim-lane-board")).toBeVisible();
      await showMoreFilters(page);
      await expect(page.getByLabel("Group issues by")).toHaveValue("type");
      await expect(
        page.getByRole("heading", { name: "Bug", exact: true }),
      ).toBeVisible();
    });
  ...
  test.describe("Combined with Filters", () => {
    test("type grouping works with priority filter", async ({ page }) => {
      await setupMocks(page);
      await page.goto(`/ws/${WORKSPACE_ID}/kanban?groupBy=type&priority=0`);
      await page.waitForResponse(
        (res) => new URL(res.url()).pathname === `${WS_API}/issues`,
      );
      await expect(page.getByTestId("swim-lane-board")).toBeVisible();
    ...
    test("type grouping works with search", async ({ page }) => {
      await setupMocks(page);
      await page.goto(`/ws/${WORKSPACE_ID}/kanban?groupBy=type&search=Bug`);
      await page.waitForResponse(
        (res) => new URL(res.url()).pathname === `${WS_API}/issues`,
      );
      await expect(page.getByTestId("swim-lane-board")).toBeVisible();
