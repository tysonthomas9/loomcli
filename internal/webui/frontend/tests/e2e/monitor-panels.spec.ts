...
async function setupMonitorApiMocks(page: Page, status: object, tasks: object) {
...
test.describe.skip("AgentActivityPanel deep behavior", () => {
  test("agent cards show correct status via data-status attribute", async ({
    page,
  }) => {
  ...
  test("agent cards display status text and task titles", async ({ page }) => {
    ...
    const novaCard = agentPanel.locator('[data-status="working"]', {
      hasText: "nova",
    });
    ...
    const cobaltCard = agentPanel.locator('[data-status="planning"]', {
      hasText: "cobalt",
    });
    ...
    const emberCard = agentPanel.locator('[data-status="error"]', {
      hasText: "ember",
    });
    ...
    const falconCard = agentPanel.locator('[data-status="idle"]', {
      hasText: "falcon",
    });
    ...
    const jadeCard = agentPanel.locator('[data-status="ready"]', {
      hasText: "jade",
    });
    await expect(jadeCard).toContainText("Ready");
  });

  test("agent cards show git sync indicators (ahead/behind)", async ({
    page,
  }) => {
    await setupMocks(page);
    await navigateToMonitor(page);
  ...
  test("agent summary bar shows correct counts", async ({ page }) => {
    ...
    const summary = agentPanel.locator(
      '[role="status"][aria-label="Agent activity summary"]',
    );
  ...
  test("agent cards have interactive attributes and keyboard support", async ({
    page,
  }) => {
    await setupMocks(page);
    await navigateToMonitor(page);
...
async function setupHealthPanelMocks(
  ...
  const blockedResponse = options?.blockedResponse ?? {
    success: true,
    data: [],
  };
  ...
  await setupMonitorApiMocks(page, loomStatus, mockLoomTasks);
}
...
test.describe.skip("ProjectHealthPanel deep behavior", () => {
  ...
  test("issue counts display open, closed, and total correctly", async ({
    page,
  }) => {
    await setupHealthPanelMocks(page, {
      stats: { open: 10, closed: 5, total: 15, completion: 33 },
  ...
  test("bottleneck section shows when blocked issues share a common blocker", async ({
    page,
  }) => {
    await setupHealthPanelMocks(page, {
  ...
  test("no bottlenecks message when none exist", async ({ page }) => {
    ...
    const bottleneckHeading = healthPanel.getByRole("heading", {
      name: "Bottlenecks",
    });
...
test.describe.skip("MiniDependencyGraph deep behavior", () => {
  test("renders nodes from blocking dependencies", async ({ page }) => {
    await setupGraphMocks(page, {
      graphIssues: mockGraphIssuesWithDeps,
      hasBlocked: true,
    });
    ...
    await expect(
      miniGraph.getByText("No blocking dependencies"),
    ).not.toBeVisible();
  });
  ...
  test("expand button is keyboard accessible and navigates to graph view", async ({
    page,
  }) => {
    await setupGraphMocks(page, {
      graphIssues: mockGraphIssuesWithDeps,
