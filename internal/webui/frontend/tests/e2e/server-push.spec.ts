...
// SSE endpoint
const SSE_ENDPOINT = "**/api/workspaces/*/events**";
...
// Helper to mock the workspace-scoped fleet APIs with test data
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          open_issues: issues.filter(
            (issue) => (issue as { status?: string }).status === "open",
          ).length,
          in_progress_issues: issues.filter(
            (issue) => (issue as { status?: string }).status === "in_progress",
          ).length,
...
test.describe("Connection establishment", () => {
  ...
  test("connection status element exists with data-state attribute", async ({
    page,
  }) => {
    await abortSSE(page);
    await mockReadyAPI(page, []);
  ...
  test("no polling requests after SSE established (only initial API calls)", async ({
    page,
  }) => {
...
test.describe("SSE event handling", () => {
  test("app handles SSE connection events without crashing", async ({
    page,
  }) => {
...
test.describe("Reconnection handling", () => {
  ...
  test("connection status shows reconnecting or disconnected on failure", async ({
    page,
  }) => {
    await page.route(SSE_ENDPOINT, async (route) => {
      await route.abort("connectionfailed");
  ...
  test("app remains functional during reconnection attempts", async ({
    page,
  }) => {
    await page.route(SSE_ENDPOINT, async (route) => {
      await route.abort("connectionfailed");
...
test.describe("Multiple clients", () => {
  test("two browser contexts can both attempt SSE connections", async ({
    browser,
  }) => {
    ...
    // Navigate both pages
    await Promise.all([
      page1.goto(workspacePath("/")),
      page2.goto(workspacePath("/")),
    ]);
...
test.describe("Error recovery", () => {
  ...
  test("mutation for non-existent issue does not crash app", async ({
    page,
  }) => {
    await page.route(SSE_ENDPOINT, async (route) => {
      await route.fulfill({
...
test.describe("Edge cases", () => {
  ...
  test("handles SSE endpoint returning wrong content type", async ({
    page,
  }) => {
    await page.route(SSE_ENDPOINT, async (route) => {
      await route.fulfill({
  ...
  test("SSE with since parameter includes timestamp", async ({ page }) => {
    ...
    expect(capturedUrls[0]).toContain("/api/workspaces/default/events");
  });
});
