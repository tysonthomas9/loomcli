...
async function setupMocks(page: Page, options?: SetupMocksOptions) {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (pathname === "/api/config") {
      ...
      if (issue) {
        ...
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify(ok(getMockIssueDetails(issue, { title }))),
        });
      } else {
        await route.fulfill({
          status: 404,
    ...
    } else if (method === "PATCH" && pathname.startsWith(`${WS_API}/issues/`)) {
...
async function navigateToApp(page: Page, view?: "kanban" | "table" | "graph") {
  const viewPath =
    view === "table" ? "table" : view === "graph" ? "graph" : "kanban";
  const url = `/ws/${WORKSPACE_ID}/${viewPath}`;

  await Promise.all([
    page.waitForResponse((res) => {
      const pathname = new URL(res.url()).pathname;
      return (
        pathname === `${WS_API}/issues` || pathname === `${WS_API}/issues/graph`
      );
    }),
    page.goto(url),
  ]);
...
test.describe("IssueDetailPanel - Graph View", () => {
  ...
  test("clicking graph node opens panel with issue data", async ({ page }) => {
    ...
    const responsePromise = page.waitForResponse(
      (res) => new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-1`,
    );
  ...
  test("clicking different graph nodes shows correct data", async ({
    page,
  }) => {
    await setupMocks(page);
    await navigateToApp(page, "graph");
    ...
    const thirdIssueResponse = page.waitForResponse(
      (res) => new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-3`,
    );
    await thirdNode.click();

    // Wait for API call for third issue
    await thirdIssueResponse;
...
test.describe("IssueDetailPanel - Title Edit Flow", () => {
  test("editing title and pressing Enter calls PATCH API", async ({ page }) => {
    ...
    const issueCard = page
      .locator("article")
      .filter({ hasText: "First Wiring Test Issue" });
    ...
    const patchPromise = page.waitForResponse(
      (res) =>
        new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-1` &&
...
test.describe("IssueDetailPanel - Reopen with Updated Data", () => {
  test("reopening issue fetches fresh data", async ({ page }) => {
    ...
    // setupMocks already handles titleRef for both list and detail endpoints.
    ...
    const issueCardAgain = page
      .locator("article")
      .filter({ hasText: "Original Title" });
    const freshIssueResponse = page.waitForResponse(
      (res) => new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-1`,
    );
    await issueCardAgain.click();

    // Wait for fresh API call
    await freshIssueResponse;
  ...
  test("switching issues shows correct data for each", async ({ page }) => {
    ...
    const cardB = page
      .locator("article")
      .filter({ hasText: "Second Wiring Test Issue" });
    const issueBResponse = page.waitForResponse(
      (res) => new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-2`,
    );
    await cardB.click();

    await issueBResponse;
    ...
    const issueAResponse = page.waitForResponse(
      (res) => new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-1`,
    );
    await cardA.click();

    await issueAResponse;
  ...
  test("title edit updates panel and can be reopened with new value", async ({
    ...
    const freshGetResponse = page.waitForResponse(
      (res) =>
        new URL(res.url()).pathname === `${WS_API}/issues/wiring-test-1` &&
