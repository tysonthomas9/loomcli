...
function getLanes(page: Page) {
  return page.getByTestId("swim-lane-board").locator(":scope > section");
}

function getPriorityLane(page: Page, title: string) {
  return getLanes(page).filter({
    has: page.getByRole("heading", { name: title }),
  });
}

async function showMoreFilters(page: Page) {
  const groupByFilter = page.getByTestId("more-filters-groupby");
  if (
...
function groupByFilter(page: Page) {
...
test.describe("groupBy Priority Swim Lanes", () => {
  test.describe("Lane Rendering", () => {
    ...
    test("renders all 5 priority lanes (P0-P4) plus No Priority lane", async ({
      ...
      await navigateAndWait(
        page,
        `/ws/${WORKSPACE_ID}/kanban?groupBy=priority`,
      );
    ...
    test("issues appear in correct priority lanes", async ({ page }) => {
      ...
      const p0Lane = getPriorityLane(page, "P0 (Critical)");
      ...
      // P1 lane has 1 issue
      const p1Lane = getPriorityLane(page, "P1 (High)");
      ...
      const p2Lane = getPriorityLane(page, "P2 (Medium)");
  ...
  test.describe("Issue Counts", () => {
    ...
    test("lane headers display correct issue counts", async ({ page }) => {
      ...
      const noPriorityLane = getPriorityLane(page, "No Priority");
  ...
  test.describe("Status Columns Within Priority Lanes", () => {
    test("each priority lane contains all status columns", async ({ page }) => {
      ...
      await expect(p0Lane.locator('section[data-status="done"]')).toBeVisible();
    });
  ...
  test.describe("Drag and Drop Within Priority Lanes", () => {
    ...
    test("dragging issue within priority lane changes status", async ({
      ...
      const patchResponse = page.waitForResponse(
        (res) =>
          new URL(res.url()).pathname === `${WS_API}/issues/p0-bug` &&
  ...
  test.describe("Edge Cases", () => {
    ...
    test("switching from priority to All falls back to default epic grouping", async ({
      page,
    }) => {
      await setupMocks(page);
      await navigateAndWait(
      ...
      await groupByFilter(page).selectOption("none");

      // Current workspace routes normalize the empty groupBy selection to the
      // default epic grouping.
      await expect(groupByFilter(page)).toHaveValue("epic");
      await expect(page.getByTestId("swim-lane-board")).toBeVisible();
      await expect(
    ...
    test("priority filter works with priority groupBy", async ({ page }) => {
      ...
      await navigateAndWait(
        page,
        `/ws/${WORKSPACE_ID}/kanban?groupBy=priority&priority=0`,
      );
  ...
  test.describe("URL Synchronization", () => {
    test("groupBy=priority persists in URL", async ({ page }) => {
      ...
      await showMoreFilters(page);
      await groupByFilter(page).selectOption("priority");
