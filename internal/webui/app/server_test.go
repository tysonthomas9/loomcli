...
func TestFindAvailablePort_FirstPortFree(t *testing.T) {
	...
	listener, port, err := webui.FindAvailablePort("127.0.0.1", startPort, 5)
	if err != nil {
		t.Fatalf("webui.FindAvailablePort(%d, 5) failed: %v", startPort, err)
	}
	defer listener.Close()

	if port != startPort {
		t.Errorf("webui.FindAvailablePort(%d, 5) = %d, want %d", startPort, port, startPort)
	}
}
...
func TestFindAvailablePort_FallbackToNextPort(t *testing.T) {
	...
	if port == startPort {
		t.Errorf("webui.FindAvailablePort(%d, 5) returned the occupied port", startPort)
	}
	if port < startPort || port > startPort+4 {
		t.Errorf("webui.FindAvailablePort(%d, 5) = %d, want port in range [%d, %d]", startPort, port, startPort+1, startPort+4)
	}
}
...
func TestFindAvailablePort_FallbackMultiplePorts(t *testing.T) {
	...
	listener, port, err := webui.FindAvailablePort("127.0.0.1", startPort, maxAttempts)
	if err != nil {
		t.Fatalf("webui.FindAvailablePort(%d, %d) failed: %v", startPort, maxAttempts, err)
	}
	defer listener.Close()
...
func TestFindAvailablePort_AllPortsInUse(t *testing.T) {
	...
	if err == nil {
		listener.Close()
		t.Errorf("webui.FindAvailablePort(%d, %d) = %d, want error (all ports occupied)", startPort, maxAttempts, port)
	}
...
func TestFindAvailablePort_SingleAttempt(t *testing.T) {
	...
	startPort := grabEphemeralPort(t)
	listener, port, err := webui.FindAvailablePort("127.0.0.1", startPort, 1)
	if err != nil {
		t.Fatalf("webui.FindAvailablePort(%d, 1) failed: %v", startPort, err)
	}
	listener.Close()

	if port != startPort {
		t.Errorf("webui.FindAvailablePort(%d, 1) = %d, want %d", startPort, port, startPort)
	}
	...
	listener, port, err = webui.FindAvailablePort("127.0.0.1", occupiedPort, 1)
	if err == nil {
		listener.Close()
		t.Errorf("webui.FindAvailablePort(%d, 1) = %d, want error (port occupied with single attempt)", occupiedPort, port)
	}
}
...
func TestFindAvailablePort_ZeroAttempts(t *testing.T) {
	...
	listener, port, err := webui.FindAvailablePort("127.0.0.1", startPort, 0)
	if err == nil {
		listener.Close()
		t.Errorf("webui.FindAvailablePort(%d, 0) = %d, want error (zero attempts)", startPort, port)
	}
}
...
func TestStartServer_WriteTimeout(t *testing.T) {
	...
	var health healthhandlers.HealthStatus
