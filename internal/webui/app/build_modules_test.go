...
// zero-value Server populates wsModules with exactly the always-constructed
// modules plus the non-store PR-list fallback, and does not panic.
...
// is non-nil, buildModules produces all modules for non-store mode.
func TestServer_BuildModules_AllDeps(t *testing.T) {
	...
	// 5 always + SSE(hub) + TerminalTab(termSvc) + IssueTab(issueTabStore) +
	// Terminal(termSvc) + Fleet(fleetRegistry) + Git(diffSvc) + File(fileSvc) +
	// gh-backed PR list fallback (non-store) = 13
