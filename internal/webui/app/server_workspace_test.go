...
func TestCreateWarnings_ContextHelpers(t *testing.T) {
	t.Run("full lifecycle", func(t *testing.T) {
		...
		ctx = workspacecoord.WithCreateWarnings(ctx)

		// Initially no warnings
		if w := workspacecoord.GetCreateWarnings(ctx); w != nil {
			t.Fatalf("expected nil warnings initially, got %v", w)
		}

		// Add a warning
		workspacecoord.AddCreateWarning(ctx, "warning one")
		...
		// Add another warning
		workspacecoord.AddCreateWarning(ctx, "warning two")
		warnings = workspacecoord.GetCreateWarnings(ctx)
		if len(warnings) != 2 {
	...
	t.Run("AddCreateWarning is no-op on plain context", func(t *testing.T) {
		ctx := context.Background()
		// Should not panic
		workspacecoord.AddCreateWarning(ctx, "should be ignored")
...
func TestWrapWorkspaceCreateFn_CollectsWarnings(t *testing.T) {
	...
	innerCreate := func(ctx context.Context, req workspacecoord.WorkspaceCreateRequest) (workspacecoord.WorkspaceCreateResult, error) {
		return workspacecoord.WorkspaceCreateResult{}, nil
	}
	...
	_, err := wrapped(ctx, workspacecoord.WorkspaceCreateRequest{Name: "my-ws"})
	if err != nil {
...
func TestWrapWorkspaceCreateFn_EmptyWorkspaceID_AbortsRegistration(t *testing.T) {
	...
	innerCreate := func(ctx context.Context, req workspacecoord.WorkspaceCreateRequest) (workspacecoord.WorkspaceCreateResult, error) {
		innerCalled = true
		return workspacecoord.WorkspaceCreateResult{}, nil // empty WorkspaceID
	}
	...
	_, err := wrapped(context.Background(), workspacecoord.WorkspaceCreateRequest{Name: "my-ws"})
	if err != nil {
		t.Fatalf("expected nil error, got %v", err)
	}
	if !innerCalled {
...
func TestWrapWorkspaceCreateFn_EmptyWorkspaceID_NoError_AbortsRegistration(t *testing.T) {
	...
	innerCreate := func(ctx context.Context, req workspacecoord.WorkspaceCreateRequest) (workspacecoord.WorkspaceCreateResult, error) {
		innerCalled = true
		return workspacecoord.WorkspaceCreateResult{WorkspaceID: ""}, nil // empty ID, no error
	}
...
func TestWrapWorkspaceCreateFn_ZeroResult_AbortsRegistration(t *testing.T) {
	...
	innerCreate := func(ctx context.Context, req workspacecoord.WorkspaceCreateRequest) (workspacecoord.WorkspaceCreateResult, error) {
		return workspacecoord.WorkspaceCreateResult{}, nil // zero-value result
	}
...
func TestWrapWorkspaceCreateFn_ResultWithID_RegistersByUUID(t *testing.T) {
	...
	innerCreate := func(ctx context.Context, req workspacecoord.WorkspaceCreateRequest) (workspacecoord.WorkspaceCreateResult, error) {
		return workspacecoord.WorkspaceCreateResult{WorkspaceID: wsUUID, WorkspacePath: wsPath}, nil
	}

	wrapped := wrapWorkspaceCreateFn(innerCreate, registry)
	_, err := wrapped(context.Background(), workspacecoord.WorkspaceCreateRequest{Name: wsName, Path: wsPath})
	if err != nil {
		t.Fatalf("expected nil error, got %v", err)
	}
...
func TestWrapWorkspaceCreateFn_InnerCreateFails_NoRegistration(t *testing.T) {
	...
	innerCreate := func(ctx context.Context, req workspacecoord.WorkspaceCreateRequest) (workspacecoord.WorkspaceCreateResult, error) {
		return workspacecoord.WorkspaceCreateResult{}, createErr
	}
