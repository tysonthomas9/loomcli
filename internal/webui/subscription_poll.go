...
func (s *DaemonSubscriber) emitGranularMutations(changed []changedIssue, now time.Time, totalCount int64) {
	...
	if mutationCount > 0 {
		logger.Info("external DB change: broadcast granular mutations", "count", mutationCount, "clients", s.hub.ClientCount())
	}
}
...
func (s *DaemonSubscriber) fetchChangedIssues(client *rpc.Client, since time.Time) []changedIssue {
	...
	if err != nil {
		logger.Error("external poll: list changed issues error", "err", err)
		return nil
	}
	if !resp.Success {
		logger.Error("external poll: list changed issues failed", "err", resp.Error)
		return nil
	}
...
func parseChangedIssues(data json.RawMessage) []changedIssue {
	...
	if err := json.Unmarshal(data, &raw); err != nil {
		logger.Error("external poll: parse changed issues error", "err", err)
		return nil
	}
...
func (s *DaemonSubscriber) broadcastRefresh(now time.Time, totalCount int64) {
	...
	s.lastPollTime = now
	s.mu.Unlock()
	logger.Info("external DB change detected, broadcast refresh", "clients", s.hub.ClientCount())
}
