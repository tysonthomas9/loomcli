...
func (m *TerminalManager) KillSession(wsID, name string) error {
	...
	return m.killByInternal(wsID, name, m.tmuxName(wsID, name))
}

// killByInternal is the implementation shared by KillSession,
// KillWorkspaceSessions, and ScheduleKill's deferred goroutine. wsID flows
// through to the onSessionKilled callback so multi-workspace history stores
// can route the completion record to the right workspace. userName is the
// user-facing session name (for the callback) and internalName is the
// qualified tmux session name used as the in-memory map key and tmux -t
// target.
func (m *TerminalManager) killByInternal(wsID, userName, internalName string) error {
	...
	// The returned path is qualified by the internal tmux name, so the
	// callback gets an unambiguous file path to record in session history.
	scrollbackPath := m.captureScrollbackToFileByInternal(userName, internalName)
	...
	delete(m.sessionOwners, userName)
	m.mu.Unlock()
	...
	// records are keyed the same way the UI refers to them, plus the
	// qualified on-disk scrollback path so history lookups don't need to
	// reconstruct it from the unqualified name.
	if m.onSessionKilled != nil {
		m.onSessionKilled(wsID, userName, scrollbackPath)
	}
...
func (m *TerminalManager) ScheduleKill(wsID, name string, delay time.Duration) {
	...
	go func() {
		...
		case <-time.After(delay):
			...
			m.mu.Unlock()
			if shouldKill {
				_ = m.killByInternal(wsID, name, internalName)
			}
		case <-ctx.Done():
