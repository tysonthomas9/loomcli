...
func openAttachStream(ctx, streamCtx context.Context, conn *agentdConn, session string, cols, rows uint16, hint string, expectReplay bool) (agentdpb.Terminal_AttachClient, *agentdpb.AttachReady, error) { //nolint:funlen
	...
	// Per-handshake context derived from streamCtx. Canceling this on
...
func (a *agentdAttachment) runRecv() { //nolint:funlen
	defer close(a.done)
...
func (a *agentdAttachment) tryReconnect(initialErr error) bool { //nolint:funlen
	...
	for i := 0; i < reconnectAttempts; i++ {
		// Honor close() cancellation in the backoff sleep so a Kill while
