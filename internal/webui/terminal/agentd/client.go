...
func (c *AgentdClient) AttachSession(key terminal.SessionKey, cols, rows uint16, launch *tabmeta.LaunchSpec) (terminal.Attachment, bool, error) {
	var argv []string
	if launch != nil {
		argv = launch.Argv
	}
...
// SessionClosed reports whether the session is known to have ended locally.
// agentd sessions live in a remote host process, so local closed-tracking
// does not apply; returning false preserves the pre-existing reattach path.
func (c *AgentdClient) SessionClosed(_ terminal.SessionKey) bool {
	return false
}
...
// the per-workspace cap lands the cap is reported as 0, signaling
