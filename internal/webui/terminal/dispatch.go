...
func (d *Dispatcher) AttachSession(key SessionKey, cols, rows uint16, launch *tabmeta.LaunchSpec) (Attachment, bool, error) {
	return d.pickFor(key).AttachSession(key, cols, rows, launch)
}

// SessionClosed forwards to the chosen backend.
func (d *Dispatcher) SessionClosed(key SessionKey) bool {
	return d.pickFor(key).SessionClosed(key)
}
