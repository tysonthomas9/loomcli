...
type ClaimIssueParams struct {
	...
	// onto, because fleet-db arbitrates locks BY actor.
	Actor string
}
...
// to open.
//
// Actor scopes the release the same way it scopes the claim: releasing a lock
// held by a different worker is a conflict, not a success. Empty Actor keeps
// the legacy status-transition path under the server-side configured actor.
