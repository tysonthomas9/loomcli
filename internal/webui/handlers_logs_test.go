...
// --- handleTaskLogStream SSE happy path tests (raw chunk contract) ---

func decodeLogChunkData(t *testing.T, data string) (LogChunkPayload, string) {
	t.Helper()
	var payload LogChunkPayload
	if err := json.Unmarshal([]byte(data), &payload); err != nil {
		t.Fatalf("failed to parse payload: %v", err)
	}
	decoded, err := base64.StdEncoding.DecodeString(payload.ChunkBase64)
	if err != nil {
		t.Fatalf("failed to decode chunk_b64: %v", err)
	}
...
func TestHandleTaskLogStream_Success(t *testing.T) {
	...
	payload, decoded := decodeLogChunkData(t, evt.Data)
	if payload.ByteOffset == 0 {
		t.Fatalf("byte_offset = 0, want > 0")
	}
	if !strings.Contains(decoded, "plan step 1") || !strings.Contains(decoded, "plan step 2") {
		t.Fatalf("decoded chunk missing expected content: %q", decoded)
	}

	cancel()
}

// TestHandleTaskLogStream_NewLinesAfterConnect tests that new bytes appended
...
func TestHandleTaskLogStream_NewLinesAfterConnect(t *testing.T) {
	...
	_, decoded := decodeLogChunkData(t, evt.Data)
	if !strings.Contains(decoded, "impl step 2\nimpl step 3\n") {
		t.Fatalf("decoded append chunk missing expected content: %q", decoded)
	}
