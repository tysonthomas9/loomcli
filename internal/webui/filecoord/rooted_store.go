...
func openScopedRoot(rootPath string) (*scopedRoot, error) {
	...
	if err != nil {
		return nil, apperrors.ErrInternal("failed to resolve scope root", err)
	}
	info, err := os.Lstat(rootPath)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, apperrors.ErrNotFound("scope root not found")
		}
		return nil, apperrors.ErrInternal("failed to stat scope root", err)
	}
	if info.Mode()&os.ModeSymlink != 0 {
		return nil, apperrors.ErrForbidden("scope root must not be a symlink")
	}
	if !info.IsDir() {
		return nil, apperrors.ErrValidation("scope root is not a directory")
	}
	root, err := os.OpenRoot(rootPath)
	if err != nil {
		return nil, apperrors.ErrInternal("failed to open scope root", err)
	}
	openedInfo, err := root.Stat(".")
	if err != nil {
		_ = root.Close()
		return nil, apperrors.ErrInternal("failed to verify scope root", err)
	}
	if !os.SameFile(info, openedInfo) {
		_ = root.Close()
		return nil, apperrors.ErrForbidden("scope root changed while it was opened")
	}
...
func resolvedRootIdentity(rootPath string, openedInfo os.FileInfo) (string, error) {
	...
	if err != nil {
		return "", apperrors.ErrInternal("failed to resolve canonical scope root", err)
	}
	resolvedInfo, err := os.Stat(resolvedRootPath)
	if err != nil {
		return "", apperrors.ErrInternal("failed to verify canonical scope root", err)
	}
	if !os.SameFile(openedInfo, resolvedInfo) {
		return "", apperrors.ErrForbidden("scope root changed while resolving canonical path")
	}
...
func normalizeFilePath(name string, allowRoot bool) (string, error) {
	if strings.IndexByte(name, 0) >= 0 {
		return "", apperrors.ErrValidation("path contains a NUL byte")
	}
	if name == "" {
		if !allowRoot {
	...
	if clean == "." {
		if !allowRoot {
			return "", apperrors.ErrValidation("scope root cannot be mutated")
		}
		return clean, nil
	}
...
func validateRootedName(name string) error {
	...
	if clean != name {
		return apperrors.ErrValidation("path must be normalized")
	}
	return nil
}
...
func (s *rootedFileRoot) ensureNoSymlinks(name string, allowMissing bool) error {
	...
	for i, part := range parts {
		...
		if i < len(parts)-1 && !info.IsDir() {
			return apperrors.ErrValidation("path component is not a directory")
		}
	}
	return nil
}
...
func (s *rootedFileRoot) stat(name string) (os.FileInfo, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, apperrors.ErrNotFound("path not found")
		}
...
func isNotFound(err error) bool {
	...
	return errors.As(err, &serviceErr) && serviceErr.Kind == apperrors.KindNotFound
}
...
func (s *rootedFileRoot) List(name string) ([]os.DirEntry, error) {
	...
	if err != nil {
		...
		return nil, apperrors.ErrInternal("failed to open directory", err)
	}
	defer f.Close()
	info, err := f.Stat()
	if err != nil {
		return nil, apperrors.ErrInternal("failed to stat directory", err)
	}
	if !info.IsDir() {
...
func (s *rootedFileRoot) Read(name string, limit int64) ([]byte, os.FileInfo, bool, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil, false, apperrors.ErrNotFound("file not found")
		}
		return nil, nil, false, apperrors.ErrInternal("failed to open file", err)
	}
	defer f.Close()
	info, err := f.Stat()
	if err != nil {
		return nil, nil, false, apperrors.ErrInternal("failed to stat file", err)
	}
	if info.IsDir() {
		return nil, nil, false, apperrors.ErrValidation("path is a directory, not a file")
	}
	if !info.Mode().IsRegular() {
		return nil, nil, false, apperrors.ErrValidation("path is not a regular file")
	}
	...
	if err != nil {
		return nil, nil, false, apperrors.ErrInternal("failed to read file", err)
	}
...
func (s *rootedFileRoot) Hash(name string) ([]byte, os.FileInfo, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil, apperrors.ErrNotFound("file not found")
		}
		return nil, nil, apperrors.ErrInternal("failed to open file", err)
	}
	defer f.Close()
	info, err := f.Stat()
	if err != nil {
		return nil, nil, apperrors.ErrInternal("failed to stat file", err)
	}
	if !info.Mode().IsRegular() {
		return nil, nil, apperrors.ErrValidation("path is not a regular file")
	}
	hash := sha256.New()
	if _, err := io.Copy(hash, f); err != nil {
		return nil, nil, apperrors.ErrInternal("failed to hash file", err)
	}
...
func (s *rootedFileRoot) ReadVersioned(name string, limit int64) ([]byte, os.FileInfo, bool, []byte, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil, false, nil, apperrors.ErrNotFound("file not found")
		}
		return nil, nil, false, nil, apperrors.ErrInternal("failed to open file", err)
	}
	defer f.Close()
	info, err := f.Stat()
	if err != nil {
		return nil, nil, false, nil, apperrors.ErrInternal("failed to stat file", err)
	}
	if !info.Mode().IsRegular() {
		return nil, nil, false, nil, apperrors.ErrValidation("path is not a regular file")
	}
...
func (s *rootedFileRoot) Readlink(name string) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return "", apperrors.ErrNotFound("symlink not found")
		}
		return "", apperrors.ErrInternal("failed to read symlink", err)
	}
	return target, nil
}
...
func readAndHashRootedFile(f *os.File, info os.FileInfo, limit int64) ([]byte, os.FileInfo, bool, []byte, error) {
	...
	if limit < 0 {
		...
		if err != nil {
			return nil, nil, false, nil, apperrors.ErrInternal("failed to read file", err)
		}
	...
	if err != nil {
		return nil, nil, false, nil, apperrors.ErrInternal("failed to hash file", err)
	}
...
func (s *rootedFileRoot) writeAtomicFallback(name string, content []byte) error {
	...
	if _, err := temp.Write(content); err != nil {
		return apperrors.ErrInternal("failed to write temporary file", err)
	}
	if err := temp.Sync(); err != nil {
		return apperrors.ErrInternal("failed to sync temporary file", err)
	}
	if err := temp.Close(); err != nil {
		return apperrors.ErrInternal("failed to close temporary file", err)
	}
	closed = true
	if err := s.ensureNoSymlinks(parent, false); err != nil {
		return err
...
func (s *rootedFileRoot) writePermissions(name string) (os.FileMode, error) {
	...
	if err != nil {
		return 0, apperrors.ErrInternal("failed to stat file", err)
	}
	if info.Mode()&os.ModeSymlink != 0 {
		return 0, apperrors.ErrForbidden("refusing to overwrite symlink")
	}
	if info.IsDir() {
		return 0, apperrors.ErrValidation("path is a directory, not a file")
	}
	if !info.Mode().IsRegular() {
		return 0, apperrors.ErrValidation("path is not a regular file")
	}
...
func (s *rootedFileRoot) createTemp(parent string, perm os.FileMode) (string, *os.File, error) {
	for i := 0; i < 10; i++ {
		...
		if _, err := rand.Read(random[:]); err != nil {
			return "", nil, apperrors.ErrInternal("failed to generate temporary file name", err)
		}
		...
		if !os.IsExist(err) {
			return "", nil, apperrors.ErrInternal("failed to create temporary file", err)
		}
	}
	return "", nil, apperrors.ErrInternal("failed to allocate temporary file", nil)
}
...
func (s *rootedFileRoot) MkdirAll(name string) error {
	...
	for _, part := range strings.Split(name, string(filepath.Separator)) {
		...
		if !os.IsNotExist(err) {
			return apperrors.ErrInternal("failed to stat directory", err)
		}
...
func (s *rootedFileRoot) Delete(name string, recursive bool) error {
	...
	if info.IsDir() {
		if !recursive {
			...
			if containsGit {
				return apperrors.ErrForbidden("directory contains .git metadata")
			}
