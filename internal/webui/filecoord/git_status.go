...
func (s *fileServiceImpl) GitStatusScoped(ctx context.Context, wsID string, scope FileScope, target, repo string) (FileGitStatusResult, error) {
	...
	default:
		return FileGitStatusResult{}, apperrors.ErrValidation("unsupported scope " + string(scope))
	}
}

func (s *fileServiceImpl) workspaceGitStatus(ctx context.Context, wsID, target, repo string) (FileGitStatusResult, error) {
	if target != "" {
		return FileGitStatusResult{}, apperrors.ErrValidation("workspace scope does not take a target")
	}
	if repo != "" {
		return FileGitStatusResult{}, apperrors.ErrValidation("repo qualifier is only supported for agent scope")
	}
...
func (s *fileServiceImpl) RepairCheckout(ctx context.Context, wsID string, req FileCheckoutRepairRequest) (*ops.RepairResult, error) {
	if err := ctx.Err(); err != nil {
		return nil, apperrors.ErrTimeout("checkout repair canceled")
	}
	scope := strings.TrimSpace(req.Scope)
	target := strings.TrimSpace(req.Target)
	repo := strings.TrimSpace(req.Repo)
	if scope == "" {
		return nil, apperrors.ErrValidation("scope is required")
	}
	if target == "" {
		return nil, apperrors.ErrValidation("target is required")
	}
	...
	if err != nil {
		...
		return nil, apperrors.ErrInternal("failed to repair checkout", err)
	}
	return &result, nil
}
...
func validateGitCheckoutRoot(root string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return apperrors.ErrNotFound("git checkout not found")
		}
		return apperrors.ErrInternal("failed to stat git checkout", err)
	}
	if fi.Mode()&os.ModeSymlink != 0 {
		return apperrors.ErrForbidden("refusing to follow symlink")
	}
	if !fi.IsDir() {
		return apperrors.ErrValidation("git checkout root is not a directory")
	}
	...
	if err != nil {
		...
		return apperrors.ErrInternal("failed to stat git metadata", err)
	}
...
func withGitCheckoutIdentity(ctx context.Context, checkoutRoot string, heldRoot *scopedRoot) (context.Context, error) {
	...
	if err != nil {
		return nil, apperrors.ErrInternal("failed to resolve git checkout", err)
	}
	if heldRoot != nil && heldRoot.store != nil && heldRoot.store.root != nil && filepath.Clean(heldRoot.path) == filepath.Clean(absRoot) {
		...
		if err != nil {
			return nil, apperrors.ErrInternal("failed to stat held git checkout", err)
		}
		if !info.IsDir() {
			return nil, apperrors.ErrValidation("git checkout root is not a directory")
		}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, apperrors.ErrNotFound("git checkout not found")
		}
		return nil, apperrors.ErrInternal("failed to stat git checkout", err)
	}
