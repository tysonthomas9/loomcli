...
func versionForPath(store *rootedFileRoot, path string) (string, os.FileInfo, error) {
	...
	if !info.IsDir() {
		return "", nil, apperrors.ErrValidation("path is not a regular file or directory")
	}
	h := sha256.New()
	entries := 0
	var bytes int64
...
func hashDirectoryManifest(store *rootedFileRoot, root, relative string, h hash.Hash, entries *int, bytes *int64) error {
	...
	for _, entry := range dirEntries {
		...
		*m.entries++
		if *m.entries > maxDirectoryVersionEntries {
			return apperrors.ErrPayloadTooLarge("directory version manifest exceeds entry limit")
		}
...
func (m *directoryManifest) hashEntry(entry os.DirEntry, relative string) error {
	...
	case entry.Type()&os.ModeSymlink != 0:
		...
		if int64(len(target)) > maxDirectoryVersionBytes-*m.bytes {
			return apperrors.ErrPayloadTooLarge("directory version manifest exceeds byte limit")
		}
	...
	default:
		info, err := entry.Info()
		if err != nil {
			return apperrors.ErrInternal("failed to stat directory manifest entry", err)
		}
		if !info.Mode().IsRegular() {
			return apperrors.ErrValidation("directory contains unsupported file type")
		}
