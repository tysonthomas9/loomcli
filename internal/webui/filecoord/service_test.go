...
func scopedSvc(root string) FileService {
...
func mustScopedVersion(t *testing.T, svc FileService, ctx context.Context, scope FileScope, target, path string) string {
	t.Helper()
...
func TestFileServiceImpl_ViewerFiltersSensitiveSurfaces(t *testing.T) {
	...
	svc := scopedSvc(root)
	viewer := WithFileCapabilities(context.Background(), FileCapabilities{Read: true})
	...
	} else if serviceErr, ok := err.(*apperrors.ServiceError); !ok || serviceErr.Kind != apperrors.KindForbidden {
...
func TestFileServiceImpl_EditorCanAccessSensitiveFiles(t *testing.T) {
	...
	svc := scopedSvc(root)
	editor := WithFileCapabilities(context.Background(), FileCapabilities{Read: true, Write: true, Sensitive: true})

	read, err := svc.ReadFileScoped(editor, "ws", ScopeWorkspace, "", "", ".env.production")
	if err != nil || read.Content != "TOKEN=secret" {
		t.Fatalf("editor read = %+v, err=%v", read, err)
	}
	if err := writeScopedFile(editor, svc, "ws", ScopeWorkspace, "", "", ".env.production", "TOKEN=updated"); err != nil {
...
func setupScopedService(t *testing.T) (FileService, []scopedCase) {
	t.Helper()
	wsRoot := t.TempDir()
	repoRoot := filepath.Join(wsRoot, "repo-a")
...
func wantKind(t *testing.T, err error, kind apperrors.ErrorKind) {
	t.Helper()
	if err == nil {
	...
	if !errors.As(err, &svcErr) {
		t.Fatalf("error = %T %v, want *apperrors.ServiceError", err, err)
	}
	if svcErr.Kind != kind {
...
func TestFileServiceImpl_ReadFileScoped_GitExplicitReadForbidden(t *testing.T) {
	...
	for _, path := range []string{".git/config", ".GIT/config", ".GiT/config"} {
		...
		wantKind(t, err, apperrors.KindForbidden)
	}
}
...
func TestFileServiceImpl_Scoped_UnsupportedScope(t *testing.T) {
	...
	wantKind(t, err, apperrors.KindValidation)
}
...
func TestFileServiceImpl_Scoped_WorkspaceNotCheckedOut(t *testing.T) {
	...
	wantKind(t, err, apperrors.KindNotFound)
}
...
func TestFileServiceImpl_PhaseA_CRUDAndVisibilityAllScopes(t *testing.T) {
	...
	for _, sc := range scopes {
		t.Run(sc.name, func(t *testing.T) {
			...
			wantKind(t, writeScopedFile(ctx, svc, "ws", sc.scope, sc.target, "", ".git/config", "mutated"), apperrors.KindForbidden)
			_, err = svc.ReadFileScoped(ctx, "ws", sc.scope, sc.target, "", ".GIT/config")
			wantKind(t, err, apperrors.KindForbidden)
			wantKind(t, svc.MkdirScoped(ctx, "ws", sc.scope, sc.target, "", ".git/refs/heads"), apperrors.KindForbidden)
			...
			wantKind(t, err, apperrors.KindConflict)
...
func TestFileServiceImpl_MutationsRejectScopeRootAliases(t *testing.T) {
	...
	for _, path := range []string{"", ".", "./", "dir/..", "dir/../."} {
		wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", path, true, "sha256:test"), apperrors.KindValidation)
		wantKind(t, svc.MkdirScoped(ctx, "ws", ScopeWorkspace, "", "", path), apperrors.KindValidation)
	}
...
func TestFileServiceImpl_ProtectsGitMetadataUnderAncestorMutation(t *testing.T) {
	...
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "checkout", true, "sha256:test"), apperrors.KindForbidden)
...
func TestMapGitInspectionErrorPreservesKinds(t *testing.T) {
	for _, tc := range []struct {
		kind string
		want apperrors.ErrorKind
	}{
		{kind: "timeout", want: apperrors.KindTimeout},
		{kind: "canceled", want: apperrors.KindTimeout},
		{kind: "validation", want: apperrors.KindValidation},
		{kind: "not_found", want: apperrors.KindNotFound},
		{kind: "failed", want: apperrors.KindInternal},
	} {
		err := mapGitInspectionError("git operation", fakeInspectionError{kind: tc.kind})
		serviceErr, ok := err.(*apperrors.ServiceError)
