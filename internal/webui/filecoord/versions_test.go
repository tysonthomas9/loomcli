...
func TestFileVersions_ReadAndStatUseStrongContentHash(t *testing.T) {
	...
	read, err := svc.ReadFileScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt")
	if err != nil {
		t.Fatal(err)
	}
	...
	stat, err := svc.StatPathScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt")
	if err != nil {
		t.Fatal(err)
	}
...
func TestFileVersions_DeleteRequiresMatchingVersion(t *testing.T) {
	...
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt", false, ""), apperrors.KindPreconditionRequired)
	version := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "file.txt")
	mustWrite(t, filepath.Join(root, "file.txt"), "two") // terminal/agent writer bypasses Loom's lock
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt", false, version), apperrors.KindPreconditionFailed)
	version = mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "file.txt")
	if err := svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt", false, version); err != nil {
		t.Fatal(err)
	}
}
...
func TestFileVersions_MoveRequiresSourceAndOverwriteDestinationVersions(t *testing.T) {
	...
	svc := scopedSvc(root)
	ctx := context.Background()
	sourceVersion := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "source.txt")
	destinationVersion := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "destination.txt")

	_, err := svc.MovePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "source.txt", "destination.txt", true, "", destinationVersion)
	wantKind(t, err, apperrors.KindPreconditionRequired)
	_, err = svc.MovePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "source.txt", "destination.txt", true, "sha256:stale", destinationVersion)
	wantKind(t, err, apperrors.KindPreconditionFailed)
	_, err = svc.MovePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "source.txt", "destination.txt", true, sourceVersion, "")
	wantKind(t, err, apperrors.KindPreconditionRequired)
	_, err = svc.MovePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "source.txt", "destination.txt", true, sourceVersion, "sha256:stale")
	wantKind(t, err, apperrors.KindPreconditionFailed)
	result, err := svc.MovePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "source.txt", "destination.txt", true, sourceVersion, destinationVersion)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFileVersions_DirectoryChildContentInvalidatesVersion(t *testing.T) {
	...
	svc := scopedSvc(root)
	ctx := context.Background()
	version := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "dir")
	...
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "dir", true, version), apperrors.KindPreconditionFailed)
}
...
func TestFileVersions_DirectorySymlinkTargetInvalidatesVersion(t *testing.T) {
	...
	svc := scopedSvc(root)
	ctx := context.Background()
	before := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "dir")
	...
	after := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "dir")
	if before == after {
...
func TestFileVersions_ConditionalWritesAndCreateRace(t *testing.T) {
	...
	svc := scopedSvc(root)
	ctx := context.Background()
	version := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "existing.txt")

	updated, err := svc.WriteFileConditionalScoped(ctx, "ws", ScopeWorkspace, "", "", "existing.txt", "two", FileWritePreconditions{IfMatch: version})
	if err != nil || updated.Version == version {
		t.Fatalf("conditional write = %+v, %v", updated, err)
	}
	_, err = svc.WriteFileConditionalScoped(ctx, "ws", ScopeWorkspace, "", "", "existing.txt", "stale", FileWritePreconditions{IfMatch: version})
	wantKind(t, err, apperrors.KindPreconditionFailed)
	if err := writeScopedFile(ctx, svc, "ws", ScopeWorkspace, "", "", "existing.txt", "lww"); err != nil {
	...
	for _, content := range []string{"first", "second"} {
		go func(content string) {
			<-start
			_, err := svc.WriteFileConditionalScoped(ctx, "ws", ScopeWorkspace, "", "", "created.txt", content, FileWritePreconditions{IfNoneMatch: true})
			errs <- err
		}(content)
	}
	close(start)
	...
	for range 2 {
		...
		if errors.As(err, &serviceErr) && serviceErr.Kind == apperrors.KindPreconditionFailed {
			failed++
			continue
		}
...
func TestFileVersions_TwoBrowserWritersWithSameVersion(t *testing.T) {
	...
	for _, content := range []string{"first", "second"} {
		go func(content string) {
			<-start
			_, err := svc.WriteFileConditionalScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt", content, FileWritePreconditions{IfMatch: version})
			errs <- err
		}(content)
	}
	close(start)
...
func TestFileVersions_CaseAliasWritersSerialize(t *testing.T) {
	...
	svc := scopedSvc(root)
	ctx := context.Background()
	version := mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "Mixed.txt")
	start := make(chan struct{})
	errs := make(chan error, 2)
	...
	for i, path := range paths {
		go func(path, content string) {
			<-start
			_, err := svc.WriteFileConditionalScoped(ctx, "ws", ScopeWorkspace, "", "", path, content, FileWritePreconditions{IfMatch: version})
			errs <- err
...
func TestDirectoryManifestBoundsFailClosed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantKind(t, err, apperrors.KindPayloadTooLarge)
		})
	}
}
