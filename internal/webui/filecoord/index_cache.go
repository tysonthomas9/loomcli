...
func (c *fileIndexCache) get(root string, allowSensitive bool) (*FileIndexResult, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *fileIndexCache) put(root string, allowSensitive bool, result *FileIndexResult) {
	c.putAtGeneration(root, allowSensitive, result, nil)
}

func (c *fileIndexCache) putIfGeneration(root string, allowSensitive bool, result *FileIndexResult, generation uint64) {
	c.putAtGeneration(root, allowSensitive, result, &generation)
}

func (c *fileIndexCache) putAtGeneration(root string, allowSensitive bool, result *FileIndexResult, expectedGeneration *uint64) {
	if result == nil {
		return
	}
...
func cloneFileIndexResult(result *FileIndexResult) *FileIndexResult {
	if result == nil {
		return nil
	}
	return &FileIndexResult{
		...
		PartialReasons: append(make([]FilePartialReason, 0, len(result.PartialReasons)), result.PartialReasons...),
	}
}

func estimateFileIndexSize(result *FileIndexResult) int64 {
