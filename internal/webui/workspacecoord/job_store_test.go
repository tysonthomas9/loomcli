...
func TestWorkspaceJobRegistry_StartReturnsWorkspaceKeyForClone(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
	...
	id := store.Start(WorkspaceCreateRequest{Name: "clone_ws", Type: "clone"}, createFn)
	if id != "CLONE-WS" {
		t.Fatalf("job id = %q, want CLONE-WS", id)
	}
}

func TestWorkspaceJobRegistry_GetRunningJob(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
...
func TestWorkspaceJobRegistry_StartAddReposRunsOutsideRequestAndUsesOpaqueJobID(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()

	started := make(chan struct{})
	proceed := make(chan struct{})
	addReposFn := func(ctx context.Context, req WorkspaceAddReposRequest) (WorkspaceCreateResult, error) {
	...
	id := store.StartAddRepos(WorkspaceAddReposRequest{
		WorkspaceID: "ALPHA",
	...
	for {
		job = store.Get(id)
		if job != nil && job.Status == JobStatusDone {
			break
		}
...
func TestWorkspaceJobRegistry_StartAddReposUsesAttachmentFailureMessage(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()

	done := make(chan struct{})
	id := store.StartAddRepos(
		WorkspaceAddReposRequest{WorkspaceID: "ALPHA"},
		func(context.Context, WorkspaceAddReposRequest) (WorkspaceCreateResult, error) {
			defer close(done)
			return WorkspaceCreateResult{}, fmt.Errorf("unclassified clone failure")
		},
	)
	<-done
...
func TestWorkspaceJobRegistry_StartPreparedSuppressesDuplicateRunningRunner(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
	...
	req := WorkspaceCreateRequest{Name: "clone_ws", Type: "clone"}
	...
	close(proceed)
	waitForWorkspaceJobStatus(t, store, "admission-create-1", JobStatusDone)
}

func TestWorkspaceJobRegistry_StartPreparedAllowsRetryAfterTerminalResult(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()

	var calls atomic.Int32
	req := WorkspaceCreateRequest{Name: "clone_ws", Type: "clone"}
	first := func(context.Context, WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
		calls.Add(1)
		return WorkspaceCreateResult{}, fmt.Errorf("first attempt failed")
	}
	store.StartPrepared("admission-create-retry", req, first)
	waitForWorkspaceJobStatus(t, store, "admission-create-retry", JobStatusFailed)

	second := func(context.Context, WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
		calls.Add(1)
		return WorkspaceCreateResult{WorkspaceID: "CLONE-WS"}, nil
	}
	...
	job := waitForWorkspaceJobStatus(t, store, "admission-create-retry", JobStatusDone)
...
func TestWorkspaceJobRegistry_GetCompletedJob(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
...
func TestWorkspaceJobRegistry_GetFailedJob(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
...
func TestWorkspaceJobRegistry_GetUnknownID(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
...
func TestWorkspaceJobRegistry_StopIdempotent(t *testing.T) {
	store := NewWorkspaceJobRegistry()
...
func TestWorkspaceJobRegistry_ConcurrentStopCancelsActiveJobAndWaitsForUnwind(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	...
	id := store.StartPrepared(
		"shutdown-active-job",
		WorkspaceCreateRequest{Name: "shutdown", Type: "clone"},
		func(ctx context.Context, _ WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
			close(started)
			<-ctx.Done()
			close(cancelled)
			<-release
			...
			return WorkspaceCreateResult{WorkspaceID: "TOO-LATE"}, nil
		},
	)
	...
	job = store.Get(id)
	if job == nil || job.Status != JobStatusFailed || job.Error != jobShutdownError {
...
func TestWorkspaceJobRegistry_StopWaitIsBoundedWhenJobIgnoresCancellation(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	...
	id := store.StartPrepared(
		...
		WorkspaceCreateRequest{Name: "shutdown", Type: "clone"},
		func(context.Context, WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
			close(started)
			<-release
			close(finished)
			return WorkspaceCreateResult{WorkspaceID: "TOO-LATE"}, nil
	...
	job = waitForWorkspaceJobStatus(t, store, id, JobStatusFailed)
	if job.Error != jobShutdownError || job.WorkspaceID != "" {
		t.Fatalf("late success replaced bounded Stop snapshot: %+v", job)
	}
}

func TestWorkspaceJobRegistry_StartAfterStopDoesNotLaunch(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	store.Stop()
	...
	id := store.StartPrepared(
		...
		func(context.Context, WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
			calls.Add(1)
			return WorkspaceCreateResult{WorkspaceID: "MUST-NOT-RUN"}, nil
		},
	)
	...
	store.StartPrepared(
		completed.ID,
		WorkspaceCreateRequest{Name: "retry", Type: "clone"},
		func(context.Context, WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
			calls.Add(1)
			return WorkspaceCreateResult{WorkspaceID: "MUST-NOT-RETRY"}, nil
		},
	)
	if got := calls.Load(); got != 0 {
...
func TestWorkspaceJobRegistry_StartStopRaceIsSafe(t *testing.T) {
	for iteration := range 64 {
		...
		go func() {
			...
			store.StartPrepared(
				id,
				WorkspaceCreateRequest{Name: "race", Type: "clone"},
				func(ctx context.Context, _ WorkspaceCreateRequest) (WorkspaceCreateResult, error) {
					calls.Add(1)
					<-ctx.Done()
...
func TestWorkspaceJobRegistry_EvictExpired(t *testing.T) {
	store := NewWorkspaceJobRegistry()
	defer store.Stop()
...
func waitForWorkspaceJobInactive(t *testing.T, store *WorkspaceJobRegistry, id string) {
	t.Helper()
	deadline := time.Now().Add(time.Second)
	for {
