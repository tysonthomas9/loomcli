...
func (hd *HealthDoctor) checkWorkspace(ctx context.Context, wsID string, pp *daemon.ProtectedPool) {
	...
	case circuitbreaker.StateOpen, circuitbreaker.StateHalfOpen:
		...
		// First time seeing this workspace unhealthy.
		if w.unhealthySince.IsZero() {
		...
		// Reset breaker after successful restart so requests flow immediately.
		pp.ResetBreaker()
		w.lastRestart = now
		w.restartCount++
		w.unhealthySince = time.Time{} // will re-check on next tick
...
func (hd *HealthDoctor) restartDaemon(ctx context.Context, wsID string) error {
	...
	stop.Stdout = nil
	stop.Stderr = nil
	_ = stop.Run() // best-effort; may already be dead
	...
	if err := start.Run(); err != nil {
		// bd daemon start may return non-zero if already running; continue to poll.
