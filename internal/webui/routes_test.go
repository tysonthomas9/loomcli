...
func TestSetupRoutes_FlatTerminalRoutesReturn404(t *testing.T) {
	...
	mux := http.NewServeMux()
	(&serverApp{termMgr: termMgr}).setupRoutes(mux)
...
func TestSetupRoutes_FleetEndpointsNotRegisteredWhenDisabled(t *testing.T) {
	mux := http.NewServeMux()
	(&serverApp{}).setupRoutes(mux) // fleetEnabled=false
...
func TestSetupRoutes_FlatFleetRoutesReturn404(t *testing.T) {
	mux := http.NewServeMux()
	(&serverApp{}).setupRoutes(mux) // fleetEnabled=true
...
func TestSetupRoutes_SSEEndpointRegisteredOnWorkspaceScope(t *testing.T) {
	...
	(&serverApp{multiPool: multiPool, hub: hub, wsExistsFn: wsExistsFn}).setupRoutes(mux)
...
func TestSetupRoutes_WorkspaceBackendPatchEndpoint(t *testing.T) {
	...
	(&serverApp{multiPool: multiPool, config: ServerConfig{WorkspaceConfigFn: workspaceConfigFn}, wsExistsFn: wsExistsFn}).setupRoutes(mux)
...
func TestSetupRoutes_FlatTerminalTokenReturns404(t *testing.T) {
	...
	mux := http.NewServeMux()
	(&serverApp{termMgr: termMgr, termAuth: termAuth}).setupRoutes(mux)
...
func TestSetupRoutes_IssueEndpoints_MethodRestrictions(t *testing.T) {
	...
	mux := http.NewServeMux()
	(&serverApp{pool: pool}).setupRoutes(mux)
...
func TestSetupRoutes_DevMode_FrontendHandler(t *testing.T) {
	...
	(&serverApp{config: ServerConfig{DevMode: true, DevFrontendDir: "/nonexistent/dev/dir"}}).setupRoutes(mux)
...
func TestSetupRoutes_LoomProxy_RegisteredWhenURLSet(t *testing.T) {
	...
	mux := http.NewServeMux()
	(&serverApp{config: ServerConfig{LoomServerURL: "http://localhost:9999"}}).setupRoutes(mux)
...
func TestSetupRoutes_LoomProxy_NotRegisteredWhenURLInvalid(t *testing.T) {
	...
	(&serverApp{config: ServerConfig{LoomServerURL: "http://external-host.example.com:9999"}}).setupRoutes(mux)
...
func TestLegacyFlatAgentRoutesRemoved(t *testing.T) {
	...
	mux := http.NewServeMux()
	(&serverApp{multiPool: multiPool, config: ServerConfig{GitOps: gitOps, FileOps: fileOps}, wsExistsFn: wsExistsFn}).setupRoutes(mux)
