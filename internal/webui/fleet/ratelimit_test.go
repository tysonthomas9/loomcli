...
func TestRateLimiter_WindowExpiry_AllowedAgain(t *testing.T) {
	mr := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	defer client.Close()
...
// TestRateLimiter_Close tests that Close closes the underlying Redis client.
...
// TestRateLimiter_Close_NilClient tests that Close with nil client returns nil.
func TestRateLimiter_Close_NilClient(t *testing.T) {
	rl := &RateLimiter{client: nil, limit: 5, window: time.Minute}
	if err := rl.Close(); err != nil {
