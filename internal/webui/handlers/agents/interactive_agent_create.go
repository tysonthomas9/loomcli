...
type canonicalInteractiveAgentInput struct {
	Name             string
	RoleName         string
	Kind             string
	Prompt           string
...
func writeCanonicalInteractiveAgentResponse(
	w http.ResponseWriter,
	agentID string,
	input canonicalInteractiveAgentInput,
) {
	handler.WriteJSON(w, http.StatusCreated, loomapi.CreatedInteractiveAgent{
		Id: agentID, Name: agentID, Kind: loomapi.CreatedInteractiveAgentKindInteractive,
		RoleName: input.RoleName, Backend: optionalAgentRecordString(input.Backend),
...
func parseCanonicalInteractiveAgentCreate(
	body []byte,
	workspace string,
) (canonicalInteractiveAgentInput, string) {
	var request loomapi.CreateInteractiveAgentRequest
	if err := handler.DecodeOneJSONBytes(body, &request, handler.JSONDecodeOptions{
		MaxBytes:              handler.MaxRequestBody,
		DisallowUnknownFields: true,
	}); err != nil {
		return canonicalInteractiveAgentInput{}, "invalid request body"
	}
	workspaceKey := optionalAgentRecordStringValue(request.WorkspaceKey)
	if workspaceKey != "" && strings.TrimSpace(workspaceKey) != workspace {
		return canonicalInteractiveAgentInput{}, "workspace_key must match request workspace"
	}
	input := canonicalInteractiveAgentInput{
		Name:             strings.ToLower(strings.TrimSpace(request.Name)),
		RoleName:         normalizeInteractiveRoleName(request.RoleName),
		Kind:             strings.ToLower(strings.TrimSpace(stringValueFromInteractiveKind(request.Kind))),
		Prompt:           strings.TrimSpace(optionalAgentRecordStringValue(request.Prompt)),
		PromptFile:       strings.TrimSpace(optionalAgentRecordStringValue(request.PromptFile)),
		Backend:          strings.TrimSpace(optionalAgentRecordStringValue(request.Backend)),
		FallbackBackends: optionalAgentRecordStringsValue(request.FallbackBackends),
		Repos:            optionalAgentRecordStringsValue(request.Repos),
		RepoGroups:       optionalAgentRecordStringsValue(request.RepoGroups),
		CrossRepo:        optionalAgentRecordBoolValue(request.CrossRepo),
	}
