...
func (m *Module) execTask(ctx context.Context, ws string, id driverIdentity, body []byte) (any, error) {
	...
	executor := driverpkg.HostBridgeTaskExecutor{
		...
		WorktreeResolver: driverpkg.LocalTaskWorktreeResolver{Store: m.store, Lineage: driverpkg.DefaultStackLineageLookup(m.store)},
		StackStore:       driverpkg.DefaultStackStore(m.store),
	}
