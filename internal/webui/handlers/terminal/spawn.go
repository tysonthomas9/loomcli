...
func HandleTerminalSpawn(svc service.TerminalService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			...
			if errors.As(err, &maxBytesErr) {
				handler.WriteJSON(w, http.StatusRequestEntityTooLarge, terminalSpawnResponse{
					Error: "request body too large",
				})
				return
			}
			handler.WriteJSON(w, http.StatusBadRequest, terminalSpawnResponse{
		...
		handler.WriteJSON(w, http.StatusOK, terminalSpawnResponse{
			Success: true,
