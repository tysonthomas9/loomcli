...
// must see these as distinct, correctly-derived route keys).
func TestGithubNormalizePullRequestActions(t *testing.T) {
	...
	for _, action := range []string{"opened", "synchronize", "reopened", "ready_for_review", "closed"} {
		t.Run(action, func(t *testing.T) {
			body := fmt.Appendf(nil,
				`{"action":%q,"pull_request":{"number":42,"head":{"sha":"abc123def456"},"base":{"ref":"main"}},"repository":{"full_name":"acme/widgets"},"sender":{"login":"octocat"}}`,
				action)
			got, err := githubAdapter{}.Normalize(githubRequest(t, "pull_request", "d-"+action, body), body)
			if err != nil {
				t.Fatalf("Normalize: %v", err)
			}
			if want := "github.pull_request." + action; got.RouteKey != want {
...
func TestGithubSubjectAttrs(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			want:  map[string]string{"pr_number": "3", "head_sha": "abc", "base_ref": "dev"},
		},
		{
