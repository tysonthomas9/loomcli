...
func TestRecoveryHTTPUsesCapturedSourceAndPreservesDocument(t *testing.T) {
	...
	raw := []byte(`{ "manifest":"fleet.issue-workspace.v6", "workspace":"WS", "through":"c2.MS0w", "issues":[], "total":0, "ready":[], "blocked":[], "deferred":[], "dependencies":[], "comments":[], "history":null, "future":{"keep":"verbatim"} }`)
	...
	var selected backend.IssueRecoveryBackend = httpRecoverySource(func(ctx context.Context) (backend.IssueRecoverySnapshot, error) {
		...
		return backend.IssueRecoverySnapshot{SourceIdentity: "s1.Zml4dHVyZQ", Manifest: "fleet.issue-workspace.v6", Workspace: "WS", Through: "c2.MS0w", Document: raw}, nil
	})
...
func TestRecoveryHTTPCanceledReadCannotPublishSuccess(t *testing.T) {
	...
	handle, err := registry.Register("user", "WS", nil, httpRecoverySource(func(context.Context) (backend.IssueRecoverySnapshot, error) {
		cancel()
		return backend.IssueRecoverySnapshot{SourceIdentity: "s1.Zml4dHVyZQ", Manifest: "fleet.issue-workspace.v6", Workspace: "WS", Through: "c2.MS0w", Document: []byte(`{"secret":"late success"}`)}, nil
	}), "s1.Zml4dHVyZQ")
	require.NoError(t, err)
	req := httpRecoveryRequest("user", "WS", handle.Handle)
