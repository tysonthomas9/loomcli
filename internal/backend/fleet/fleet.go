...
var _ backend.EventHistoryBackend = (*FleetBackend)(nil)
...
// fix, moved one layer up — `design` lands, `design_format` is dropped, and the
// caller is told the whole update succeeded.
func (b *FleetBackend) applyTolerantPatch(ctx context.Context, id string, req map[string]interface{}, actor string) error {
	dropped, err := b.patchIssueTolerantly(ctx, id, req, actor)
	if err != nil {
		return err
	}
	if len(dropped) > 0 {
...
func (b *FleetBackend) Update(ctx context.Context, id string, params backend.UpdateParams) error {
	...
	if len(req) > 0 {
		if err := b.applyTolerantPatch(ctx, id, req, params.Actor); err != nil {
			return err
		}
		handled = true
	}
