...
func eventDataFromHistory(history []fleetEventWire, issueID string) []backend.EventData {
	...
	for _, event := range history {
		values := fleetHistoryValues(event)
...
func fleetHistoryValues(e fleetEventWire) fleetHistoryValuesResult {
...
func valuesFromChanges(changes []backend.FieldChange) (*string, *string, *string, []string, int) {
...
func fieldsFromChanges(changes []backend.FieldChange) []string {
	if len(changes) == 0 {
		return nil
	}
