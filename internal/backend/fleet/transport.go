...
// FleetBackend instance that does not supply its own *http.Client. It is
...
// HTTP client for FleetBackend instances when Config.HTTPClient is nil.
// It is backed by sharedTransport (so connection reuse is shared across
// every FleetBackend in the process) and uses a Timeout that is decoupled
