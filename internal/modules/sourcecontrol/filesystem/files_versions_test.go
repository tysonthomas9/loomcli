...
func TestFileVersions_DeleteRequiresMatchingVersion(t *testing.T) {
	...
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt", false, ""), failurePreconditionRequired)
	...
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "file.txt", false, version), failurePreconditionFailed)
	version = mustScopedVersion(t, svc, ctx, ScopeWorkspace, "", "file.txt")
...
func TestFileVersions_DirectoryChildContentInvalidatesVersion(t *testing.T) {
	...
	wantKind(t, svc.DeletePathVersionedScoped(ctx, "ws", ScopeWorkspace, "", "", "dir", true, version), failurePreconditionFailed)
}
...
func TestFileVersions_ConditionalWritesAndCreateRace(t *testing.T) {
	...
	for range 2 {
		...
		if errors.As(err, &serviceErr) && serviceErr.Kind == failurePreconditionFailed {
			failed++
			continue
		}
...
func TestDirectoryManifestBoundsFailClosed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantKind(t, err, failurePayloadTooLarge)
		})
	}
}
