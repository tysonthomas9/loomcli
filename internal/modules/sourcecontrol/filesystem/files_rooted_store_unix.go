...
func openRootedPlatform(rootPath string, expected os.FileInfo) (rootedPlatform, error) {
	...
	if err != nil {
		_ = root.Close()
		return nil, newInternal("failed to verify Unix scope root", err)
	}
	if !os.SameFile(expected, info) {
		_ = root.Close()
		return nil, newForbidden("scope root changed while opening Unix descriptor")
	}
...
func (p *unixRootedPlatform) Stat(name string) (os.FileInfo, error) {
	...
	info, err := f.Stat()
	if err != nil {
		return nil, newInternal("failed to stat rooted path", err)
	}
	return info, nil
}
...
func (p *unixRootedPlatform) List(name string) ([]os.DirEntry, error) {
	...
	if err != nil {
		return nil, newInternal("failed to read rooted directory", err)
	}
...
func (p *unixRootedPlatform) Read(name string, limit int64) ([]byte, os.FileInfo, bool, error) {
	...
	info, err := f.Stat()
	if err != nil {
		return nil, nil, false, newInternal("failed to stat rooted file", err)
	}
	if info.IsDir() {
...
func (p *unixRootedPlatform) Hash(name string) ([]byte, os.FileInfo, error) {
	...
	info, err := f.Stat()
	if err != nil {
		return nil, nil, newInternal("failed to stat rooted file", err)
	}
	if !info.Mode().IsRegular() {
	...
	if _, err := io.Copy(hash, f); err != nil {
		return nil, nil, newInternal("failed to hash rooted file", err)
	}
...
func (p *unixRootedPlatform) ReadVersioned(name string, limit int64) ([]byte, os.FileInfo, bool, []byte, error) {
	...
	info, err := f.Stat()
	if err != nil {
		return nil, nil, false, nil, newInternal("failed to stat rooted file", err)
	}
	if !info.Mode().IsRegular() {
...
func (p *unixRootedPlatform) Readlink(name string) (string, error) {
	...
	if n == len(buffer) {
		return "", newPayloadTooLarge("symlink target exceeds limit")
	}
...
func readRootedFile(f *os.File, info os.FileInfo, limit int64) ([]byte, os.FileInfo, bool, error) {
	...
	if err != nil {
		return nil, nil, false, newInternal("failed to read rooted file", err)
	}
...
func (p *unixRootedPlatform) openParent(name string) (int, string, error) {
	...
	if len(parts) == 0 || name == "." {
		return -1, "", newInvalid("path has no parent component")
	}
	fd, err := unix.Dup(int(p.root.Fd()))
	if err != nil {
		return -1, "", newInternal("failed to duplicate scope descriptor", err)
	}
...
func unixPathError(action string, err error) error {
	...
	case errors.Is(err, unix.EACCES), errors.Is(err, unix.EPERM):
		return newForbidden("filesystem access denied")
	case errors.Is(err, unix.ENXIO), errors.Is(err, unix.ENODEV):
		...
		return newInvalid("path is not a regular file")
	default:
		return newInternal(action, err)
	}
}
...
func writeUnixTemp(temp *os.File, content []byte, perm os.FileMode) error {
	...
	if err := temp.Chmod(perm); err != nil {
		return newInternal("failed to set temporary file permissions", err)
	}
...
func (p *unixRootedPlatform) MkdirAll(name string) error {
	...
	fd, err := unix.Dup(int(p.root.Fd()))
	if err != nil {
		return newInternal("failed to duplicate scope descriptor", err)
	}
