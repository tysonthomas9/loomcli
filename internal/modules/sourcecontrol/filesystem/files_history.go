...
func (s *fileServiceImpl) ReadFileAtRevScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path, rev string) (*FileReadResult, error) {
	return s.readFileAtRevScoped(ctx, wsID, scope, target, repo, path, rev, fileAccess{})
}

func (s *fileServiceImpl) readFileAtRevScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path, rev string, access fileAccess) (*FileReadResult, error) {
	if err := requireSensitiveFileAccess(access, path); err != nil {
		return nil, err
	}
...
func (s *fileServiceImpl) DiffFileScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path, from, to string) (*FileDiffResult, error) {
	return s.diffFileScoped(ctx, wsID, scope, target, repo, path, from, to, fileAccess{})
}

func (s *fileServiceImpl) diffFileScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path, from, to string, access fileAccess) (*FileDiffResult, error) {
	if err := requireSensitiveFileAccess(access, path); err != nil {
		return nil, err
	}
	root, cleanPath, checkout, err := s.resolveScopedContainingCheckout(wsID, scope, target, repo, path)
...
func (s *fileServiceImpl) BlameFileScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path string) (*FileBlameResult, error) {
	return s.blameFileScoped(ctx, wsID, scope, target, repo, path, fileAccess{})
}

func (s *fileServiceImpl) blameFileScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path string, access fileAccess) (*FileBlameResult, error) {
	if err := requireSensitiveFileAccess(access, path); err != nil {
		return nil, err
	}
	root, cleanPath, checkout, err := s.resolveScopedContainingCheckout(wsID, scope, target, repo, path)
...
func (s *fileServiceImpl) HistoryFileScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path string) (*FileHistoryResult, error) {
	return s.historyFileScoped(ctx, wsID, scope, target, repo, path, fileAccess{})
}

func (s *fileServiceImpl) historyFileScoped(ctx context.Context, wsID string, scope FileScope, target, repo, path string, access fileAccess) (*FileHistoryResult, error) {
	if err := requireSensitiveFileAccess(access, path); err != nil {
		return nil, err
	}
	root, cleanPath, checkout, err := s.resolveScopedContainingCheckout(wsID, scope, target, repo, path)
...
func mapGitInspectionError(operation string, err error) error {
	...
	if errors.As(err, &inspectionErr) {
		...
		case "timeout", "canceled":
			return newTimeout(operation)
		case "validation":
			return newInvalid(err.Error())
		case "not_found":
			return newNotFound(operation)
		}
	}
	return newInternal(operation, err)
}
...
func (s *fileServiceImpl) resolveScopedContainingCheckout(wsID string, scope FileScope, target, repo, path string) (*scopedRoot, string, fileCheckoutRef, error) {
	...
	case ScopeRepo, ScopeAgent:
		...
		if err != nil || repoRel == "." || strings.HasPrefix(repoRel, ".."+string(filepath.Separator)) || filepath.IsAbs(repoRel) {
			root.Close()
			return nil, "", fileCheckoutRef{}, newForbidden("path outside git checkout")
		}
	...
	default:
		root.Close()
		return nil, "", fileCheckoutRef{}, newInvalid("unsupported scope " + string(scope))
	}
...
func resolveContainingCheckout(root *scopedRoot, relPath string) (fileCheckoutRef, error) {
	...
	for current := start; ; current = filepath.Dir(current) {
		...
		if err := validateGitCheckoutRoot(currentAbs); err == nil {
			...
			if relErr != nil || repoRel == "." || strings.HasPrefix(repoRel, ".."+string(filepath.Separator)) || filepath.IsAbs(repoRel) {
				return fileCheckoutRef{}, newForbidden("path outside git checkout")
			}
	...
	return fileCheckoutRef{}, newNotFound("containing git checkout not found")
}
