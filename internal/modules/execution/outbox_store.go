...
// implementations derive it with TaskRunEventID(TaskRunID,
// Attempt, Type).
...
type TaskRunEventStore interface {
	...
	Append(ctx context.Context, in TaskRunEventAppend) (*TaskRunJournalEvent, error)

	// ListSince returns events for the workspace matching the filter,
	// ordered by ascending Seq.
	ListSince(ctx context.Context, workspaceKey string, filter TaskRunEventFilter) ([]*TaskRunJournalEvent, error)
}

// OutboxCreate is the input to OutboxStore.Create. New records start in
// OutboxDeliveryStatusPending with Attempt 0.
...
type OutboxStore interface {
	...
	Create(ctx context.Context, in OutboxCreate) (*OutboxDelivery, error)

	// ListDue returns pending records eligible for a delivery attempt at
	// filter.Now, ordered by ascending Seq.
	ListDue(ctx context.Context, workspaceKey string, filter OutboxDueFilter) ([]*OutboxDelivery, error)

	// MarkResult records the outcome of a delivery attempt.
	MarkResult(ctx context.Context, workspaceKey, outboxID string, update OutboxDeliveryUpdate) (*OutboxDelivery, error)

	// Get returns a single record by ID.
	Get(ctx context.Context, workspaceKey, outboxID string) (*OutboxDelivery, error)
}
