...
// executionstore.AwaitStore.RegisterAwaitAndCheck itself: a single transactional
...
var (
	...
	ErrAwaitPatternUnscoped = fmt.Errorf(
		"execution: %s: await pattern must be a rendered subject-scoped key: %w",
	...
	ErrAwaitTimeoutRequired = fmt.Errorf(
		"execution: %s: await deadline is mandatory and must be in the future: %w",
	...
	ErrAwaitInstanceKeyMalformed = fmt.Errorf(
		"execution: %s: await instance key must be runID#await-{n} with n >= 1: %w",
...
var (
	...
	ErrAwaitActorForbidden = errors.New("execution: await actor not eligible to resolve")
)
