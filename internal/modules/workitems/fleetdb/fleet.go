// Package fleetdb implements Work Items durable ports as a FleetDB HTTP adapter
...
var _ workitems.Store = (*Adapter)(nil)
var _ workitems.ReadyQueries = (*Adapter)(nil)
var _ workitems.DeferredQueries = (*Adapter)(nil)
...
var _ workitems.MutationStream = (*Adapter)(nil)
var _ workitems.ClaimLeaseCommands = (*Adapter)(nil)
...
// New creates a Adapter with the given configuration.
func New(cfg Config) (*Adapter, error) {
	...
	if httpClient == nil {
		// Use the shared transport-backed client so every Adapter in
...
func (b *Adapter) doRequestURL(ctx context.Context, method, rawURL string, body interface{}) (*apiResponse, int, error) {
	b.mu.RLock()
...
func (b *Adapter) SetAuthToken(token string) {
	b.mu.Lock()
	b.authToken = token
	b.mu.Unlock()
}
...
// Safe to call concurrently with Work Items calls.
func (b *Adapter) SetAPIKey(key string) {
	b.mu.Lock()
	b.apiKey = key
	b.mu.Unlock()
}

func (b *Adapter) BackendName() string { return "fleet" }
...
func (b *Adapter) doRequestAsActor(ctx context.Context, path string, body interface{}, actor string) (*apiResponse, int, error) {
	b.mu.RLock()
	auth := fleethttp.Auth{BearerToken: b.authToken, APIKey: b.apiKey, Actor: b.actor}
	b.mu.RUnlock()
	if actor != "" {
...
func (b *Adapter) execURL(ctx context.Context, op, method, rawURL string, body interface{}) (*apiResponse, error) {
...
func (b *Adapter) execAsActor(ctx context.Context, op, path string, body interface{}, actor string) error {
...
func (b *Adapter) List(ctx context.Context, opts workitems.ListFilter) ([]workitems.IssueSummary, error) {
...
func (b *Adapter) createIssueOnce(ctx context.Context, params workitems.CreateCommand) (*workitems.IssueSummary, error) {
...
func (b *Adapter) Patch(ctx context.Context, params workitems.PatchCommand) error {
	id := params.IssueID
	if id == "" {
...
func (b *Adapter) applyStatusUpdate(ctx context.Context, id string, params workitems.PatchCommand) (bool, error) {
	if params.Status == nil {
		return false, nil
	}
...
func (b *Adapter) ReleaseClaim(ctx context.Context, id, actor string) error {
	if id == "" {
...
func (b *Adapter) transitionToBlockedOrReview(ctx context.Context, id, target string, current *workitems.IssueDetail) error {
...
func (b *Adapter) transitionToOpen(ctx context.Context, id string, current *workitems.IssueDetail, clearAssignee bool) error {
	if current == nil {
...
func (b *Adapter) claimActor(assignee *string, current *workitems.IssueDetail) string {
...
func (b *Adapter) assignIssue(ctx context.Context, id, assignee string) error {
	body := struct {
...
func (b *Adapter) releaseClaim(ctx context.Context, id string) error {
	return b.assignIssue(ctx, id, "")
}

func (b *Adapter) deferIssue(ctx context.Context, id string, until time.Time) error {
	var body interface{}
	if !until.IsZero() {
		body = struct {
