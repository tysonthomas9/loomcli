...
func OrchestrationSessionFor(ctx context.Context, s OrchestrationSessionStore, workspaceKey, agentID string) (*SessionRecord, error) {
...
func mostRecentActiveSession(sessions []*SessionRecord) *SessionRecord {
...
func activeInteractiveSession(sess *SessionRecord) bool {
	if sess == nil || sess.FinishedAt != nil {
		return false
	}
	switch sess.Status {
	case "", SessionRecordLeased, SessionRecordStarting, SessionRecordRunning, SessionRecordIdle, SessionRecordYielded:
		return true
	default:
		return false
	}
