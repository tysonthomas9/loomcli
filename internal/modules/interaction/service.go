...
func (service *Service) ListSessions(ctx context.Context, query SessionArchiveQuery) ([]*AgentSession, error) {
	...
	values, err := service.sessions.List(ctx, query)
	if err != nil {
		return nil, err
	}
	...
	for _, session := range values {
		if session == nil || session.WorkspaceKey != query.WorkspaceKey ||
			(query.AgentID != "" && session.AgentID != query.AgentID) ||
			(query.WorkItemID != "" && session.TaskID != query.WorkItemID) {
			return nil, ErrInvalidPersistedState
		}
		if err := validateQueriedSession(session, query.WorkspaceKey, session.SessionID); err != nil {
			return nil, err
		}
