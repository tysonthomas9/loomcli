...
func stubHarnessesOnPath(t *testing.T, binaries ...string) {
	...
	t.Cleanup(ResetHarnessVersionCache)
...
// writeAgentProfile materializes a profile root under the workspace's
...
func writeAgentProfile(t *testing.T, runtimeDir, agent, version string, files map[string]string) string {
	t.Helper()
	return writeAgentHarnessProfile(t, runtimeDir, agent, "claude", version, files)
}

// writeAgentHarnessProfile is writeAgentProfile for a named harness root.
func writeAgentHarnessProfile(t *testing.T, runtimeDir, agent, harness, version string, files map[string]string) string {
	t.Helper()
...
func TestVerifyProfile_UnsetConfigDirIsNotVerified(t *testing.T) {
	// No stub on PATH: an unprofiled lead must not even probe the harness.
	if err := verifyProfile(t.TempDir(), "", "claude"); err != nil {
		t.Fatalf("unprofiled lead must stay silent, got %v", err)
	}
}

func TestVerifyProfile_ConfigDirOutsideProfileRootIsNotVerified(t *testing.T) {
	...
	if err := verifyProfile(runtimeDir, outside, "claude"); err != nil {
	...
	if err := verifyProfile(runtimeDir, root, "claude"); err != nil {
		t.Fatalf("profile root itself must stay silent, got %v", err)
	}
}

func TestVerifyProfile_DriftedProfileRefusesWithDoctorRepair(t *testing.T) {
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	dir := writeAgentProfile(t, runtimeDir, "lead", "2.1.235 (Claude Code)", map[string]string{
		"settings.json": `{"model":"opus"}`,
	})

	err := verifyProfile(runtimeDir, dir, "claude")
	...
	if got := Repair(err, dir); got != "loom doctor --fix" {
		t.Fatalf("drift repair = %q, want the doctor re-bless", got)
	}
}

func TestVerifyProfile_TamperedProfileRefusesWithProvisionRepair(t *testing.T) {
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	dir := writeAgentProfile(t, runtimeDir, "lead", fakeHarnessVersion, map[string]string{
		"settings.json": `{"model":"opus"}`,
	})
	...
	if got := Repair(err, dir); got != "scripts/provision-profile.sh lead" {
		t.Fatalf("content repair = %q, want the provisioner named for this agent", got)
	}
}

func TestVerifyProfile_VerifyingProfileProceeds(t *testing.T) {
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	dir := writeAgentProfile(t, runtimeDir, "lead", fakeHarnessVersion, map[string]string{
	...
	if err := verifyProfile(runtimeDir, dir, "claude"); err != nil {
		t.Fatalf("verifying profile must proceed, got %v", err)
	}
}

func TestVerifyProfile_UnprovisionedProfileRefuses(t *testing.T) {
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
...
func TestVerifyProfile_EmptyRuntimeDirVerifiesNothing(t *testing.T) {
	if err := verifyProfile("", "/somewhere/.loom/agent-profiles/lead/claude", "claude"); err != nil {
...
func clearProfileEnv(t *testing.T) {
	...
	for _, harness := range ProfileHarnesses() {
		t.Setenv(ProfileEnvVar(harness), "")
		if err := os.Unsetenv(ProfileEnvVar(harness)); err != nil {
			t.Fatal(err)
		}
	}
}

func TestApply_InjectsClaudeConfigDirWhenUnset(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	dir := writeAgentProfile(t, runtimeDir, "lead", fakeHarnessVersion, map[string]string{
	...
	if _, err := Apply(runtimeDir, "lead", "claude"); err != nil {
...
func TestApply_InjectsCodexHomeWhenUnset(t *testing.T) {
	clearProfileEnv(t)
	stubHarnessesOnPath(t, "codex")
	runtimeDir := t.TempDir()
	dir := writeAgentHarnessProfile(t, runtimeDir, "lead", "codex", fakeHarnessVersion, map[string]string{
		"config.toml": "model = \"gpt-5\"\n",
	})

	if _, err := Apply(runtimeDir, "lead", "codex"); err != nil {
		t.Fatalf("injection must succeed, got %v", err)
	}
...
func TestApply_InjectsBothHarnessRoots(t *testing.T) {
	clearProfileEnv(t)
	stubHarnessesOnPath(t, "claude", "codex")
	runtimeDir := t.TempDir()
	claudeDir := writeAgentProfile(t, runtimeDir, "lead", fakeHarnessVersion, map[string]string{
		"settings.json": `{"model":"opus"}`,
	})
	codexDir := writeAgentHarnessProfile(t, runtimeDir, "lead", "codex", fakeHarnessVersion, map[string]string{
		"config.toml": "model = \"gpt-5\"\n",
	})

	for _, harness := range ProfileHarnesses() {
		if _, err := Apply(runtimeDir, "lead", harness); err != nil {
...
func TestApply_NoProfileOnDiskLeavesEnvUnset(t *testing.T) {
	clearProfileEnv(t)
	// No stub on PATH: an unprofiled lead must not even probe the harness.
	runtimeDir := t.TempDir()
	...
	for _, harness := range ProfileHarnesses() {
		...
		if got := os.Getenv(ProfileEnvVar(harness)); got != "" {
			t.Fatalf("%s = %q, want unset", ProfileEnvVar(harness), got)
		}
	}
}
...
func TestApply_UnverifiableProfileRefusesInjection(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	dir := filepath.Join(runtimeDir, ".loom", agentprofile.DirName, "lead", "claude")
	...
	failed, err := Apply(runtimeDir, "lead", "claude")
	if !errors.Is(err, ErrProfileManifestMissing) {
		t.Fatalf("want missing manifest, got %v", err)
	}
	if failed != dir {
	...
	if got := Repair(err, failed); got != "scripts/provision-profile.sh lead" {
...
// on purpose.
func TestApply_ValidInheritedValueIsPreserved(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	inherited := writeAgentProfile(t, runtimeDir, "nova", fakeHarnessVersion, map[string]string{
		"settings.json": `{"model":"opus"}`,
	})
	writeAgentProfile(t, runtimeDir, "lead", fakeHarnessVersion, map[string]string{
...
func TestApply_InvalidInheritedValueRefuses(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	inherited := writeAgentProfile(t, runtimeDir, "lead", "2.1.235 (Claude Code)", map[string]string{
		"settings.json": `{"model":"opus"}`,
	})
	t.Setenv("CLAUDE_CONFIG_DIR", inherited)

	dir, err := Apply(runtimeDir, "lead", "claude")
	if !errors.Is(err, ErrProfileVersionDrift) {
		t.Fatalf("want version drift, got %v", err)
	}
	if got := Repair(err, dir); got != "loom doctor --fix" {
...
func TestApply_OutsideConfigRootIsLeftAlone(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	...
	writeAgentProfile(t, runtimeDir, "lead", "2.1.235 (Claude Code)", map[string]string{
		"settings.json": `{"model":"opus"}`,
	})
	outside := filepath.Join(t.TempDir(), ".claude")
	if err := os.MkdirAll(outside, 0o750); err != nil {
...
func TestApply_UnusableAgentNameDegradesToLegacyEnv(t *testing.T) {
	clearProfileEnv(t)
	runtimeDir := t.TempDir()
	for _, agent := range []string{"", "..", "../escape", "a/b"} {
		dir, err := Apply(runtimeDir, agent, "claude")
		if err != nil || dir != "" {
...
func TestApply_UnresolvableWorkspaceRootInjectsNothing(t *testing.T) {
	clearProfileEnv(t)
	if dir, err := Apply("", "lead", "claude"); err != nil || dir != "" {
...
func TestHarnessBinaryAndEnvVarResolveForEveryHarness(t *testing.T) {
	for _, harness := range ProfileHarnesses() {
		if got := ProfileHarnessBinary(harness); got == "" {
			t.Errorf("harness %q has no version-pin binary", harness)
		}
		if got := ProfileEnvVar(harness); got == "" {
			t.Errorf("harness %q has no config-root variable", harness)
		}
	}
}

// clearProfileToken makes CLAUDE_CODE_OAUTH_TOKEN unset for the test and restored
// after it, so an injection is visible as a change and never leaks out.
func clearProfileToken(t *testing.T) {
	t.Helper()
...
func TestApply_InjectsProfileOAuthToken(t *testing.T) {
	clearProfileEnv(t)
	clearProfileToken(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
...
func TestApply_InheritedRootStillYieldsItsToken(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	inherited := writeAgentProfile(t, runtimeDir, "lead", fakeHarnessVersion, map[string]string{
		"settings.json": `{"model":"opus"}`,
	})
...
func TestApply_OutsideConfigRootLeavesTokenAlone(t *testing.T) {
	clearProfileEnv(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	outside := t.TempDir()
...
func TestApply_NoTokenFileLeavesTokenUnset(t *testing.T) {
	clearProfileEnv(t)
	clearProfileToken(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
...
func TestRepair_TokenFailureNamesTheTokenScript(t *testing.T) {
	clearProfileEnv(t)
	clearProfileToken(t)
	stubClaudeOnPath(t)
	runtimeDir := t.TempDir()
	...
	if got, want := Repair(err, failed), "scripts/setup-profile-token.sh lead"; got != want {
