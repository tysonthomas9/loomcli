...
/**
 * Unit tests for RejectCommentForm component.
 */
...
describe('RejectCommentForm', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders label with question text', () => {
      ...
      expect(screen.getByText('Why are you rejecting?')).toBeInTheDocument();
    });
    ...
    it('submit button shows "Submit"', () => {
      render(<RejectCommentForm {...defaultProps} />);

      expect(screen.getByTestId('reject-submit')).toHaveTextContent('Submit');
    });
    ...
    it('textarea has associated label via htmlFor', () => {
      ...
      expect(textarea).toHaveAttribute('id', 'reject-textarea-my-issue');
    });
  });
  ...
  describe('click propagation', () => {
    it('form click does not propagate to parent', () => {
      const parentClick = vi.fn();
      render(
        <div onClick={parentClick}>
          <RejectCommentForm {...defaultProps} />
        </div>
      );

      fireEvent.click(screen.getByTestId('reject-comment-form'));

      expect(parentClick).not.toHaveBeenCalled();
    });

    it('cancel button click does not propagate to parent', () => {
      const parentClick = vi.fn();
      render(
        <div onClick={parentClick}>
          <RejectCommentForm {...defaultProps} />
    ...
    it('submit button click does not propagate to parent', () => {
      const parentClick = vi.fn();
      render(
        <div onClick={parentClick}>
          <RejectCommentForm {...defaultProps} />
