...
func (s *Server) handleRequest(req *Request, connCtx ...context.Context) Response {
	...
	if req.Operation != OpPing && req.Operation != OpHealth && req.Operation != OpMetrics &&
		...
		// single-flight lock. Prevents unbounded goroutine fan-out when many
		// requests race to check staleness, and avoids sharing the per-request
		// *Request pointer with background work.
		if s.importInProgress.CompareAndSwap(false, true) {
			go func() {
				if err := s.checkAndAutoImportIfStale(); err != nil {
