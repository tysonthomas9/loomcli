#!/usr/bin/env bash
# Re-validate landed fixes against a throwaway local-mode stack.
#
#   make local-mode-up && go build -o loom-validate ./cmd/loom \
...
export LOOM_FLEET_DB_URL="${LOOM_FLEET_DB_URL:-http://127.0.0.1:${LOCAL_MODE_FLEETDB_PORT:-8280}}"
export LOOM_WORKSPACE="${LOOM_WORKSPACE:-LOCALMODE}"
...
if [ "${VALIDATE_ALLOW_TARGET:-0}" != "1" ]; then
  ...
  if [ -n "$refuse" ]; then
    ...
    echo "Point it at the throwaway stack (make local-mode-up), or set" >&2
...
curl -fsS -m 5 "$LOOM_FLEET_DB_URL/healthz" >/dev/null 2>&1 || {
  echo "test stack is not up at $LOOM_FLEET_DB_URL — run: make local-mode-up" >&2; exit 1; }
...
go_test_pinned "no-work exit preserves the restart budget" \
  ...
  'TestShouldRestart_NoWorkPreservesFailureBudget|TestShouldRestart_CleanSuccessClearsBudgetAfterNoWork|TestShouldRestart_FailingIdleAgentReachesMaxRetries' \
