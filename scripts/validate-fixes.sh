...
if go test ./internal/cli/daemon/supervisor/ \
  ...
  pass "no-work exit preserves the restart budget"
else
  fail "no-work exit preserves the restart budget" "see /tmp/d36.log"
fi
...
if [ "${repos:-0}" -le 1 ]; then
  ...
  if go test ./internal/driver/ -run 'TestSelectRepo_' -count=1 >/tmp/d47.log 2>&1; then
    pass "arbitrary repo pick is logged (${repos} repos registered)"
  else
    fail "arbitrary repo pick is logged" "see /tmp/d47.log"
  fi
fi
