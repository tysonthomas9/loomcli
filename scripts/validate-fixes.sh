...
if curl -fsS -m 5 "$api/api/workspaces" >/dev/null 2>&1; then
  ...
  if [ -n "$bare" ] && [ -n "$filt" ] && [ "$filt" -ge "$bare" ] 2>/dev/null; then
    pass "repo filter keeps unscoped issues ($bare -> $filt)"
  elif [ "${bare:-0}" -eq 0 ] 2>/dev/null; then
    skip "repo filter" "no ready issues to discriminate with"
  else
    fail "repo filter keeps unscoped issues" "unfiltered=$bare filtered=$filt"
  fi
else
...
if [ -x "$fe/node_modules/.bin/vitest" ]; then
  if (cd "$fe" && node_modules/.bin/vitest run src/__tests__/App.test.tsx \
    ...
    fail "plan approve clears needs-revision" "see /tmp/d27.log"
  fi
else
...
skip "no-work respawn budget" "needs sustained observation; see the procedure in this script"
...
if [ "${repos:-0}" -gt 1 ]; then
  skip "arbitrary repo pick is logged" "multi-repo stack present; needs a dispatch to observe"
else
  skip "arbitrary repo pick is logged" "single-repo workspace cannot trigger the fallback"
fi
