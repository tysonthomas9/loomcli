...
probe=$(curl -s -w '\n%{http_code}' "$url/api/workspaces/NOPE/readyz")
body=$(printf '%s\n' "$probe" | sed '$d')
code=$(printf '%s\n' "$probe" | tail -n 1)

if [ "$code" = "404" ]; then
	echo "[repro] expected /readyz route; got 404 (route missing)"
	exit 1
fi

if [ "$code" != "503" ]; then
	echo "[repro] expected /readyz 503, got $code"
	printf '%s\n' "$body"
	exit 1
fi
...
echo "[repro] OK - /readyz route returns 503 with diagnostic reason"
