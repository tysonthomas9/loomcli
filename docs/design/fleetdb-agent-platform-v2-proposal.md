...
## Phase 0 Contract Lock (V2 Phased Delivery)

> Status: Locked 2026-06-03. Companion:
> `docs/design/fleetdb-agent-platform-v2-phased-delivery.md`.

V2 is delivered in provable phases (see the phased-delivery doc). Phase 0 fixes
the foundational contract so no later phase reintroduces a parallel scheduler.

### The claim contract — FleetDB is the sole authority

FleetDB — not Loom, not a driver, not a runner — is authoritative for:

- ready-task discovery (the ready frontier);
- single-task claim (race-free, exactly one owner);
- task state transitions;
- dependency unlocks;
- task completion acceptance.

**Loom does not reimplement task claim.** Loom adds the dynamic-driver layer
*around* the contract: storing driver source/versions, running a `DriverRun`,
calling FleetDB-backed claim APIs through the Loom SDK, starting child
`TaskRun`s, connecting runtimes, and preserving logs/artifacts/patch-back. No
new scheduler, ready frontier, or task-concurrency system may be introduced
unless an existing FleetDB API cannot express the required operation — in which
case it is recorded as a *narrow FleetDB dependency*, never replaced by
Loom-side orchestration.

### SDK rule

Driver code may *request* a ready task, but FleetDB decides whether a task is
returned or already claimed. The driver never assigns work to itself; it asks,
and acts on FleetDB's answer.

### Claim semantics (answers the Phase 0 proof questions)

Observed behavior of the current implementation
(`fleet-db/internal/storage/claim.go`):

- **Two drivers ask for the same ready task** → claim is one atomic Lua op:
  `SET <ws>:locks:issue:<id> <worker> NX EX <ttl>`, then status → `in_progress`
  and `ZREM` from `<ws>:queue:ready`. Exactly one driver wins; the loser gets
  `ERR:ALREADY_CLAIMED:<holder>` (`ClaimResult.Success = false`).
- **The task is already claimed** → the existing owner is returned; no double
  assignment is possible.
- **What prevents dependents from unlocking early** → dependents stay off the
  ready frontier via `blocked_by` sets and ready-eligibility evaluation; FleetDB
  recomputes `queue:ready` only when it accepts a completion/close. A patch,
  commit, or PR unlocks nothing by itself (Principle 6).

### Dead-runner recovery (current behavior + the one narrow dependency)

If a runner dies mid-task today:

- the claim lock auto-expires after its TTL, so the task becomes
  **re-claimable**;
- but the issue's status stays `in_progress` and it is **not** automatically
...
The early phases use four records only — `Driver`, `DriverVersion`, `DriverRun`,
and links to the existing `TaskRun`. The richer entities below (TriggerBinding,
AgentService, WorkerProfile, DriverStep, ActionLedger, …) are layered in only
after the core claim → run → complete flow is proven.

See `docs/design/fleetdb-agent-platform-v2-execution-topology-addendum.md` for
the current implementation status and the concrete remediation plan for
DriverRun lifecycle, executor placement, child TaskRuns, recovery, and cloud
runtime topology. The block diagrams and richer entity catalog below describe
target-state V2 unless an implementation status is called out explicitly.
