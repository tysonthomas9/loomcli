...
- Driver SDK method:
  ...
  ```ts
  ctx.taskRuns.start({ taskId, runtime: "flue-daytona" })
  ```
...
DriverRun
  ...
  -> ctx.taskRuns.start(task_id, flue-daytona)
  -> remote task agent produces patch/log artifacts
  -> local Loom applies patch-back
  -> FleetDB accepts completion
...
- `ctx.taskRuns.wait(taskRunId)` to wait for each child task result.
...
export default defineDriver({
  ...
  async run(ctx) {
    while (true) {
      ...
      const child = await ctx.taskRuns.start({
        taskId: task.id,
        runtime: "flue-daytona",
      });

      const result = await ctx.taskRuns.wait(child.id);
