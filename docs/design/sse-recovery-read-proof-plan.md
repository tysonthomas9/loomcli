...
Status: recovery acknowledgment remains proposed. The internal [certified issue client and captured-source capability](../testing/sse-certified-issue-source-proof.md) are implemented, with an [authenticated recovery-handle bridge](../testing/sse-recovery-handle-proof.md), but without a frontend consumer or acknowledgment. The current frontend resync checkpoint repair is separate. This plan does not authorize resetting a resume checkpoint after ordinary REST refreshes.
...
This plan records the remaining whole-client recovery protocol. Certified Fleet issue reads, the Loom handle bridge, browser offer decoding and native preparation/HTTP API now exist; durable PostgreSQL source/incarnation binding now exists. Browser recovery attempt ownership now suspends the exact client and retains prepared data privately; see [the attempt proof](../testing/sse-recovery-attempt-proof.md). Cache-generation publication and acknowledgment remain unimplemented. Existing registered-query recovery remains useful and intentionally weaker. Fixed replay fences and connection-bound mutation sources are prerequisites, not substitutes for complete read coverage.
