...
- The task-ready lane exists and is **already enabled in the local-mode stack**: emission in `internal/trigger/issue_journal_bridge_task_ready.go:97-130` (payload `{taskId, status}`, route `internal.task.ready`), gated by `LOOM_TASK_READY_EVENTS` (`internal/cli/serve/serve_loops.go:264`, `:317-327`), bridge polls the journal every 2s (`:302-304`), and **`test/local-mode/docker-compose.yml:90` sets `LOOM_TASK_READY_EVENTS: "1"`**.
