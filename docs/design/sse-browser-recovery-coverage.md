...
| Issue-store writers | `runFetchIssues` merges by timestamps and preserves recent local creations; mutations, optimistic updates and rollback timers write separately | One publication generation must fence every writer. Timestamp ordering cannot certify snapshot membership, deletions or cleared fields. |
...
| Selected detail | App uses `hooks/issues/useIssueDetail.ts`, with local detail state and separate requests | This is not a recovery participant. Detail dependencies, dependents and comments are outside the issue manifest. Workspace retirement is now guarded, but certified detail recovery remains missing. |
| Detail comments and history | `components/IssueDetailPanel/IssueDetailPanel.tsx` owns comments and events locally | These need their own authoritative read/coverage rules. A successful issue-store refresh does not prove either collection. |
