...
`claude` all mean something specific here. The glossary is a dictionary, not an architecture
narrative: it enumerates each overloaded term's senses and pins them to `path:line`. Consult
it first; for architecture read `docs/arch/README.md` and `docs/design/README.md`.

`docs/README.md` indexes the whole docs tree — what each subdirectory is for, the
per-doc status vocabulary, and which docs are generated. Start there when you need a
doc you cannot name.

## Sibling Checkouts (fleet-db, flue)

This repo is **not self-contained** for real E2E, container, or workflow-bundle
work. Two sibling repositories must be checked out and, for flue, built:

| Repo | Required by | Failure without it |
|---|---|---|
| `fleet-db` | `make test-distributed-smoke` (`Makefile:243`), real Playwright lanes (`scripts/start-e2e-server.sh:87-94`), `scripts/test-podman-stack.sh` (`deploy/podman-stack/build.sh:28,41`) | build step exits before anything runs |
| `flue` | `make test-builtin-workflows` (`Makefile:59-62`), podman-stack images (`deploy/podman-stack/build.sh:29,42`) | podman-stack: `die "flue repo not found"` / `die "flue CLI dist missing"` (`build.sh:42,47-50`). `test-builtin-workflows` fails differently: `ERROR: @flue/runtime not found; build ../flue or set FLUE_REPO` (`scripts/test-builtin-workflows.sh:14-16`) |

Every consumer resolves the checkout with its own default — at least five
distinct resolutions exist, and they disagree on **depth** (`../` vs `../../`),
so **set the env vars explicitly** instead of relying on a default:
...
- `Makefile:475-477` probes `../fleet-db` then `../../fleet-db`.
- `scripts/start-e2e-server.sh:14` tries only `<repo>/../../fleet-db`.
- `deploy/podman-stack/build.sh:28-29` tries only `<repo>/../fleet-db` and `<repo>/../flue`.
- `desktop/scripts/prepare-sidecar.sh:8` tries only `<repo>/../fleet-db`.
- `scripts/test-builtin-workflows.sh:14`, `rebuild-builtin-bundle.sh:19`,
  `test-flue-durability-store.sh:18` and `test-lead-epic-runner-playwright.sh:7`
  try only `<repo>/../flue`.
- `e2e/run_epic_runner_real_codex_octocat_podman.sh:8,10` uses **two different
  depths in one file** — `../../fleet-db` and `../flue`.

If a checkout is missing, report the run as **blocked**, not failed, and never
substitute mocked state for the real leg — see the terminology handshake below.

`make build`, `make check-go`, `make check-frontend`, `make gate` (= `check`,
`Makefile:554,578`), `make dev`, and the Go unit tests need neither sibling.
`make gate-e2e` does: its Playwright run boots
`scripts/start-e2e-server.sh` as the webServer
(`internal/webui/frontend/playwright.config.ts:73`).
...
`docs/api.md` is generated — never hand-edit it. Change `api/openapi.yaml` for
endpoint facts, or `docs/api.preamble.md` / `docs/api.appendix.md` for prose,
then run `make gen-api-docs`. `make check-go` step [12/15]
(`scripts/check-api-docs-staleness.sh`) fails when the committed file does not
match its inputs, including the route registrations under `internal/webui`.

`internal/backend/api/gen/types.gen.go` is likewise generated from
`api/openapi.yaml` via `make gen-go-api` and gated by step [11/15].

`docs/reference/{env-vars,cli,architecture}.md` are generated by
`scripts/loomdoc` from the Go source in this repo — never hand-edit them. Edit the
matching `docs/reference/*.preamble.md`, then run `make docs-gen` (which also
refreshes `docs/api.md`). Gated by step [13/15]
(`scripts/check-loomdoc-staleness.sh`).

The generator excludes *gitignored* files, not merely uncommitted ones, so a
new `doc.go` still appears before you commit it (`scripts/loomdoc/common.go:59-67`).
Adding a package to `.gitignore` does drop it from the reference docs entirely.
...
`delegated` is for T3/T4 deployments (UNVERIFIED: the deployment tiers
"T3"/"T4" are named here and in `deploy/podman-stack/README.md` but are not
defined anywhere in this repo — read it as "orchestrated multi-node") where a
...
`LOOM_SANDBOX_PODMAN_TEST=1 go test ./internal/driver/sandbox -run
...
- [`docs/loom-glossary.md`](docs/loom-glossary.md) — mandatory first read; the
  overloaded-term dictionary, each sense pinned to `path:line`
- [`docs/README.md`](docs/README.md) — index of the whole `docs/` tree, the
  status-banner vocabulary, and the generated-file rules
- [`docs/testing-terminology.md`](docs/testing-terminology.md) — the four axes,
  the trap words, and the handshake protocol this file enforces
- [`docs/agents/issue-tracker.md`](docs/agents/issue-tracker.md) — `loom data`
  runbook and the fleet-db backend split
- [`docs/agents/domain.md`](docs/agents/domain.md) — which domain docs to read
- [`.agent-skills/loom-pr-test/SKILL.md`](.agent-skills/loom-pr-test/SKILL.md) —
  real Loom PR runtime testing
- [`README.md`](README.md) — human-facing overview, CLI surface, env vars
- [`CLAUDE.md`](CLAUDE.md) — repo rules and pointers into `docs/agents/`
