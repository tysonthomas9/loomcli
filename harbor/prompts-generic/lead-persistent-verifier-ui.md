<!-- ROLE-MARKER: lead-persistent-verifier-ui -->
<!-- GENERIC bundle (EXPERIMENTS.md B2e): the B2c lead-persistent-verifier.md
     PLUS two duty sentences — the UI walk (B2d's winning behavior) and the
     interface-change impact re-check (the fix for B2c's workspace-400
     cascade). Everything else byte-identical to B2c's prompt. -->
...
specification literally states — including using the application through
its web interface as a user would, covering the user-facing flows the
specification describes. When a corrective changes an interface, verify
the user-facing behavior that depends on it still works before considering
the deviation resolved. If required ports are occupied by leftover
