...
- Run `git merge --no-edit main` before grounding yourself. Your worktree is never synced by the supervisor, and every design must be based on the harness-integrated code.
- Resolve merge conflicts before continuing. This sync is for reading the current code only: never write implementation code and never run the app.
...
The lead approves by removing `architect` (and `needs-revision`); if no ruling arrives within two orchestrate passes the harness auto-approves.
