...
func main() { //nolint:funlen // Small e2e probe keeps flag parsing and attach flow in one executable path.
	...
	for name, v := range map[string]string{
		...
		if v == "" {
			fatalf("agent-attach-check: %s is required", name)
		}
	}

	caBytes, err := os.ReadFile(*agentdCA)
	if err != nil {
		fatalf("agent-attach-check: read -agentd-ca: %v", err)
	}
	...
	if err != nil {
		fatalf("agent-attach-check: agentd.New: %v", err)
	}
	...
	for i := 0; i < maxAttempts; i++ {
		...
		if i == maxAttempts-1 {
			fatalf("agent-attach-check: AttachSession (after %d attempts): %v", maxAttempts, err)
		}
		warnf("agent-attach-check: AttachSession attempt %d failed: %v; retrying in 2s", i+1, err)
	...
	if _, err := att.WriteInput(cmd); err != nil {
		fatalf("agent-attach-check: WriteInput: %v", err)
	}
	...
	for time.Now().Before(deadline) {
		...
		case <-readDone:
			...
			fatalf("agent-attach-check: output closed before %q seen; reason=%q got=%q", *expect, reason, tail)
	...
	tail := outputBuf.String()
	mu.Unlock()
	fatalf("agent-attach-check: timeout %ds waiting for %q; got=%q", *deadlineSecs, *expect, tail)
}
